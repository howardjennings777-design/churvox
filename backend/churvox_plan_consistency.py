...
PLAN_META = {
    "solo": {
        "name": "Start",
        "price": 39,
        "price_label": "$39/month + GST",
        "max_workers": 0,
        "max_team_members": 1,
        "max_clients": 20,
        "sms": False,
        "xero_addon_available": False,
        "myob": False,
        "team": False,
        "payroll": False,
        "ai_operator": False,
    ...
    "pro": {
        "name": "Operator",
        "price": 149,
        "price_label": "$149/month + GST",
        "max_workers": 20,
        "max_team_members": 21,
        "max_clients": 40,
        "sms": False,
        "xero_addon_available": True,
        "myob": False,
        "team": True,
        "payroll": False,
        "ai_operator": True,
    ...
    "enterprise": {
        ...
        "scheduling": True,
        "extra_blocks": True,
        "growth_pack_price": 99,
    },
}


def _price_envs(plan):
    p = _normal_plan(plan)
    if p == "solo":
...
def _patch_globals():
    ...
    app.PLAN_PRICE_IDS = {key: _first_env(_price_envs(key))[0] for key in PLAN_META.keys()}

    def get_stripe_price_id(plan):
        normal = _normal_plan(plan)
        price_id, env_name = _first_env(_price_envs(normal))
        if not price_id:
            HTTPException = getattr(app, "HTTPException", None)
            if HTTPException:
                raise HTTPException(status_code=400, detail=f"Missing Stripe price ID for {PLAN_META[normal]['name']}. Add {env_name} in Render.")
            raise RuntimeError(f"Missing Stripe price ID for {normal}")
        return price_id
    ...
    async def set_business_plan_from_checkout(user_id: str, plan: str, stripe_customer_id: str = None, stripe_subscription_id: str = None):
        ...
        update = {
            "plan": normal,
            "plan_name": PLAN_META[normal]["name"],
            "plan_price": PLAN_META[normal]["price"],
            "stripe_customer_id": stripe_customer_id,
            "stripe_subscription_id": stripe_subscription_id,
            "updated_at": datetime.now(timezone.utc),
        }
        await db.users.update_one({"_id": business_id}, {"$set": update})
        await db.users.update_many({"business_id": business_id, "role": {"$in": ["worker", "manager", "office_admin", "payroll"]}}, {"$set": {"plan": normal, "plan_name": PLAN_META[normal]["name"]}})
...
def install(router):
    ...
    async def plan_metadata():
        return {"success": True, "plans": PLAN_META, "key_map": {"Start": "solo", "Crew": "team", "Operator": "pro", "Command": "enterprise"}}
    ...
    async def create_checkout(payload: dict, request):
        ...
        price_id, env_name = _first_env(_price_envs(plan))
        if not price_id:
            return {"success": False, "error": f"Missing Stripe price ID for {PLAN_META[plan]['name']}. Add {env_name} in Render."}
        ...
        args = {
            ...
            "success_url": f"{frontend}/billing/success?session_id={{CHECKOUT_SESSION_ID}}&plan={plan}",
            "cancel_url": f"{frontend}/billing/cancel",
            "metadata": {"user_id": user.get("id"), "business_id": business_id, "plan": plan, "plan_name": PLAN_META[plan]["name"], "purpose": "plan_subscription"},
            "subscription_data": {"metadata": {"user_id": user.get("id"), "business_id": business_id, "plan": plan, "purpose": "plan_subscription"}},
        }
        ...
        try:
            ...
            return {"success": True, "url": session.url, "checkout_url": session.url, "session_id": session.id, "plan": plan, "plan_name": PLAN_META[plan]["name"]}
