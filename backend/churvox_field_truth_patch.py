...
def job_lookup_query(user, ObjectId, job_id):
    job_values = id_values(job_id, ObjectId)
...
def normalize_steps(payload):
    ...
    if isinstance(raw_steps, dict):
        for key, value in raw_steps.items():
            clean_key = normalize_step_key(key)
            if clean_key in PROOF_STEPS:
                steps[clean_key] = bool(value)
...
async def get_passport(db, user, ObjectId, job_id):
    ...
    query = {"business_id": business_id, "job_id": str(job_id)}
    passport = await safe_find_one(db.worker_proof_passports, query)
    if not passport:
        passport = {
    ...
    try:
        ...
        slips_count = 0
    readiness = proof_readiness(passport.get("steps") or {}, photos_count, slips_count)
...
async def save_passport(db, user, ObjectId, job_id, payload):
    ...
    current_steps = dict(existing.get("steps") or {})
    current_steps.update(normalize_steps(payload or {}))
    update = {
        "business_id": business_id,
        "job_id": str(job_id),
        "worker_id": worker_id,
    ...
    if isinstance(payload, dict):
        if payload.get("gps") or payload.get("gps_event"):
            gps = payload.get("gps") or payload.get("gps_event")
            if isinstance(gps, dict):
                gps = {**gps, "at": gps.get("at") or now}
                update.setdefault("$push", {})
        if payload.get("note") or payload.get("worker_note"):
            note_text = clean(payload.get("note") or payload.get("worker_note"))
            if note_text:
                update["latest_worker_note"] = note_text
        if payload.get("finish_summary"):
            update["finish_summary"] = clean(payload.get("finish_summary"))
        if payload.get("offline_token"):
            update["last_offline_token"] = clean(payload.get("offline_token"))
    try:
        await db.worker_proof_passports.update_one(
            {"business_id": business_id, "job_id": str(job_id)},
    ...
    try:
        await db.jobs.update_one(
            job_lookup_query(user, ObjectId, job_id),
            {"$set": {
...
async def list_field_slips(db, user, ObjectId, status=None, limit=80):
    ...
    if status:
        ...
        query["status"] = {"$in": ["waiting_owner_review", "needs_owner_edit", "parked"]}
    rows = await safe_recent(db.worker_field_slips, query, limit, "updated_at")
    items = [command_item_from_slip(row) for row in rows]
    return rows, items


async def decide_slip(db, user, ObjectId, slip_id, decision, payload=None):
    ...
    update = {
        ...
        "decision_note": clean((payload or {}).get("note") or (payload or {}).get("decision_note")),
        "decided_by": user_id_string(user),
        "decided_at": now,
        "updated_at": now,
        "auto_sent": False,
        "accounting_synced": False,
    ...
    try:
        await db.command_decisions.insert_one({
            ...
            "payload": payload or {},
            "source": "worker_field_truth",
            "note": "Owner decision recorded. Churvox did not auto-send, file tax, sync accounting, or create payout files.",
            "created_at": now,
            "updated_at": now,
        })
    except Exception:
...
async def enhanced_snapshot(db, user, ObjectId):
    ...
    counts["worker_field_slips"] = field_count
    counts["total"] = len(combined)
    briefing = dict(snapshot.get("briefing") or {})
    if field_count:
        briefing["summary"] = f"Churvox found {field_count} worker field slip(s) plus {max(0, len(combined) - field_count)} other Command item(s)."
    snapshot.update({
        "success": True,
        "actions": combined,
        "items": combined,
        "command_items": combined,
        "worker_field_slips": json_safe(rows),
        "counts": counts,
        "briefing": briefing or {"summary": "Churvox is checking worker proof, jobs, invoices, quotes and timesheets."},
    })
...
def install(module):
    ...
    async def worker_status_endpoint(request: Request):
        ...
        return json_safe({
            "success": True,
            "mode": "field_truth",
            "worker_flow": ["Today", "Jobs", "Proof", "Help", "Me"],
            "fair_gps": {
                "enabled": True,
                "rule": "GPS is job proof only while the worker is clocked into work.",
                "after_hours_tracking": False,
            },
            "proof_passport": PROOF_STEPS,
    ...
    async def offline_sync_endpoint(request: Request):
        user = await get_current_user(request)
        payload = await read_payload(request)
        return json_safe(await offline_sync(db, user, ObjectId, payload))

    async def owner_job_passport_endpoint(request: Request, job_id: str):
        user = await get_current_user(request)
        passport = await get_passport(db, user, ObjectId, job_id)
        photos = await safe_recent(db.worker_proof_photos, {"business_id": business_id_string(user), "job_id": str(job_id), "status": {"$ne": "deleted"}}, 20, "created_at")
        slips = await safe_recent(db.worker_field_slips, {"business_id": business_id_string(user), "job_id": str(job_id)}, 20, "created_at")
        return json_safe({"success": True, "passport": passport, "photos": photos, "slips": slips})
    ...
    async def decide_slip_endpoint(request: Request, slip_id: str, decision: str):
        user = await get_current_user(request)
        payload = await read_payload(request)
        return json_safe(await decide_slip(db, user, ObjectId, slip_id, decision, payload))
    ...
    for decision in ["approve", "edit", "park", "reject", "dismiss"]:
        path = f"/api/command/field-slips/{{slip_id}}/{decision}"
        remove_route(app, path, "POST")
        app.add_api_route(path, decide_slip_endpoint, methods=["POST"])
