...
def _addon(addon):
    ...
    if key in ["xero", "xero_addon", "xero_sync"]:
        return {"key": "xero_addon", "name": "Xero add-on", "requires_plan": "pro", "price_envs": ["STRIPE_PRICE_XERO_ADDON", "STRIPE_PRICE_XERO", "STRIPE_XERO_ADDON_PRICE_ID"], "active_field": "xero_addon_active", "subscription_field": "stripe_xero_addon_subscription_id"}
    if key in ["command_growth_pack", "growth_pack", "command_growth"]:
        return {"key": "command_growth_pack", "name": "Command Growth Pack", "requires_plan": "enterprise", "price_envs": ["STRIPE_PRICE_COMMAND_GROWTH_PACK", "STRIPE_PRICE_GROWTH_PACK", "STRIPE_COMMAND_GROWTH_PACK_PRICE_ID"], "active_field": "command_growth_pack_active", "subscription_field": "stripe_command_growth_pack_subscription_id"}
    return None


def _addon_price_envs(cfg, country):
    c = _country(country)
    key = cfg["key"].upper()
    if cfg["key"] == "xero_addon":
        return [f"STRIPE_PRICE_XERO_ADDON_{c}", f"STRIPE_PRICE_{c}_XERO_ADDON", f"STRIPE_PRICE_XERO_{c}", f"STRIPE_PRICE_{c}_XERO", *cfg["price_envs"]]
    return [f"STRIPE_PRICE_COMMAND_GROWTH_PACK_{c}", f"STRIPE_PRICE_{c}_COMMAND_GROWTH_PACK", f"STRIPE_PRICE_GROWTH_PACK_{c}", f"STRIPE_PRICE_{c}_GROWTH_PACK", *cfg["price_envs"]]
...
def install(router):
    ...
    async def create_addon_checkout(payload: dict, request):
        ...
        price_id, env_name = _first_env(_addon_price_envs(cfg, country))
        if not price_id:
            return {"success": False, "error": f"Missing Stripe price env for {cfg['name']} {country}. Add {env_name} in Render."}
        ...
        args = {"mode": "subscription", "line_items": [{"price": price_id, "quantity": 1}], "success_url": f"{frontend}/billing/success?addon={cfg['key']}&country={country}&session_id={{CHECKOUT_SESSION_ID}}", "cancel_url": f"{frontend}/plans?addon_cancelled=1&addon={cfg['key']}&country={country}", "metadata": {"purpose": "addon_subscription", "addon": cfg["key"], "business_id": business_id, "owner_user_id": owner_id, "country": country}, "subscription_data": {"metadata": {"purpose": "addon_subscription", "addon": cfg["key"], "business_id": business_id, "owner_user_id": owner_id, "country": country}}}
        ...
        try:
            ...
            await db.billing_addon_sessions.update_one({"stripe_session_id": session.id}, {"$setOnInsert": {"business_id": business_id, "owner_user_id": owner_id, "addon": cfg["key"], "country": country, "stripe_session_id": session.id, "status": "created", "created_at": datetime.now(timezone.utc)}}, upsert=True)
            return {"success": True, "url": session.url, "checkout_url": session.url, "session_id": session.id, "country": country}
    ...
    async def confirm_addon_checkout(payload: dict, request):
        ...
        update = {cfg["active_field"]: True, cfg["subscription_field"]: subscription_id, "billing_country": country, "business_country": country, "country": country, "updated_at": datetime.now(timezone.utc)}
        if customer_id:
        ...
        await db.billing_addon_sessions.update_one({"stripe_session_id": session_id}, {"$set": {"business_id": business_id, "addon": cfg["key"], "country": country, "status": "confirmed", "stripe_subscription_id": subscription_id, "stripe_customer_id": customer_id, "confirmed_at": datetime.now(timezone.utc)}}, upsert=True)
        return {"success": True, "message": f"{cfg['name']} activated", "addon": cfg["key"], "country": country}
