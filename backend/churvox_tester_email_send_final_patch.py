...
def install(module):
    ...
    async def tester_intake(request: Request, payload: dict = Body(default={})):
        ...
        if payload.get("send_email", True):
            email_status = await send_tester_email(email, payload, existing, access_link)
        result = {"tester": tester_doc, "access_link": access_link, "email": email_status}
        await db.app_owner_control_log.insert_one({"created_at": now_utc(), "owner_email": email_of(owner), "action": "tester_intake", "target_email": email, "payload": safe(payload), "result": safe(result)})
        return safe({
            "success": True,
