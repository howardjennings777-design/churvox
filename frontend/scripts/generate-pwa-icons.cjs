...
function roundedMask(x, y, r = 0.225) { const rx = Math.abs(x - 0.5) - (0.5 - r); const ry = Math.abs(y - 0.5) - (0.5 - r); const qx = Math.max(rx, 0); const qy = Math.max(ry, 0); const outside = Math.hypot(qx, qy) + Math.min(Math.max(rx, ry), 0) - r; return 1 - smooth(-0.008, 0.008, outside); }
...
function iconColor(x, y) {
  ...
  if (mask > 0) {
    ...
    c = [Math.round(mix(23, 5, warmth)), Math.round(mix(31, 8, warmth)), Math.round(mix(27, 11, warmth)), Math.round(255 * mask)];
    const rim = Math.abs(Math.min(x, y, 1 - x, 1 - y) - 0.045) < 0.006 ? 1 : 0;
    c = blend(c, [255, 255, 255, 255], rim * 0.08 * mask);
    const glow = Math.exp(-((x - 0.30) ** 2 + (y - 0.18) ** 2) / 0.045);
    c = blend(c, [249, 115, 22, 255], glow * 0.20 * mask);
  }

  const topArc = arcAlpha(x, y, 0.51, 0.52, 0.405, 0.045, 206, 318);
  const bottomArc = arcAlpha(x, y, 0.52, 0.52, 0.405, 0.045, 40, 158);
  c = blend(c, [249, 115, 22, 255], Math.max(topArc, bottomArc) * 0.94);

  const white = Math.max(
    roundSegmentAlpha(x, y, 0.59, 0.30, 0.42, 0.30, 0.088),
    roundSegmentAlpha(x, y, 0.42, 0.30, 0.29, 0.43, 0.088),
    roundSegmentAlpha(x, y, 0.29, 0.43, 0.29, 0.57, 0.088),
    roundSegmentAlpha(x, y, 0.29, 0.57, 0.42, 0.70, 0.088),
    roundSegmentAlpha(x, y, 0.42, 0.70, 0.59, 0.70, 0.088),
    roundSegmentAlpha(x, y, 0.42, 0.30, 0.53, 0.70, 0.088),
    roundSegmentAlpha(x, y, 0.53, 0.70, 0.77, 0.30, 0.088)
  );
  c = blend(c, [12, 15, 18, 255], white * 0.22);
  c = blend(c, [248, 250, 252, 255], white);

  const orangeCut = Math.max(
    softSegmentAlpha(x, y, 0.29, 0.43, 0.29, 0.57, 0.032),
    softSegmentAlpha(x, y, 0.53, 0.70, 0.77, 0.30, 0.032)
  );
  c = blend(c, [249, 115, 22, 255], orangeCut * 0.96);
  return c;
}
