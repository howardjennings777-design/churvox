...
async function main() {
  ...
  const steps = [
    ...
    playwright('Hardened signup, login and role contract', ['tests/e2e/churvox-auth-paid-launch-contract.spec.js']),
    playwright('Public customer document safety and payment state', ['tests/e2e/churvox-public-documents-paid-launch.spec.js']),
    playwright('Public customer request owner-review flow', ['tests/e2e/churvox-public-request-paid-launch.spec.js']),
    playwright('Desktop and mobile More navigation', ['tests/e2e/churvox-more-menu-paid-launch.spec.js']),
  ...
  if (enabled(env.CHURVOX_E2E_SIGNUP)) {
    steps.push(playwright('Real signup, Stripe and verification-email audit', ['tests/e2e/churvox-real-signup-paid-launch-v2.spec.js', 'tests/e2e/churvox-email-verification-paid-launch-v2.spec.js']));
  } else {
