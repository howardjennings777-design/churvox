...
import /* DISABLED_CONFLICT_CommandShell */ from "./components//* DISABLED_CONFLICT_CommandShell */";
import ConceptCFrame from ".//* DISABLED_CONFLICT_concept-c *//ConceptCFrame";
...
function BusinessRoute({ children }) { const { user, loading, isWorker, isPayroll, hasAppAccess } = useAuth(); if (loading) return <Spinner />; if (!user) return <Navigate to="/login" replace />; if (isWorker) return <Navigate to="/worker/today" replace />; if (isPayroll) return <Navigate to="/payroll-board" replace />; if (!hasAppAccess) return <Navigate to="/plans" replace />; return </* DISABLED_CONFLICT_CommandShell */><AppPage>{children}</AppPage><//* DISABLED_CONFLICT_CommandShell */>; }
