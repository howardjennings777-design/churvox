...
import ExecutiveHomePage from "./pages/marketing/ExecutiveHomePage";
...
const Spinner = () => (
  <div className="min-h-screen flex items-center justify-center bg-[#f5f7f1] p-6 text-center text-slate-950">
    <div>
      <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-t-2 border-amber-400" />
      <p className="text-sm font-black uppercase tracking-[0.16em] text-orange-700">Loading Churvox</p>
      <h1 className="mt-2 text-3xl font-black tracking-[-0.05em]">Today Jobs Messages</h1>
      <p className="mt-2 text-sm font-bold text-slate-600">Getting your worker screen ready.</p>
    </div>
  </div>
);
...
function BusinessRoute({ children }) { const { user, loading, isWorker, isPayroll, hasAppAccess } = useAuth(); if (loading) return <Spinner />; if (!user) return <Navigate to="/login" replace />; if (isWorker) return <Navigate to="/worker/today" replace />; if (isPayroll) return <Navigate to="/payroll-board" replace />; if (!hasAppAccess) return <Navigate to="/plans" replace />; return <><AppPage>{children}</AppPage></>; }
function FreshBusinessRoute({ children }) { const { user, loading, isWorker, isPayroll, hasAppAccess } = useAuth(); if (loading) return <Spinner />; const setupAccess = typeof window !== "undefined" && (window.location.pathname === "/plans" || window.location.pathname === "/guide" || window.location.pathname === "/setup-guide" || window.location.search.includes("first_setup=1") || window.location.search.includes("checkout=saved") || window.location.search.includes("checkout=save_failed") || window.location.hash === "#setupassistant" || window.location.hash === "#firstrun"); if (!user && setupAccess) return <AppPage>{children}</AppPage>; if (!user) return <Navigate to="/login" replace />; if (isWorker) return <Navigate to="/worker/today" replace />; if (isPayroll) return <Navigate to="/payroll-board" replace />; if (!hasAppAccess && !setupAccess) return <Navigate to="/plans" replace />; return <AppPage>{children}</AppPage>; }
function OwnerRoute({ children }) { const { user, loading, isWorker, isPayroll } = useAuth(); if (loading) return <Spinner />; if (!user) return <Navigate to="/login" replace />; if (isWorker) return <Navigate to="/worker/today" replace />; if (isPayroll) return <Navigate to="/payroll-board" replace />; return <AppPage>{children}</AppPage>; }
function WorkerRoute({ children }) { const { user, loading } = useAuth(); if (loading) return <Spinner />; if (!user) return <Navigate to="/login" replace />; return <AppPage>{children}</AppPage>; }
...
function App() {
  ...
  return <BrowserRouter><AuthProvider><ErrorBoundary><Toaster position="top-right" richColors /><ChurvoxHelpWidget /><GlobalJobCreateModal /><HQPlanMixFloating /><Routes>
    <Route path="/guide" element={<FreshBusinessRoute><FreshApp /></FreshBusinessRoute>} /><Route path="/setup" element={<FreshBusinessRoute><FreshApp /></FreshBusinessRoute>} /><Route path="/setup-guide" element={<FreshBusinessRoute><FreshApp /></FreshBusinessRoute>} /><Route path="/operator-tools" element={<Navigate to="/command-board" replace />} /><Route path="/command-board" element={<BusinessRoute><CommandDeskRoute fallbackHref="/dashboard" fallbackLabel="Back to Dashboard" /></BusinessRoute>} /><Route path="/cockpit" element={<Navigate to="/dashboard#command" replace />} /><Route path="/smart-hub" element={<Navigate to="/dashboard" replace />} />
    <Route path="/public/proof/:token" element={<PublicProofPackPage />} />
    <Route path="/request" element={<PublicRequestPage />} /><Route path="/public/request" element={<PublicRequestPage />} /><Route path="/offline-sync" element={<PrivateRoute><OfflineSyncPage /></PrivateRoute>} />
    <Route path="/dispatch-board" element={<Navigate to="/dashboard#workers" replace />} /><Route path="/dispatch/map" element={<BusinessRoute><WorkerMapCommandPage /></BusinessRoute>} /><Route path="/crew-map" element={<Navigate to="/dashboard#workers" replace />} /><Route path="/dispatch" element={<Navigate to="/dashboard#workers" replace />} /><Route path="/schedule" element={<Navigate to="/dashboard#workers" replace />} /><Route path="/calendar" element={<Navigate to="/dashboard#workers" replace />} />
    ...
    <Route path="/onboarding" element={<BusinessRoute><OnboardingCommandPage /></BusinessRoute>} /><Route path="/jobs-board" element={<Navigate to="/dashboard#jobs" replace />} /><Route path="/jobs" element={<Navigate to="/jobs-board" replace />} /><Route path="/jobs/new" element={<BusinessRoute><JobFormPage /></BusinessRoute>} /><Route path="/jobs/:id" element={<BusinessRoute><null area="jobs"><JobDetailPage /></null></BusinessRoute>} /><Route path="/jobs/:id/edit" element={<BusinessRoute><JobFormPage /></BusinessRoute>} />
    <Route path="/clients/:clientId/workbench" element={<BusinessRoute><ClientWorkbenchCommandPage /></BusinessRoute>} /><Route path="/clients-board" element={<Navigate to="/dashboard#clients" replace />} /><Route path="/clients" element={<Navigate to="/clients-board" replace />} /><Route path="/clients/new" element={<BusinessRoute><null area="clients"><ClientFormPage /></null></BusinessRoute>} /><Route path="/clients/:id" element={<BusinessRoute><null area="clients"><ClientDetailPage /></null></BusinessRoute>} /><Route path="/clients/:id/edit" element={<BusinessRoute><null area="clients"><ClientFormPage /></null></BusinessRoute>} />
    <Route path="/quotes-board" element={<Navigate to="/dashboard#quotes" replace />} /><Route path="/quotes" element={<Navigate to="/quotes-board" replace />} /><Route path="/quotes/new" element={<BusinessRoute><null area="quotes"><QuoteFormPage /></null></BusinessRoute>} /><Route path="/quotes/:id" element={<BusinessRoute><null area="quotes"><QuoteDetailPage /></null></BusinessRoute>} /><Route path="/quotes/:id/edit" element={<BusinessRoute><null area="quotes"><QuoteFormPage /></null></BusinessRoute>} />
    <Route path="/invoices-board" element={<Navigate to="/dashboard#invoices" replace />} /><Route path="/invoices" element={<Navigate to="/invoices-board" replace />} /><Route path="/invoices/new" element={<BusinessRoute><null area="invoices"><InvoiceFormPage /></null></BusinessRoute>} /><Route path="/invoices/:id" element={<BusinessRoute><null area="invoices"><InvoiceDetailPage /></null></BusinessRoute>} /><Route path="/invoices/:id/edit" element={<BusinessRoute><null area="invoices"><InvoiceFormPage /></null></BusinessRoute>} />
    <Route path="/reports-board" element={<Navigate to="/dashboard#invoices" replace />} /><Route path="/reports" element={<Navigate to="/dashboard#invoices" replace />} /><Route path="/settings-board" element={<Navigate to="/dashboard#settings" replace />} /><Route path="/settings" element={<Navigate to="/settings-board" replace />} /><Route path="/support-board" element={<Navigate to="/dashboard#help" replace />} /><Route path="/support" element={<Navigate to="/dashboard#help" replace />} />
