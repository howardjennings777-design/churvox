...
function BillingReturnBridge({ cancelled = false }) { const params = new URLSearchParams(window.location.search || ""); const next = new URLSearchParams(); next.set("checkout", cancelled ? "cancelled" : "success"); const sessionId = params.get("session_id"); const plan = params.get("plan"); const country = params.get("country"); if (sessionId) next.set("session_id", sessionId); if (plan) next.set("plan", plan); if (country) next.set("country", country); return <Navigate to={`/plans?${next.toString()}`} replace />; }
