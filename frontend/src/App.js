...
function BusinessRoute({ children }) { const { user, loading, isWorker, isPayroll, hasAppAccess } = useAuth(); if (loading) return <Spinner />; if (!user) return <Navigate to="/login" replace />; if (isWorker) return <Navigate to="/worker/today" replace />; if (isPayroll) return <Navigate to="/payroll-board" replace />; if (!hasAppAccess) return <Navigate to="/plans" replace />; return <CommandShell><AppPage>{children}</AppPage></CommandShell>; }
