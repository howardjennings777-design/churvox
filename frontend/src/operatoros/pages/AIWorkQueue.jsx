...
function extractActions(payload) {
  return rowsFromPayload(payload).map(normaliseAction).filter(Boolean);
}
...
function mergeActions(first, second) {
  ...
  first.concat(second).forEach((item, index) => {
    const clean = normaliseAction(item, index);
  ...
  return (
    <main className="op-workspace op-ai-work-queue">
      <section className="op-workspace-head">
        ...
        <button type="button" className="primary" onClick={() => prepareAndLoad(false)} disabled={busy}>
