...
export default function PublicPricingPage() {
  return (
    <main className="pricepub">
      <header><a href="/" className="pricepub-brand"><img src="/brand/churvox-holo-c.svg" alt="" /><strong>CHURVOX</strong></a><nav><a href="/">Home</a><a href="/demo">Try live demo</a><a href="/signup">Start free trial</a><a href="/login">Sign in</a></nav></header>
      <section className="pricepub-hero"><p>CHURVOX PRICING</p><h1>Pricing built around your AI Operator.</h1><span>Churvox is not just job management. It prepares dispatch, proof-to-paid, invoice and follow-up actions for owner approval.</span><div><a href="/signup">Start free trial</a><a href="/demo">Try live demo</a></div></section>
      <section className="pricepub-grid">{plans.map(([name, price, caption, features]) => <article key={name} className={name === "Pro" ? "hot" : ""}><small>{name === "Pro" ? "MOST POPULAR" : name === "Enterprise" ? "MYOB INCLUDED" : "CHURVOX"}</small><h2>{name}</h2><p>{caption}</p><strong>{price}<span>/month NZD</span></strong><a href={name === "Enterprise" ? "/contact" : "/signup"}>{name === "Enterprise" ? "Email us" : "Start free trial"}</a><ul>{features.map((feature) => <li key={feature}>{feature}</li>)}</ul></article>)}</section>
