...
function emailOf(item) { return low(item?.email || item?.user_email || item?.owner_email || item?.tester_email || item?.target_email || item?.to); }
...
function normaliseTester(row, source = "tester") {
  ...
  return {
    ...base,
    email,
    name: text(base.name || base.full_name || base.business_name || base.company || email),
...
function mergeTesterData({ testerEndpoint, control, billing, optimistic }) {
  ...
  const add = (row, source) => {
    ...
    map.set(key, { ...current, ...tester, source: current.source && current.source !== source ? `${current.source}, ${source}` : tester.source });
  };
...
function TesterForm({ onSaved }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      await onSaved?.(result?.tester || { ...form, status: "pending_signup", source: "saved this session", invited_at: new Date().toISOString() });
