...
function dateText(value) { if (!value) return "Unavailable"; const parsed = new Date(value); return Number.isNaN(parsed.getTime()) ? text(value, "Unavailable") : parsed.toLocaleString("en-NZ"); }
...
function emailOf(item) { return low(item?.email || item?.canonical_email || item?.user_email || item?.owner_email || item?.tester_email || item?.target_email || item?.to); }
...
function planOf(item) { const value = low(item?.plan_name || item?.plan || item?.subscription_plan || item?.tier); return PLAN_LABELS[value] || (value ? value.charAt(0).toUpperCase() + value.slice(1) : "No plan"); }
function tone(value) { const v = low(value); if (/pass|ready|connected|verified|active|paid|online|clear|loaded|configured|accepted|trialing|sent|saved/.test(v)) return "good"; if (/fail|blocked|error|unavailable|past_due|unpaid|cancel|locked|revoked|expired/.test(v)) return "bad"; return "warn"; }
function logout() { try { localStorage.removeItem("token"); localStorage.removeItem("owner_portal_session"); localStorage.removeItem("platform_owner_email"); sessionStorage.clear(); } catch {} window.location.href = "/login"; }
...
function mergeTesterData({ testerEndpoint, control, billing, optimistic }) {
  ...
  const add = (row, source) => {
    ...
    map.set(key, { ...current, ...tester, display_email: tester.display_email || current.display_email, source: current.source && current.source !== source ? `${current.source}, ${source}` : tester.source });
  };
...
function Metric({ label, value, note, icon: Icon, state = "neutral" }) { return <article className={`hq2Metric ${state}`}><header><span>{label}</span>{Icon ? <Icon size={18} /> : null}</header><strong>{value}</strong><p>{note}</p></article>; }
function Empty({ title = "No records returned", detail = "The live endpoint returned no rows for this section." }) { return <div className="hq2Empty"><CheckCircle2 size={22} /><div><strong>{title}</strong><p>{detail}</p></div></div>; }
function EndpointStatus({ name, entry }) { const state = entry?.status || "waiting"; return <article className={`hq2Endpoint ${tone(state)}`}><div>{state === "loaded" ? <CheckCircle2 size={17} /> : state === "error" ? <XCircle size={17} /> : <RefreshCw size={17} />}</div><section><strong>{name}</strong><span>{entry?.path}</span><small>{state === "loaded" ? `Loaded ${dateText(entry.loadedAt)}` : entry?.error || "Waiting for live response"}</small></section></article>; }
function LaunchChecks({ checks }) { const rows = arr(checks); if (!rows.length) return <Empty title="Launch checks unavailable" detail="The paid-launch report did not return launch checks." />; return <div className="hq2CheckGrid">{rows.map((item) => <article key={item.key || item.label} className={tone(item.status)}><span>{item.status === "pass" ? <CheckCircle2 size={18} /> : item.status === "fail" ? <XCircle size={18} /> : <AlertTriangle size={18} />}</span><div><strong>{item.label}</strong><p>{item.detail}</p></div><em>{item.status}</em></article>)}</div>; }
...
function TesterForm({ onSaved }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const payload = { ...form, email: typedEmail, display_email: typedEmail, original_email: typedEmail };
      const result = await apiPost("/api/admin/owner/tester-intake", payload);
      setMessage(result.message || "Tester intake saved.");
      await onSaved?.(result?.tester || { ...payload, status: "pending_signup", source: "saved this session", invited_at: new Date().toISOString() });
