...
export default function PlansCommandPage() {
  ...
  return <main className="cv-launch-plans-page min-h-screen bg-[#f7f3ea] p-4 text-slate-950 md:p-6 xl:pl-[320px]" data-command-canvas><section className="mx-auto max-w-7xl space-y-5"><FirstSetupGuide mode="full" force />
    ...
    <section className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,.07)] md:p-6"><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-700">Pricing notes</div><h2 className="mt-2 text-3xl font-black tracking-[-.06em]">Operator is the main selling plan.</h2><div className="mt-4 flex flex-wrap gap-2">{QUICK_PRICING_NOTES.map((note) => <span key={note} className="rounded-full bg-slate-100 px-3 py-2 text-xs font-black text-slate-700">{note}</span>)}</div></section>
  </section></main>;
}
