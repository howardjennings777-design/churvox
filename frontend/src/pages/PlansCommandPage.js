...
import { CHURVOX_PLANS, COMMAND_GROWTH_PACK, XERO_ADDON, COUNTRY_OPTIONS, normaliseCountry, getCountryMeta, pricePlanForCountry, addonPriceForCountry, pricingNotesForCountry, getPlan, hasPlanAtLeast, nicePlanName } from "../config/churvoxPlans";
...
export default function PlansCommandPage() {
  ...
  const initialCountry = normaliseCountry(localStorage.getItem("churvox_billing_country") || user?.billing_country || user?.business_country || user?.country || "NZ");
  ...
  React.useEffect(() => {
    ...
    async function handleReturn() {
      ...
      const returnCountry = params.get("country") || "";
      if (returnCountry) setBillingCountry(returnCountry);
      ...
      const res = await post("/billing/confirm-addon-checkout", { addon, session_id: sessionId, country: returnCountry || country }, { timeout: 25000 });
      if (!alive) return;
      setBusy("");
      if (res?.success) {
        ...
        ["addon", "session_id", "addon_success", "country"].forEach((key) => clean.searchParams.delete(key));
    ...
    handleReturn();
    return () => { alive = false; };
  }, [location.search, post, refreshPlan, country]);
  ...
  const reviewedAccess = [["Current plan", currentPlanData?.name || nicePlanName(currentPlan) || "Not set"], ["Billing country", `${countryMeta.label} · ${countryMeta.currency}`], ["Trial", trialText(user || {})], ["App access", hasAppAccess ? "Allowed" : "Locked"]];
  ...
  return <main className="cv-launch-plans-page min-h-screen bg-[#f7f3ea] p-4 text-slate-950 md:p-6 xl:pl-[320px]" data-command-canvas><section className="mx-auto max-w-7xl space-y-5"><FirstSetupGuide mode="full" force />
    <DarkCard className="p-6 pl-9 md:p-8 md:pl-10"><div className="inline-flex rounded-full border border-amber-300/30 bg-amber-300/10 px-4 py-2 text-[10px] font-black uppercase tracking-[.22em] text-amber-300">Plans</div><h1 className="mt-5 max-w-4xl text-5xl font-black leading-[.9] tracking-[-.08em] text-white md:text-7xl">Churvox does the admin. You approve.</h1><p className="mt-5 max-w-3xl text-sm font-bold leading-7 text-slate-300 md:text-base">Pick your billing country first, then choose the plan. Checkout will use the matching country Stripe price when configured.</p><div className="mt-6 flex flex-wrap gap-3"><Link to="/dashboard" className="rounded-2xl bg-[linear-gradient(135deg,#facc15,#fb923c_55%,#22d3ee)] px-5 py-3 text-sm font-black text-slate-950 no-underline">Command Board</Link><button type="button" onClick={refreshPlan} className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-black text-white">Refresh plan status</button></div></DarkCard>
    <section className="rounded-[30px] border-2 border-orange-200 bg-[#fffaf0] p-5 shadow-[0_18px_50px_rgba(15,23,42,.10)] md:p-6"><div className="text-[10px] font-black uppercase tracking-[.2em] text-orange-700">Billing country</div><h2 className="mt-2 text-3xl font-black tracking-[-.06em] text-slate-950">Show prices for {countryMeta.label}</h2><div className="mt-4 grid gap-3 md:grid-cols-[320px_1fr]"><select value={country} onChange={(e) => setBillingCountry(e.target.value)} className="rounded-2xl border-2 border-orange-200 bg-white px-4 py-3 text-base font-black text-slate-950"><option value="NZ">New Zealand · NZD + GST</option><option value="AU">Australia · AUD + GST</option><option value="US">United States · USD tax applies</option><option value="UK">United Kingdom · GBP + VAT</option></select><p className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black leading-6 text-white">Checkout looks for country-specific Stripe prices first, then falls back to the base NZ/current price envs.</p></div></section>
