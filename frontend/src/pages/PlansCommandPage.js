...
export default function PlansCommandPage() {
  return (
    <main className="cv-launch-plans-page min-h-screen bg-[#f7f3ea] p-4 text-slate-950 md:p-6 xl:pl-[320px]">
      <section className="mx-auto max-w-7xl space-y-5">
        ...
        <section className="grid gap-5 xl:grid-cols-4">
          {plans.map((plan) => (
            <article key={plan.name} className={`cv-launch-plan-card ${plan.featured ? "cv-launch-plan-featured" : ""} relative overflow-hidden rounded-[30px] border p-5 pl-7 text-white`}>
              <Tape color={plan.featured ? "#fb923c" : "#22d3ee"} />
              ...
              <div className="cv-launch-plan-badge inline-flex rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[.16em]">{plan.badge}</div>
              <h2 className="mt-4 text-3xl font-black tracking-[-.06em]">{plan.name}</h2>
              <div className="mt-3 flex items-end gap-1">
                <span className="cv-launch-plan-price text-5xl font-black tracking-[-.08em]">{plan.price}</span>
                <span className="cv-launch-plan-muted pb-2 text-sm font-black">{plan.note}</span>
              </div>
              <p className="cv-launch-plan-muted mt-4 text-sm font-bold leading-6">{plan.summary}</p>
              <ul className="mt-5 grid gap-3">
                {plan.features.map((feature) => (
                  <li key={feature} className="cv-launch-plan-feature flex items-start gap-3 text-sm font-black leading-6">
                    <span className="mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-emerald-300 text-[10px] text-slate-950">✓</span>
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
              <Link to="/support" className="cv-launch-plan-cta mt-6 inline-flex w-full justify-center rounded-2xl px-5 py-4 text-sm font-black no-underline">Choose {plan.name}</Link>
            </article>
          ))}
        </section>
        ...
        <section className="cv-launch-clean-white-card rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,.07)] md:p-6">
