              ...
              <form onSubmit={saveRule} className="grid gap-4 lg:grid-cols-[1fr_1fr_220px] lg:items-end">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="submit"
                    disabled={saving}
                    className="rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-100"
                  >
                ...
                {rules.map((rule) => (
                  <article key={rule.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm ring-1 ring-slate-100">
                    ...
                    <div className="mt-3 grid grid-cols-4 gap-2">
                      ...
                      <button type="button" onClick={() => testRule(rule)} disabled={testingId === rule.id} className="rounded-xl bg-slate-900 px-2 py-2 text-xs font-semibold text-white transition hover:bg-slate-800 disabled:opacity-100">{testingId === rule.id ? "..." : "Test"}</button>
                      <button type="button" onClick={() => setConfirmDeleteId(rule.id)} disabled={deletingId === rule.id} className="rounded-xl bg-red-600 px-2 py-2 text-xs font-semibold text-white transition hover:bg-red-700 disabled:opacity-100">Del</button>
                    </div>
                ...
                {templateChoices.map((template) => (
                  ...
                  >
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      <button type="button" disabled={template.enabled || enablingTemplate === template.key} onClick={() => enableTemplate(template.key)} className="rounded-lg bg-blue-600 px-3 py-1 text-xs font-bold text-white disabled:opacity-100">{template.enabled ? "Enabled" : enablingTemplate === template.key ? "Enabling..." : "Enable"}</button>
                    </div>
                  </div>
                ))}
              </div>
