...
function fromJob(job, lane) {
  const id = rowId(job); const client = job.client_name || job.customer_name || "Client"; const title = job.title || job.job_name || client; const worker = job.assigned_worker_name || job.worker_name || "Unassigned"; const address = job.address || job.location || "No address saved";
...
function fromInvoice(invoice) {
  const id = rowId(invoice); const customer = invoice.customer_name || invoice.client_name || "Client"; const total = invoice.balance_due || invoice.balance || invoice.total || invoice.amount || 0;
...
const S = {
  page: { minHeight: "100vh", background: "radial-gradient(circle at 12% 8%, rgba(0,167,255,.20), transparent 30%), linear-gradient(140deg,#f5fbff,#eef5ff 48%,#dceeff)", color: "#142033", fontFamily: "Inter, system-ui, sans-serif" },
  ...
  logo: { display: "flex", textDecoration: "none" }, nav: { display: "flex", gap: 4, flex: 1 }, navA: { color: "rgba(255,255,255,.84)", textDecoration: "none", fontWeight: 850, fontSize: 13, padding: "10px 12px", borderRadius: 12 }, run: { border: 0, borderRadius: 14, background: "#fff", color: "#1d2d4a", padding: "12px 15px", fontWeight: 950, boxShadow: "0 14px 34px rgba(20,32,51,.14)" },
  main: { padding: "clamp(18px,3vw,34px) clamp(14px,3vw,44px) 56px" },
  hero: { display: "grid", gridTemplateColumns: "minmax(0,1fr) 280px", gap: 18, alignItems: "stretch" },
  heroCard: { background: "rgba(255,255,255,.94)", border: "1px solid #c9d8ef", borderRadius: 34, padding: "clamp(28px,4vw,48px)", boxShadow: "0 28px 90px rgba(35,58,102,.16)", backdropFilter: "blur(14px)" },
  ...
  lead: { color: "#61708a", fontSize: 18, lineHeight: 1.55, maxWidth: 880 },
  radar: { background: "linear-gradient(160deg,#1d2d4a,#2764ff)", borderRadius: 34, padding: 22, color: "#fff", boxShadow: "0 28px 90px rgba(39,100,255,.28)", display: "grid", alignContent: "center" },
  modeBar: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginTop: 18 },
  mode: { border: "1px solid #c9d8ef", background: "#fff", color: "#142033", borderRadius: 22, padding: 18, textAlign: "left", cursor: "pointer", boxShadow: "0 16px 42px rgba(35,58,102,.1)" },
  modeOn: { background: "linear-gradient(135deg,#2764ff,#00a7ff)", color: "#fff", borderColor: "#8ed7ff", boxShadow: "0 18px 48px rgba(39,100,255,.25)" },
  focus: { marginTop: 18, background: "rgba(255,255,255,.95)", border: "1px solid #c9d8ef", borderRadius: 36, padding: "clamp(24px,4vw,44px)", boxShadow: "0 30px 95px rgba(35,58,102,.17)" },
  h2: { margin: 0, fontFamily: "Outfit, Inter, sans-serif", fontSize: "clamp(38px,5.4vw,82px)", lineHeight: .82, letterSpacing: "-.08em", color: "#142033" },
  facts: { display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginTop: 22 },
  fact: { background: "#eef5ff", border: "1px solid #c9d8ef", borderRadius: 20, padding: 16 },
  open: { border: 0, borderRadius: 16, background: "linear-gradient(135deg,#2764ff,#00a7ff)", color: "#fff", padding: "14px 18px", fontWeight: 950, marginTop: 24, boxShadow: "0 18px 42px rgba(39,100,255,.28)" },
  conveyor: { marginTop: 18, background: "linear-gradient(135deg,#eaf3ff,#ffffff)", border: "1px solid #c9d8ef", borderRadius: 30, padding: 18, boxShadow: "0 22px 70px rgba(35,58,102,.12)" },
  rail: { display: "flex", gap: 12, overflowX: "auto", paddingBottom: 6 },
  card: { minWidth: 280, textAlign: "left", border: "1px solid #c9d8ef", background: "#fff", borderRadius: 22, padding: 18, cursor: "pointer" },
  cardOn: { borderColor: "#2764ff", boxShadow: "inset 0 -5px 0 #00a7ff, 0 16px 42px rgba(39,100,255,.14)" },
  pill: { display: "inline-flex", background: "#eef5ff", color: "#1d2d4a", borderRadius: 999, padding: "8px 10px", fontSize: 12, fontWeight: 950 },
};

function Stat({ label, value }) { return <div><span style={{ color: "rgba(255,255,255,.72)", textTransform: "uppercase", letterSpacing: ".12em", fontSize: 11, fontWeight: 900 }}>{label}</span><strong style={{ display: "block", fontFamily: "Outfit, Inter, sans-serif", fontSize: 48, lineHeight: .9 }}>{value}</strong></div>; }
...
export default function TechFlowDeskPage() {
  ...
  const load = useCallback(async () => { setLoading(true); const [snap, actionRes, jobRes, invoiceRes] = await Promise.all([get("/ai-operator/command-snapshot"), get("/ai-operator/actions"), get("/jobs"), get("/invoices")]); if (snap.success) setSnapshot(snap.data || null); if (actionRes.success) setActions(asArray(actionRes.data)); if (jobRes.success) setJobs(asArray(jobRes.data)); if (invoiceRes.success) setInvoices(asArray(invoiceRes.data)); setLoading(false); }, [get]);
  useEffect(() => { load(); }, [load]);
  ...
  const moneyLane = useMemo(() => { const readyJobs = jobs.filter((j) => ["completed", "done", "complete"].includes(String(j.status || "").toLowerCase()) && !(j.invoice_id || j.draft_invoice_id || j.invoiced)).slice(0, 8).map((j) => fromJob(j, "money")); const openInvoices = invoices.filter((i) => ["draft", "sent", "open", "overdue", "unpaid", "pending"].includes(String(i.status || "").toLowerCase())).slice(0, 8).map(fromInvoice); return [...readyJobs, ...openInvoices]; }, [jobs, invoices]);
  const lanes = { approve, fix, field, money: moneyLane }; const visible = lanes[lane] || []; const current = selected && selected.lane === lane ? selected : visible[0] || null;
