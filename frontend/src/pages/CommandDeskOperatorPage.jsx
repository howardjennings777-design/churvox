...
function makeActions({ jobs, invoices, quotes, workers }) {
  ...
  invoices.filter(isOverdue).slice(0, 6).forEach((invoice) => {
    actions.push({ id: `follow-invoice-${idOf(invoice)}`, box: "invoices", type: "invoice_follow_up", title: "Follow up invoice", summary: "Overdue invoice needs owner review.", ready: Boolean(idOf(invoice)), form: { invoice_id: idOf(invoice), client_name: clientOf(invoice), message: `Friendly reminder for ${clientOf(invoice)}.` } });
  });

  quotes.filter((quote) => statusOf(quote).includes("sent") && !quote.converted_job_id).slice(0, 6).forEach((quote) => {
    actions.push({ id: `follow-quote-${idOf(quote)}`, box: "quotes", type: "quote_follow_up", title: "Follow up quote", summary: "Sent quote is ready for a polite follow-up.", ready: Boolean(idOf(quote)), form: { quote_id: idOf(quote), client_name: clientOf(quote), message: `Hi ${clientOf(quote)}, just checking whether you had any questions about your quote.` } });
  });
...
function ActionCard({ action, onOpen }) {
  return <button type="button" onClick={() => onOpen(action)} className="rounded-[22px] border border-white/10 bg-white/[0.06] p-4 text-left text-white hover:bg-white/[0.09]"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black tracking-[-.04em] text-white">{action.title}</h3><Pill good={action.ready}>{action.ready ? "Ready" : "Needs details"}</Pill></div><p className="mt-2 text-sm font-bold leading-6 text-slate-300">{action.summary}</p><div className="mt-4 inline-flex rounded-2xl bg-amber-300 px-4 py-2 text-sm font-black text-slate-950">Review slip</div></button>;
}
...
function Slip({ action, onClose, onApprove }) {
  ...
  return <div className="fixed inset-0 z-[2147483647] overflow-y-auto bg-slate-950/90 p-4"><section className="mx-auto max-w-5xl rounded-[34px] bg-[#f7f3ea] shadow-2xl"><header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white p-6"><div><div className="inline-flex rounded-full bg-orange-100 px-3 py-1 text-[10px] font-black uppercase tracking-[.18em] text-orange-700">Approval slip</div><h1 className="mt-3 text-4xl font-black tracking-[-.07em] text-slate-950">{action.title}</h1><p className="mt-3 text-sm font-bold leading-6 text-slate-600">{action.summary}</p></div><button type="button" onClick={onClose} className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Close</button></header><main className="grid gap-5 p-5 md:grid-cols-[1fr_320px]"><section className="rounded-[28px] border border-slate-200 bg-white p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-blue-600">Review and edit</div><div className="mt-4 grid gap-3 md:grid-cols-2">{fields.map((key) => <label key={key} className="rounded-2xl border border-slate-200 bg-slate-50 p-3"><span className="text-[10px] font-black uppercase tracking-[.14em] text-slate-500">{key.replaceAll("_", " ")}</span><input value={form[key] || ""} onChange={(event) => setForm((prev) => ({ ...prev, [key]: event.target.value }))} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-950" /></label>)}</div></section><aside className="rounded-[28px] border border-slate-200 bg-white p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-orange-600">Owner action</div><h2 className="mt-2 text-3xl font-black tracking-[-.06em] text-slate-950">Approve when right.</h2><p className="mt-2 text-sm font-bold leading-6 text-slate-600">Churvox prepares. You approve.</p><div className="mt-5 grid gap-3"><button type="button" onClick={() => onApprove(action, form)} className="rounded-2xl bg-orange-400 px-4 py-3 text-sm font-black text-slate-950">Approve action</button><button type="button" onClick={onClose} className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white">Back</button></div></aside></main></section></div>;
}
...
export default function CommandDeskOperatorPage() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      const next = { jobs: jobsRes.status === "fulfilled" ? listFrom(jobsRes.value, ["jobs"]) : [], invoices: invoicesRes.status === "fulfilled" ? listFrom(invoicesRes.value, ["invoices"]) : [], quotes: quotesRes.status === "fulfilled" ? listFrom(quotesRes.value, ["quotes"]) : [], workers: workersRes.status === "fulfilled" ? listFrom(workersRes.value, ["workers", "team", "users"]) : [] };
      setData(next);
