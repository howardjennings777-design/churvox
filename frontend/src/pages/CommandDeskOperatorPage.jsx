...
function Slip({ action, onClose, onApprove, busy }) {
  ...
  return <div className="fixed inset-0 z-[2147483647] overflow-y-auto bg-slate-950/90 p-4"><section className="mx-auto max-w-5xl rounded-[34px] bg-[#f7f3ea] shadow-2xl"><header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white p-6"><div><div className="inline-flex rounded-full bg-orange-100 px-3 py-1 text-[10px] font-black uppercase tracking-[.18em] text-orange-700">AI approval slip</div><h1 className="mt-3 text-4xl font-black tracking-[-.07em] text-slate-950">{action.title}</h1><div className="mt-4 grid gap-2 text-sm font-bold leading-6 text-slate-600"><p><b className="text-slate-950">What I found:</b> {action.found}</p><p><b className="text-slate-950">What I prepared:</b> {action.prepared}</p><p><b className="text-slate-950">Approval will do:</b> {action.outcome}</p></div></div><button type="button" onClick={onClose} className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Close</button></header><main className="grid gap-5 p-5 md:grid-cols-[1fr_320px]"><section className="rounded-[28px] border border-slate-200 bg-white p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-blue-600">Review and edit</div><div className="mt-4 grid gap-3 md:grid-cols-2">{fields.map((key) => <label key={key} className="rounded-2xl border border-slate-200 bg-slate-50 p-3"><span className="text-[10px] font-black uppercase tracking-[.14em] text-slate-500">{key.replaceAll("_", " ")}</span><input value={form[key] || ""} onChange={(event) => setForm((prev) => ({ ...prev, [key]: event.target.value }))} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-950" /></label>)}</div><div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-black leading-6 text-amber-950">Approval-first: Churvox prepares the admin, but the owner stays in control.</div></section><aside className="rounded-[28px] border border-slate-200 bg-white p-5"><div className="text-[10px] font-black uppercase tracking-[.18em] text-orange-600">Owner action</div><h2 className="mt-2 text-3xl font-black tracking-[-.06em] text-slate-950">Approve when right.</h2><p className="mt-2 text-sm font-bold leading-6 text-slate-600">{action.outcome}</p>{!action.ready ? <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-black text-red-800">{blocker(action)}</div> : null}<div className="mt-5 grid gap-3"><button type="button" disabled={busy || !action.ready} onClick={() => onApprove(action, form)} className="rounded-2xl bg-orange-400 px-4 py-3 text-sm font-black text-slate-950 disabled:opacity-50">{busy ? "Applying…" : "Approve prepared action"}</button><Link to={href} onClick={onClose} className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-slate-950 ring-1 ring-slate-200">Open related page</Link><button type="button" onClick={onClose} className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white">Back</button></div></aside></main></section></div>;
}
