...
function statusStyle(status) {
  if (["ready", "approved", "complete"].includes(status)) return "border-emerald-200 bg-emerald-50 text-emerald-800";
  if (["needs_review", "pending", "unapproved"].includes(status)) return "border-amber-200 bg-amber-50 text-amber-800";
  ...
  return (
    <article className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)] transition hover:-translate-y-0.5 hover:shadow-[0_24px_70px_rgba(15,23,42,0.10)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <span className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{worker?.role || "worker"}</span>
          <h3 className="mt-1 text-lg font-black tracking-[-0.04em] text-slate-950">{nameOf(worker)}</h3>
        </div>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-sm font-bold text-slate-600">
        <div className="rounded-2xl bg-slate-50 p-3"><span className="block text-[10px] uppercase tracking-[0.14em] text-slate-400">Approved</span><b>{hours(approvedHours(worker))}h</b></div>
        <div className="rounded-2xl bg-slate-50 p-3"><span className="block text-[10px] uppercase tracking-[0.14em] text-slate-400">Pending</span><b>{hours(pendingHours(worker))}h</b></div>
      ...
      <div className="flex h-[100dvh] w-screen flex-col overflow-hidden bg-[#f5f7f1]">
        <header className="relative overflow-hidden border-b border-slate-800 bg-slate-950 p-6 text-white md:p-7">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <div className="inline-flex rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-amber-300">FULL SCREEN PAYROLL SLIP</div>
        ...
        <main className="min-h-0 flex-1 overflow-y-auto bg-[#f4f6f8] p-5 md:p-6">
          <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm font-bold leading-6 text-amber-950">Payroll is review/export only here. No tax, bank payout or compliance decision is made automatically.</div>
          </section>

          <section className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Approved hours</div><div className="mt-1 text-sm font-black text-slate-950">{hours(approvedHours(worker))}h</div></div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Pending hours</div><div className="mt-1 text-sm font-black text-slate-950">{hours(pendingHours(worker))}h</div></div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Rate</div><div className="mt-1 text-sm font-black text-slate-950">{payRate(worker) ? money(payRate(worker)) : "Not set"}</div></div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Gross estimate</div><div className="mt-1 text-sm font-black text-slate-950">{money(grossPay(worker))}</div></div>
          </section>
        ...
        <footer className="flex flex-wrap gap-3 border-t border-slate-200 bg-white p-5">
          <Link to="/reports" className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700">Open reports</Link>
      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1 p-4 pb-28 md:p-6 md:pb-28 xl:p-8 xl:pb-28">
          <header className="mb-5 flex flex-wrap items-center justify-between gap-4 rounded-[24px] border border-slate-200 bg-white px-5 py-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)]">
            <div><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Payroll Command</div><div className="text-sm font-bold text-slate-500">Review approved hours, pending time and payroll handoff summaries.</div></div>
            <div className="flex flex-wrap gap-3"><Link to="/team" className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-black text-slate-800 hover:bg-slate-50">Team</Link><Link to="/reports" className="rounded-2xl bg-amber-500 px-4 py-2 text-sm font-black text-slate-950 shadow-lg shadow-amber-500/20 hover:bg-amber-400">Reports</Link></div>
          </header>
          ...
          <section className="grid gap-5 xl:grid-cols-[1fr_430px]">
            <div className="overflow-hidden rounded-[30px] border border-slate-900 bg-slate-950 shadow-[0_26px_80px_rgba(15,23,42,0.20)]">
              <div className="relative p-6 md:p-8">
                ...
                <div className="relative">
                  <span className="inline-flex rounded-full border border-amber-400/30 bg-amber-400/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-amber-300">Payroll Command</span>
            ...
            <div className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_14px_38px_rgba(15,23,42,0.055)]">
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600">Payroll health</div>
              <h2 className="mt-2 text-2xl font-black tracking-[-0.055em] text-slate-950">What needs attention</h2>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="text-2xl font-black text-amber-800">{counts.review}</div><div className="text-xs font-black uppercase tracking-[0.14em] text-amber-700">Need review</div></div>
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4"><div className="text-2xl font-black text-blue-800">{hours(counts.approved)}h</div><div className="text-xs font-black uppercase tracking-[0.14em] text-blue-700">Approved hours</div></div>
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="text-2xl font-black text-emerald-800">{money(counts.gross)}</div><div className="text-xs font-black uppercase tracking-[0.14em] text-emerald-700">Gross estimate</div></div>
              </div>
            </div>
          </section>

          <section className="mt-5 grid gap-4 md:grid-cols-4">
            <div className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)]"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">People</div><div className="mt-3 text-3xl font-black tracking-[-0.06em]">{counts.people}</div></div>
            <div className="rounded-[22px] border border-blue-200 bg-blue-50 p-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)]"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-blue-700">Approved</div><div className="mt-3 text-3xl font-black tracking-[-0.06em] text-blue-900">{hours(counts.approved)}h</div></div>
            <div className="rounded-[22px] border border-amber-200 bg-amber-50 p-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)]"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700">Pending</div><div className="mt-3 text-3xl font-black tracking-[-0.06em] text-amber-900">{hours(counts.pending)}h</div></div>
            <div className="rounded-[22px] border border-emerald-200 bg-emerald-50 p-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)]"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-700">Gross</div><div className="mt-3 text-3xl font-black tracking-[-0.06em] text-emerald-900">{money(counts.gross)}</div></div>
          </section>

          <section className="mt-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_14px_38px_rgba(15,23,42,0.055)]">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-600">Payroll review list</div><h2 className="mt-2 text-3xl font-black tracking-[-0.06em] text-slate-950">Open payroll slips</h2></div>{loading && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">Loading…</span>}{error && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-700">Showing sample layout</span>}</div>
