...
function statusOf(worker) { if (pendingHours(worker) > 0) return "Needs review"; if (approvedHours(worker) > 0) return "Ready"; return "No hours"; }
function statusClass(worker) { if (pendingHours(worker) > 0) return "bg-amber-300 text-slate-950"; if (approvedHours(worker) > 0) return "bg-emerald-300 text-slate-950"; return "bg-slate-300 text-slate-950"; }
...
function exportCsv(rows) {
  const header = ["Name", "Email", "Role", "Approved Hours", "Pending Hours", "Pay Rate", "Gross Estimate"];
  const body = rows.map((worker) => [nameOf(worker), emailOf(worker), roleOf(worker), hours(approvedHours(worker)), hours(pendingHours(worker)), payRate(worker), grossPay(worker)]);
...
function PayrollSlip({ worker, approved, onClose, onApprove }) {
  ...
  return (
    <div className="fixed inset-0 z-[2147483600] overflow-y-auto bg-slate-950/92 p-3 text-white backdrop-blur-xl md:p-6" role="dialog" aria-modal="true">
      <div className="mx-auto flex min-h-[calc(100vh-24px)] max-w-6xl flex-col overflow-hidden rounded-[34px] border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 shadow-2xl md:min-h-[calc(100vh-48px)]">
        <header className="flex items-start justify-between gap-4 border-b border-white/10 p-5 md:p-7"><div><div className="inline-flex rounded-full bg-amber-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-amber-200">Payroll slip</div><h2 className="mt-3 text-4xl font-black leading-[0.95] tracking-[-0.07em] text-white md:text-6xl">{nameOf(worker)}</h2><p className="mt-4 max-w-3xl text-sm font-bold leading-6 text-slate-300 md:text-base">{roleOf(worker)} · {emailOf(worker)} · {statusOf(worker)}</p></div><button type="button" onClick={onClose} className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950">Close</button></header>
        <div className="grid flex-1 gap-5 p-5 md:grid-cols-[1.15fr_.85fr] md:p-7">
          ...
          <aside className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200">Payroll action</div><p className="mt-3 text-sm font-bold leading-6 text-slate-300">Review approved hours and hand off the summary. Churvox does not submit taxes, create bank payout files, or make compliance decisions.</p>{approved ? <div className="mt-4 rounded-3xl border border-emerald-300/25 bg-emerald-300/10 p-4 text-sm font-black text-emerald-100">Approved. This payroll slip is marked reviewed.</div> : null}<div className="mt-5 grid gap-3"><button type="button" onClick={onApprove} className="rounded-2xl bg-emerald-300 px-5 py-4 text-sm font-black text-slate-950">Approve slip</button><Link to="/reports" onClick={onClose} className="rounded-2xl bg-white px-5 py-4 text-center text-sm font-black text-slate-950 no-underline">Open reports</Link><button type="button" onClick={onClose} className="rounded-2xl bg-white/10 px-5 py-4 text-sm font-black text-white ring-1 ring-white/10">Back to payroll</button></div></aside>
        </div>
      </div>
    </div>
  );
...
function PayrollRow({ worker, onOpen }) {
  const tape = pendingHours(worker) > 0 ? "#facc15" : approvedHours(worker) > 0 ? "#34d399" : "#a78bfa";
  return <button type="button" onClick={() => onOpen(worker)} className="relative w-full overflow-hidden rounded-[26px] border border-white/10 bg-white/[0.06] p-4 pl-7 text-left text-white transition hover:border-cyan-300/40 hover:bg-white/[0.09] active:scale-[0.99]"><Tape color={tape} /><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-xl font-black tracking-[-0.05em] text-white">{nameOf(worker)}</h3><p className="mt-1 line-clamp-1 text-sm font-bold leading-6 text-slate-300">{hours(approvedHours(worker))}h approved · {hours(pendingHours(worker))}h pending · {money(grossPay(worker))}</p></div><span className={`shrink-0 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${statusClass(worker)}`}>{statusOf(worker)}</span></div></button>;
}
...
export default function PayrollCommandPage() {
  ...
  const review = workers.filter((worker) => pendingHours(worker) > 0).length;
  ...
  return (
    <main className={industrialPageShell} data-industrial-simple-page="payroll" data-command-canvas>
      <section className={`${industrialContentLane} space-y-5`}>
        <section className="relative overflow-hidden rounded-[30px] border border-white/10 p-5 pl-8 text-white md:p-7 md:pl-9" style={tileStyle}><Tape color="#a78bfa" /><span className={industrialChip}>Payroll</span><h1 className="mt-4 max-w-4xl text-4xl font-black leading-[0.92] tracking-[-0.075em] text-white md:text-6xl">Review hours. Approve summaries. Export the handoff.</h1><p className="mt-4 max-w-3xl text-sm font-semibold leading-6 text-slate-300 md:text-base">Payroll is locked to review, approved hours, summaries and CSV export. No tax submission, no bank payout files, no automatic compliance decisions.</p><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => exportCsv(workers)} className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialAction}`}>Export payroll CSV</button><Link to="/team" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Team</Link><Link to="/dashboard" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Command Board</Link></div></section>
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4"><Metric label="Need review" value={loading ? "…" : review} text="People with pending hours." color="#facc15" /><Metric label="Approved" value={`${hours(approved)}h`} text="Approved hours ready for handoff." color="#34d399" /><Metric label="Pending" value={`${hours(pending)}h`} text="Unapproved hours to check." color="#fb923c" /><Metric label="Gross estimate" value={money(gross)} text="Gross estimate only, before payroll processing." color="#a78bfa" /></section>
        <section className="rounded-[30px] border border-white/10 p-5 text-white md:p-6" style={tileStyle}><div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-300">Payroll review list</div><h2 className="mt-2 text-3xl font-black tracking-[-0.06em] text-white">Tap a person to review pay</h2></div><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-slate-300">{loading ? "Loading…" : `${workers.length} people`}</span></div>{workers.length ? <div className="grid gap-3">{workers.map((worker, index) => <PayrollRow key={idOf(worker) || `${nameOf(worker)}-${index}`} worker={worker} onOpen={setSelectedWorker} />)}</div> : <div className="rounded-[26px] border border-white/10 bg-white/[0.06] p-5"><h3 className="text-2xl font-black tracking-[-0.05em] text-white">No payroll people showing yet.</h3><p className="mt-2 text-sm font-bold leading-6 text-slate-300">Add workers or approved hours and payroll review slips will appear here. No sample payroll data is shown.</p></div>}</section>
      </section>
      <PayrollSlip worker={selectedWorker} approved={Boolean(approvedIds[selectedId])} onClose={() => setSelectedWorker(null)} onApprove={() => setApprovedIds((prev) => ({ ...prev, [selectedId]: true }))} />
    </main>
  );
}
