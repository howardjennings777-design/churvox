...
function isOverdue(invoice) {
  ...
  if (rawStatus(invoice).includes("overdue") || Number(invoice?.days_overdue || 0) > 0) return true;
...
function exportCsv(rows, filename = "churvox-report-summary") {
    ...
    <div className="fixed inset-0 z-[2147483600] overflow-y-auto bg-slate-950/92 p-3 text-white backdrop-blur-xl md:p-6" role="dialog" aria-modal="true">
      <div className="mx-auto flex min-h-[calc(100vh-24px)] max-w-6xl flex-col overflow-hidden rounded-[34px] border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 shadow-2xl md:min-h-[calc(100vh-48px)]">
        ...
        <div className="grid flex-1 gap-5 p-5 md:grid-cols-[1.15fr_.85fr] md:p-7">
          ...
          <aside className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200">Report action</div><p className="mt-3 text-sm font-bold leading-6 text-slate-300">Use this as a review and export view. It is not tax, legal or accounting advice, and it does not submit anything externally.</p>{reviewed ? <div className="mt-4 rounded-3xl border border-emerald-300/25 bg-emerald-300/10 p-4 text-sm font-black text-emerald-100">Reviewed. This report is ready for handoff/export.</div> : null}<div className="mt-5 grid gap-3"><button type="button" onClick={onReview} className="rounded-2xl bg-emerald-300 px-5 py-4 text-sm font-black text-slate-950">Mark report reviewed</button><button type="button" onClick={() => exportCsv(report.details.map(([a, b]) => [a, b, report.title]), `churvox-${report.id}-report`)} className="rounded-2xl bg-white px-5 py-4 text-sm font-black text-slate-950">Export this report</button><button type="button" onClick={onClose} className="rounded-2xl bg-white/10 px-5 py-4 text-sm font-black text-white ring-1 ring-white/10">Back to reports</button></div></aside>
        </div>
      </div>
    </div>
  );
}
function ReportRow({ report, reviewed, onOpen }) {
...
export default function ReportsCommandPage() {
  ...
  const [reviewed, setReviewed] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem("churvox_reports_reviewed") || "{}"); } catch { return {}; }
  });

  const loadReports = React.useCallback(async () => {
    setLoading(true);
    try {
      const [jobRes, invoiceRes, quoteRes, teamRes] = await Promise.allSettled([get("/jobs"), get("/invoices"), get("/quotes"), get("/team/workers")]);
      setJobs(jobRes.status === "fulfilled" ? listFrom(jobRes.value, ["jobs", "items", "results", "data"]) : []);
      setInvoices(invoiceRes.status === "fulfilled" ? listFrom(invoiceRes.value, ["invoices", "items", "results", "data"]) : []);
      setQuotes(quoteRes.status === "fulfilled" ? listFrom(quoteRes.value, ["quotes", "items", "results", "data"]) : []);
      setTeam(teamRes.status === "fulfilled" ? listFrom(teamRes.value, ["workers", "team", "users", "items", "results", "data"]) : []);
  ...
  function markReviewed(report) {
    const next = { ...reviewed, [report.id]: true };
    setReviewed(next);
    localStorage.setItem("churvox_reports_reviewed", JSON.stringify(next));
  ...
  return (
    <main className={industrialPageShell} data-industrial-simple-page="reports" data-command-canvas>
      <section className={`${industrialContentLane} space-y-5`}>
        <section className="relative overflow-hidden rounded-[30px] border border-white/10 p-5 pl-8 text-white md:p-7 md:pl-9" style={tileStyle}><Tape color="#22d3ee" /><span className={industrialChip}>Reports</span><h1 className="mt-4 max-w-4xl text-4xl font-black leading-[0.92] tracking-[-0.075em] text-white md:text-6xl">Simple business reports you can review and export.</h1><p className="mt-4 max-w-3xl text-sm font-semibold leading-6 text-slate-300 md:text-base">Reports show what happened, what needs attention, and what can be handed off. No fake charts, no tax decisions, no confusing dashboards.</p><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => exportCsv(reports.flatMap((report) => report.details.map(([a, b]) => [a, b, report.title])))} className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialAction}`}>Export reports CSV</button><button type="button" onClick={loadReports} className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Refresh reports</button><Link to="/payroll-board" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Payroll</Link><Link to="/dashboard" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Command Board</Link></div></section>
