...
const approvals = [
  {
    ...
    href: "/invoices/new",
    evidence: [
      "Worker marked the job complete",
      "Photos and notes are attached",
      "Invoice description is drafted",
      "Owner approval required before sending",
    ],
    primary: "Open invoice draft",
  },
  {
    ...
    evidence: [
      "Worker is available",
      "No obvious schedule conflict",
      "Area match looks good",
      "Assignment waits for owner approval",
    ],
  ...
  },
  {
    ...
    href: "/quotes",
    evidence: [
      "Quote has not been answered",
      "Follow-up message is prepared",
      "Tone is polite and direct",
  ...
  },
  {
    ...
    evidence: [
      "Invoice is overdue",
      "Reminder is prepared",
      "Customer wording can be edited",
      "Owner approval required before sending",
    ],
    primary: "Open money desk",
  },
];

const jobs = [
  ["8:30", "Lawn service", "In progress", "blue"],
  ["10:00", "Rental cleanup", "Assigned", "slate"],
  ["1:30", "Quote visit", "Needs worker", "amber"],
  ["3:00", "Hedge trim", "Ready", "green"],
];
...
function Approval({ item, onOpen }) {
  return (
    <article className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-[0_14px_38px_rgba(15,23,42,0.055)] transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-[0_24px_70px_rgba(15,23,42,0.10)]">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <button type="button" onClick={() => onOpen(item)} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-black text-slate-800 hover:bg-slate-50">View details</button>
        <Link to={item.href} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-black text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700">Open record</Link>
      </div>
    </article>
  );
}
      ...
      <div className="ml-auto flex h-full max-w-[680px] flex-col overflow-hidden rounded-[34px] border border-slate-200 bg-white shadow-[0_35px_120px_rgba(15,23,42,0.40)]">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto bg-[#f4f6f8] p-5 md:p-6">
          <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm font-bold leading-6 text-blue-950">Review the prepared action, check the evidence, then open the real record when it looks right.</div>
          </section>
          ...
          <section className="mt-4 rounded-[26px] border border-amber-200 bg-amber-50 p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm font-bold leading-6 text-amber-950">Open the matching record to edit or approve the real action. Nothing is sent automatically from this preview slip.</p>
          </section>
        </main>

        <footer className="flex flex-wrap gap-3 border-t border-slate-200 bg-white p-5">
          <Link to={active.href} className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700">{active.primary}</Link>
          <button type="button" onClick={onClose} className="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-black text-slate-800 hover:bg-slate-50">Keep pending</button>
        </footer>
      </div>
    </div>
  );
...
function StatusBadge({ state, tone }) {
  const colour = {
    amber: "bg-amber-50 text-amber-800 border-amber-200",
    blue: "bg-blue-50 text-blue-800 border-blue-200",
    green: "bg-emerald-50 text-emerald-800 border-emerald-200",
  ...
  return <span className={`rounded-full border px-3 py-1 text-xs font-black ${colour}`}>{state}</span>;
}
        ...
        <div className="overflow-hidden rounded-[30px] border border-slate-900 bg-slate-950 shadow-[0_26px_80px_rgba(15,23,42,0.20)]">
          <div className="relative p-6 md:p-8">
            ...
            <div className="relative">
              <span className="inline-flex rounded-full border border-amber-400/30 bg-amber-400/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-amber-300">Command Desk</span>
              ...
              <p className="mt-5 max-w-2xl text-sm font-semibold leading-6 text-slate-300 md:text-base">Your business command page should show what needs attention, what Churvox has prepared, and the next button to press.</p>
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link to="/jobs" className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-black text-white hover:bg-white/15">Open jobs</Link>
              </div>
            </div>
          </div>
        </div>
        ...
        <aside className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_14px_38px_rgba(15,23,42,0.055)]">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black tracking-[-0.055em] text-slate-950">Ready for review</h2>
            </div>
          ...
          <div className="mt-5 space-y-3">
            {approvals.map((item) => (
              <button key={item.title} type="button" onClick={() => setActiveSlip(item)} className="block w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left hover:border-blue-200 hover:bg-blue-50">
                ...
                <div className="mt-1 text-xs font-bold text-slate-500">Open slip, review evidence, then open the record.</div>
              </button>
            ))}
          </div>
        </aside>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.22fr_0.78fr]">
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_14px_38px_rgba(15,23,42,0.055)]">
          <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
            ...
            <span className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-600">Approval-first</span>
          </div>
            ...
            <div className="mt-5 space-y-3">
              {jobs.map(([time, name, state, tone]) => (
                <div key={`${time}-${name}`} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <StatusBadge state={state} tone={tone} />
                </div>
              ))}
            </div>
          </div>
