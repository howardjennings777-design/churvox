...
function routeFor(type, id) {
  if (!id) return type === "invoice" ? "/invoices" : type === "quote" ? "/quotes" : type === "team" ? "/team" : "/jobs";
  if (type === "invoice") return `/invoices/${id}`;
  if (type === "quote") return `/quotes/${id}`;
  if (type === "job") return `/jobs/${id}`;
  return type === "team" ? "/team" : "/dashboard";
}
...
function localActions(model) {
  const rows = [];
  if (model.unassignedJobs.length) rows.push({ id: "local-unassigned", title: `Assign ${model.unassignedJobs.length} open jobs`, description: "Open work needs a responsible worker.", priority: "high", confidence: "high", status: "local", route: "/jobs" });
  if (model.overdueInvoices.length) rows.push({ id: "local-overdue", title: `Chase ${model.overdueInvoices.length} overdue invoices`, description: "Overdue invoices are cash waiting.", priority: "high", confidence: "high", status: "local", route: "/invoices" });
  if (model.completedNoInvoice.length) rows.push({ id: "local-uninvoiced", title: `Invoice ${model.completedNoInvoice.length} completed jobs`, description: "Completed work should become draft invoices.", priority: "high", confidence: "medium", status: "local", route: "/jobs" });
  if (model.openQuotes.length) rows.push({ id: "local-quotes", title: `Follow up ${model.openQuotes.length} open quotes`, description: `${money(model.openQuotes.reduce((s, q) => s + amountOf(q), 0))} in quote value is waiting for action.`, priority: "medium", confidence: "high", status: "local", route: "/quotes" });
  if (model.workerIssues.length) rows.push({ id: "local-workers", title: `Fix ${model.workerIssues.length} worker setup issues`, description: "Missing role, region or rate weakens scheduling and payroll checks.", priority: "medium", confidence: "medium", status: "local", route: "/team" });
  return rows;
}

function localDrafts(model, type = "invoice_reminder") {
  if (type.includes("quote") && model.openQuotes[0]) {
    const q = model.openQuotes[0];
    return [{ id: "local-draft-quote", title: "Quote follow-up draft", type, draft_text: `Hi ${nameOf(q)}, just checking in on your quote. Happy to answer any questions or make any changes if needed. Thanks.` }];
  }
  if (type.includes("worker") && model.unassignedJobs[0]) {
    const j = model.unassignedJobs[0];
    return [{ id: "local-draft-worker", title: "Worker instruction draft", type, draft_text: `Please review this job for assignment: ${nameOf(j)}. Check the address, notes and timing before accepting.` }];
  }
  if ((type.includes("job") || type.includes("customer")) && model.stuckJobs[0]) {
    const j = model.stuckJobs[0];
    return [{ id: "local-draft-job", title: "Customer job update draft", type, draft_text: `Hi ${nameOf(j)}, quick update on your job. We are reviewing the schedule and will keep you updated before anything changes. Thanks.` }];
  }
...
function localIdeas(model) {
  const rows = [];
  if (model.openQuotes.length) rows.push({ id: "local-auto-quotes", title: "Quote follow-up automation", description: "When a quote is pending for a few days, create a follow-up task.", status: "local", priority: "medium" });
  if (model.overdueInvoices.length) rows.push({ id: "local-auto-invoices", title: "Invoice reminder automation", description: "When an invoice becomes overdue, draft a reminder for owner approval.", status: "local", priority: "high" });
  if (model.completedNoInvoice.length) rows.push({ id: "local-auto-jobs", title: "Completed job to invoice task", description: "When a job is completed, create a draft invoice task.", status: "local", priority: "high" });
  if (model.unassignedJobs.length) rows.push({ id: "local-auto-unassigned", title: "Unassigned job alert", description: "When a job has no worker, alert the owner or manager.", status: "local", priority: "high" });
  return rows;
}

function localBrief(model) {
  const risk = model.overdueInvoices.length || model.unassignedJobs.length || model.completedNoInvoice.length || model.stuckJobs.length ? "high" : model.openQuotes.length || model.unpaidInvoices.length ? "medium" : "low";
  return { headline: risk === "high" ? "High-priority work needs attention" : risk === "medium" ? "Some revenue and workflow items need review" : "No urgent issues found", summary: `${model.unassignedJobs.length} unassigned jobs, ${model.overdueInvoices.length} overdue invoices, ${model.completedNoInvoice.length} completed jobs not invoiced, and ${model.openQuotes.length} open quotes.` };
}
...
function localMemory(model) {
  ...
  if (model.completedNoInvoice.length >= 2) rows.push({ id: "local-memory-invoice", title: "Completed work is not becoming invoices", description: `${model.completedNoInvoice.length} completed jobs appear uninvoiced.`, evidence_count: model.completedNoInvoice.length });
  return rows;
}

function localProfit(model) {
  const unpaid = model.unpaidInvoices.reduce((s, i) => s + amountOf(i), 0);
  const quotes = model.openQuotes.reduce((s, q) => s + amountOf(q), 0);
  const uninvoiced = model.completedNoInvoice.reduce((s, j) => s + amountOf(j), 0);
...
export default function SafeAIAssistantPage() {
  ...
  const loadBase = async () => {
    ...
    const j = unwrap(calls[0]);
    const q = unwrap(calls[1]);
    const i = unwrap(calls[2]);
    const w = unwrap(calls[3]);
    setBase({ jobs: safeArray(j, "jobs"), quotes: safeArray(q, "quotes"), invoices: safeArray(i, "invoices"), workers: safeArray(w, "workers") });
  };
  ...
  const loadSaved = async () => {
    ...
    setProfit(localProfit(buildLocalModel(base)));
    setWarnings(ws);
  };

  const loadAll = async () => { await loadBase(); await loadSaved(); };
  useEffect(() => { loadAll(); }, []);

  const run = async (label, backendFn, fallbackFn) => {
    setBusy(label);
    const timer = setTimeout(() => { fallbackFn?.(); setBusy(""); }, 3500);
    try {
  ...
  const mutateAction = (id, op) => id.startsWith("local-") ? null : run(`action-${op}`, () => post(`/ai/actions/${id}/${op}`, {}));
  const mutateIdea = (id, op) => id.startsWith("local-") ? null : run(`idea-${op}`, () => post(`/ai/automation-suggestions/${id}/${op}`, {}));
  const mutateDraft = (id, op) => id.startsWith("local-") ? null : run(`draft-${op}`, () => op === "delete" ? del(`/ai/drafts/${id}`) : post(`/ai/drafts/${id}/${op}`, {}));

  return <Layout><div className="cx-page space-y-6">
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h1 className="text-3xl font-black">AI Business Assistant</h1><p className="text-sm text-slate-600">AI suggests. You approve. No emails, SMS, payroll, MYOB sync, pricing, status, or customer data changes happen automatically.</p>{busy && <p className="mt-2 text-xs font-bold text-blue-700">Working: {busy}...</p>}</section>
