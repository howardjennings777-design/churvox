      ...
      <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6" data-testid="sms-page">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Send reminders and messages to clients</p>
          </div>
          {isEmployer && (
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setShowBuy(true)} className="border-slate-200 text-slate-500 hover:text-slate-900" data-testid="buy-credits-button">
                <CreditCard size={14} className="mr-2" /> Buy Credits
              </Button>
              <Button onClick={openSendDialog} className="bg-blue-600 hover:bg-blue-700 text-white" data-testid="send-sms-button">
          ...
          <CardContent className="p-5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">SMS credits remaining</p>
              </div>
            </div>
            {lowCredit && (
        ...
        {/* Credit Packs */}
        <div>
          <h2 className="text-base font-semibold text-slate-900 mb-3">Credit Packs</h2>
          <div className="grid grid-cols-3 gap-3" data-testid="credit-packs">
            {PACKS.map((pack) => (
              <Card key={pack.id} className="bg-white border-slate-200 hover:border-blue-600/50 transition-all cursor-pointer" data-testid={`pack-${pack.id}`}>
                <CardContent className="p-4 text-center" onClick={() => handleBuy(pack.id)}>
                  ...
                  <p className="text-lg font-semibold text-blue-600 mt-2">{formatCurrency(pack.price)}</p>
                  <Button size="sm" className="mt-3 w-full bg-blue-600/20 text-blue-600 hover:bg-blue-600/30 text-xs" disabled={loading}>
                    Buy
                  </Button>
                </CardContent>
              </Card>
          ...
          <p className="text-[10px] text-slate-500 mt-2 text-center">Payment processing is a placeholder — no real charge.</p>
        </div>
          ...
          {history.length === 0 ? (
            <Card className="bg-white border-slate-200 shadow-sm">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-xs text-slate-500/60">Send customer reminders, on-the-way alerts, or invoice reminders. You can also send from job and invoice detail pages.</p>
              </CardContent>
            </Card>
          ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase bg-blue-600/20 text-blue-600">
                        ...
                        <p className="text-sm text-slate-900 truncate">{sms.message}</p>
                        <p className="text-xs text-slate-500 mt-1 flex items-center gap-2">
                          <Phone size={11} /> {sms.recipient_phone}
                          <Clock size={11} /> {formatDate(sms.created_at)}
                          {sms.sent_by_name && <><span className="text-blue-600/70">by {sms.sent_by_name}</span></>}
                        </p>
                      </div>
                    </div>
                  </CardContent>
            ...
            <div className="space-y-3">
              {PACKS.map((pack) => (
                <button key={pack.id} onClick={() => handleBuy(pack.id)} disabled={loading} data-testid={`buy-pack-${pack.id}`}
                  ...
                  <span className="text-lg font-bold text-blue-600">{formatCurrency(pack.price)}</span>
                </button>
              ))}
            </div>
            <p className="text-[10px] text-slate-500 text-center">Payment processing is a placeholder.</p>
          </DialogContent>
        </Dialog>
      </div>
    </Layout>
