...
export default function SmartHubBrainPage() {
  ...
  const renderDrawerContent = () => {
    ...
    if (workspaceDrawer === "Invoices") {
      return (
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-semibold text-white">Invoices Workspace</h3>
          ...
          {!readyToBillJobs.length ? (
            ...
            readyToBillJobs.map((job) => {
              ...
              return (
                <div key={String(job?.id || job?._id || job?.job_id || Math.random())} className="rounded-2xl border border-[#2a2f36] bg-[#111317] p-4 shadow-[0_14px_35px_rgba(15,17,21,0.20)] transition hover:border-[#d94f17] operator-panel operator-card" data-smart-hub-card="true">
                  ...
                  <button
                    ...
                    className="mt-4 rounded-lg bg-[#d94f17] px-4 py-2 text-sm font-medium text-white hover:bg-[#b93f10] disabled:cursor-not-allowed disabled:opacity-60"
                  >
    ...
    if (workspaceDrawer === "Payment Reminders") {
      ...
      return (
        <div className="space-y-4">
          ...
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[['Open invoices', reminderInvoices.length], ['Overdue invoices', overdueCount], ['Draft reminders', draftCount], ['Missing contact details', missingContactCount]].map(([label, value]) => <article key={label} className="rounded-xl border border-[#8c8274] tradie-panel p-3"><p className="text-xs uppercase tracking-wide text-white/55">{label}</p><p className="mt-1 text-xl font-semibold text-[#0f1115]">{value}</p></article>)}
          </section>
    ...
    if (workspaceDrawer === "Quotes" || workspaceDrawer === "Quote Follow-ups") {
      ...
      return (
        <div className="space-y-4">
          <div><h3 className="text-lg font-semibold text-white">Quote Follow-ups</h3><p className="text-sm text-[#5f646b]">Review AI-prepared quote follow-up drafts before anything is sent.</p></div>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Quotes waiting", waitingQuotes.length], ["Follow-ups prepared", preparedCount], ["Missing contact details", missingContactCount], ["Oldest waiting quote", oldestWaiting ? `${oldestWaiting}d` : "—"]].map(([label, value]) => <article key={label} className="rounded-xl border border-[#8c8274] tradie-panel p-3"><p className="text-xs uppercase tracking-wide text-white/55">{label}</p><p className="mt-1 text-xl font-semibold text-[#0f1115]">{value}</p></article>)}</section>
          ...
          {!waitingQuotes.length ? <p className="text-sm text-[#5f646b]">No quotes are waiting for follow-up right now.</p> : waitingQuotes.map((quote) => {
            ...
            return <article key={id} className="rounded-2xl border border-[#2a2f36] bg-[#111317] p-4 shadow-[0_14px_35px_rgba(15,17,21,0.20)] transition hover:border-[#d94f17] operator-panel operator-card" data-smart-hub-card="true"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedQuoteIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/60">{approvedQuoteIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#0f1115]">{textOr(client?.name || quote?.client_name || quote?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Quote: {textOr(quote?.quote_number || quote?.number || quote?.title, "Untitled quote")}</p><p className="text-sm text-[#5f646b]">Amount: {money(Number(quote?.total ?? quote?.amount ?? quote?.price))}</p><p className="text-sm text-[#5f646b]">Status: {textOr(quote?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Created/Sent: {textOr(displayDate, "Unknown date")}</p><p className="text-sm text-[#5f646b]">Age: {age ?? "—"} days</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-[#c8bfb1] px-2 py-1 text-xs text-[#111317]">Warning: missing client contact details. You can save this draft, but it is not ready to send.</p> : null}{editingQuoteDraft[id] ? <textarea className="mt-3 w-full rounded-lg border border-[#8c8274] p-2 text-sm" rows={4} value={quoteDrafts[id] || ""} onChange={(e) => setQuoteDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#c8bfb1] p-3 text-sm text-[#111317] operator-inner" data-smart-hub-inner="true">{quoteDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setQuoteDrafts((prev) => ({ ...prev, [id]: quoteDraftOriginals[id] || prev[id] })); setEditingQuoteDraft((prev) => ({ ...prev, [id]: false })); }} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(quote)} className="rounded bg-[#d94f17] px-3 py-1 text-sm text-white">Approve follow-up draft</button><button type="button" onClick={() => navigate(`/quotes/${id}`)} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Open full quote page</button></div></article>;
          })}
        </div>
      );
    }
    if (workspaceDrawer === "AI Dispatch") {
      ...
      return <div className="space-y-4"><div><h3 className="text-lg font-semibold text-white">AI Dispatch</h3><p className="text-sm text-[#5f646b]">Review recommended worker assignments before jobs are updated.</p></div>
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Unassigned jobs", dispatchRecs.length],["Crew available", crewAvailable],["Schedule conflicts", conflicts],["Missing job details", missingData]].map(([l,v])=><article key={l} className="rounded-xl border border-[#8c8274] tradie-panel p-3"><p className="text-xs uppercase tracking-wide text-white/55">{l}</p><p className="mt-1 text-xl font-semibold text-[#0f1115]">{v}</p></article>)}</section>
      {!dispatchRecs.length ? <p className="text-sm text-[#5f646b]">No unassigned jobs require approval right now.</p> : dispatchRecs.map(({ job, jobId, recommendation, selectedWorkerId }) => {
        ...
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={!selectedWorkerId || savingJobId===jobId} onClick={() => applyAssign(job, selectedWorkerId)} className="rounded bg-[#d94f17] px-3 py-1 text-sm text-white">Approve assignment</button><button type="button" onClick={() => navigate(`/jobs/${jobId}`)} className="rounded border px-3 py-1 text-sm">Open full job page</button><button type="button" onClick={async () => { setRejectedDispatchIds((prev) => ({ ...prev, [jobId]: true })); await logActivity({ action_type: "recommendation_rejected", title: "Recommendation rejected", message: `AI recommendation rejected: ${textOr(job?.title, "Job")}`, related_type: "job", related_id: jobId, status: "rejected" }); }} className="rounded border px-3 py-1 text-sm">Reject recommendation</button></div></article>;
      })}</div>;
    }

    return (
      <div className="space-y-3">
        <h3 className="text-lg font-semibold text-white">{workspaceDrawer} Workspace</h3>
        <p className="text-sm text-[#5f646b]">Nothing needs attention here.</p>
        <button
          type="button"
          onClick={() => navigate(`/${workspaceDrawer.toLowerCase()}`)}
          className="rounded-lg bg-[#d94f17] px-4 py-2 text-sm font-medium text-white hover:bg-[#b93f10]"
        >
          Open full page
        </button>
      </div>
  ...
  return (
    <Layout title="Smart Hub">
      <div className="min-h-screen bg-[#b3aa9d] text-[#0f1115] tradie-page-bg smart-hub-hard-trade-v4" style={{ background: "linear-gradient(180deg, #b3aa9d 0%, #aaa194 100%)" }}>
        <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" onClick={runScanNow} className="rounded-xl border border-[#ff8a3d]/40 bg-[#d94f17] px-4 py-2 text-sm font-semibold text-white shadow-lg hover:bg-[#b93f10]">Run today's AI plan</button>
          ...
          <div className="fixed inset-0 z-[80] overflow-hidden bg-[#0d0f12]/75 backdrop-blur-sm">
            <div className="mx-auto flex h-[100dvh] w-full max-w-6xl flex-col overflow-hidden bg-[#c8bfb1] sm:my-4 sm:h-[94vh] sm:rounded-2xl">
              <div className="flex-none border-b border-[#8c8274] bg-[#d7d0c4] px-5 py-4">
                ...
                <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{[["approvals","Approvals"],["dispatch","Dispatch"],["invoices","Invoices"],["quotes","Quotes"],["jobs_at_risk","Jobs at risk"],["crew_workload","Crew workload"],["ai_settings","AI Settings"],["activity","Activity"]].map(([k,l])=><button key={k} type="button" onClick={()=>setCommandCentreTab(k)} className={`shrink-0 rounded px-3 py-1 text-xs ${commandCentreTab===k?"bg-[#d94f17] text-white":"bg-[#c8bfb1] text-[#111317]"}`}>{l}</button>)}</div>
                {commandCentreTab==="approvals" ? (
                  ...
                  {filteredApprovalItems.map((item) => {
                    ...
                    <article key={item.id} className="rounded-xl border border-[#8c8274] bg-[#d7d0c4] p-4 shadow-[0_14px_35px_rgba(15,17,21,0.20)]">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        <button type="button" onClick={()=>setApprovalDetail(item)} className="rounded-xl bg-[#d94f17] px-3 py-2 text-sm font-semibold text-white hover:bg-[#b93f10]">Edit</button>
                        <button type="button" onClick={() => rejectApprovalItem(item)} className="rounded-xl bg-[#d94f17] px-3 py-2 text-sm font-semibold text-white hover:bg-[#b93f10]">Reject</button>
                        <button type="button" onClick={()=>setApprovalDetail(item)} className="rounded-xl bg-[#d94f17] px-3 py-2 text-sm font-semibold text-white hover:bg-[#b93f10]">Details</button>
                      </div>
                    </article>
                  );})}
