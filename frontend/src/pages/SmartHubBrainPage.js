...
import { Bell, Bot, LogOut, Settings, UserCircle2, X } from "lucide-react";
...
const workspaces = ["today", "dispatch", "jobs", "clients", "quotes", "invoices", "crew", "payroll", "automation", "reports", "communications", "approvals"];
...
export default function SmartHubBrainPage() {
  ...
  return <Layout smartHubMode><div className="min-h-screen bg-slate-50"><div className="mx-auto max-w-[1500px] space-y-4 p-4 sm:p-6"><section className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><ChurvoxLogo size="sm" /><div><p className="text-xs uppercase tracking-wide text-slate-500">Owner Workspace</p><h1 className="text-2xl font-bold text-slate-900">Smart Hub · {user?.business_name || "Churvox"}</h1></div></div><div className="hidden lg:block text-xs text-slate-500">Business pulse · {jobsToday.length} jobs today · {openInvoices.length} open invoices · {approvals.length} approvals</div><div className="flex items-center gap-2"><NotificationsBell /><button onClick={() => setModal("ask")} className="rounded-full bg-blue-600 text-white px-4 py-2 text-sm font-semibold">Ask AI</button><details className="relative"><summary className="list-none cursor-pointer rounded-full border px-3 py-2 text-sm font-semibold text-slate-700">{(user?.name || "Account").split(" ")[0]}</summary><div className="absolute right-0 mt-2 w-56 rounded-xl border bg-white p-2 shadow-xl z-20"><p className="px-2 py-1 text-xs text-slate-500">{user?.email}</p><Link to="/account" className="block rounded-lg px-2 py-2 text-sm hover:bg-slate-100">Account</Link><Link to="/settings" className="block rounded-lg px-2 py-2 text-sm hover:bg-slate-100">Settings</Link><button onClick={async () => { await logout(); navigate("/login"); }} className="mt-1 w-full text-left rounded-lg px-2 py-2 text-sm text-red-600 hover:bg-red-50">Log out</button></div></details></div></div></section>
    ...
    <section className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex gap-2 overflow-x-auto">{workspaces.map((k) => <button key={k} onClick={() => setActiveWorkspace(k)} className={`whitespace-nowrap rounded-full px-3 py-1.5 text-sm capitalize ${activeWorkspace === k ? "bg-blue-600 text-white" : "border text-slate-700"}`}>{k}</button>)}</div></section>
    ...
    {activeWorkspace !== "today" ? <section className="rounded-2xl border bg-white p-4 shadow-sm"><h3 className="text-lg font-semibold capitalize">{activeWorkspace}</h3><div className="mt-2 text-sm text-slate-700 space-y-2">
      {activeWorkspace === "dispatch" ? <><p>Unassigned jobs: {unassignedJobs.length}</p><p>AI worker recommendations: {derivedActions.filter((a) => a.kind === "assign_worker").length}</p><button onClick={() => setModal("dispatch")} className="rounded-full border px-3 py-1.5">Approve assignment buttons</button><button onClick={() => navigate("/dispatch")} className="ml-2 rounded-full border px-3 py-1.5">Open full dispatch page</button></> : null}
      {activeWorkspace === "jobs" ? <><p>Today jobs: {jobsToday.length} · Unassigned: {unassignedJobs.length}</p><p>Recently completed: {completedReadyToBill.length}</p><button onClick={() => setModal("job")} className="rounded-full border px-3 py-1.5">Create job</button><button onClick={() => navigate("/jobs")} className="ml-2 rounded-full border px-3 py-1.5">Open full jobs page</button></> : null}
      {activeWorkspace === "clients" ? <><p>Recent clients: {data.clients.length}</p><button onClick={() => setModal("client")} className="rounded-full border px-3 py-1.5">Add client</button><button onClick={() => navigate("/clients")} className="ml-2 rounded-full border px-3 py-1.5">Open full clients page</button></> : null}
      {activeWorkspace === "quotes" ? <><p>Quotes waiting: {waitingQuotes.length}</p><button onClick={() => setModal("quote")} className="rounded-full border px-3 py-1.5">Create quote</button><button onClick={() => navigate("/quotes")} className="ml-2 rounded-full border px-3 py-1.5">Open full quotes page</button></> : null}
      {activeWorkspace === "invoices" ? <><p>Ready to bill: {completedReadyToBill.length} · Open invoices: {openInvoices.length}</p><p>AI draft invoices: {derivedActions.filter((a) => a.kind === "create_invoice_draft").length}</p><button onClick={() => setModal("invoice")} className="rounded-full border px-3 py-1.5">Create invoice</button><button onClick={() => navigate("/invoices")} className="ml-2 rounded-full border px-3 py-1.5">Open full invoices page</button></> : null}
      {activeWorkspace === "crew" ? <><p>Workers available today: {workers.length - crewActive}</p><p>Workers with no jobs: {workers.filter((w) => calcWorkerLoadToday(data.jobs, asId(w), today)===0).length} · Overloaded workers: {workers.filter((w) => calcWorkerLoadToday(data.jobs, asId(w), today)>4).length}</p><button onClick={() => navigate("/team")} className="rounded-full border px-3 py-1.5">Open full team page</button></> : null}
      {activeWorkspace === "payroll" ? <><p>Approved timesheets proxy: active crew {crewActive}</p><button onClick={() => navigate("/payroll")} className="rounded-full border px-3 py-1.5">Open payroll page</button></> : null}
      {activeWorkspace === "automation" ? <><p>Automation prepared actions: {actionCards.length}</p><button onClick={() => navigate("/automation")} className="rounded-full border px-3 py-1.5">Open automation page</button></> : null}
      {activeWorkspace === "reports" ? <><p>Key numbers: Jobs {data.jobs.length}, Quotes {data.quotes.length}, Invoices {data.invoices.length}</p><button onClick={() => navigate("/reports")} className="rounded-full border px-3 py-1.5">Open reports page</button></> : null}
      {activeWorkspace === "communications" ? <><p>Prepared message drafts: {derivedActions.filter((a) => ["invoice_reminder","quote_follow_up"].includes(a.kind)).length}</p><button onClick={() => navigate("/communications")} className="rounded-full border px-3 py-1.5">Open communications page</button></> : null}
      {activeWorkspace === "approvals" ? <><p>Pending approvals: {approvals.length}</p><button onClick={() => setIsActionModalOpen(true)} className="rounded-full border px-3 py-1.5">Review queue</button></> : null}
    </div></section> : null}
  </div></div>
