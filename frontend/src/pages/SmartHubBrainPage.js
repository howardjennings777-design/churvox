...
export default function SmartHubBrainPage() {
  ...
  return <Layout smartHubMode><div className="min-h-screen bg-slate-50"><div className="mx-auto max-w-[1500px] space-y-4 p-4 sm:p-6"><section className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-wide text-slate-500">Smart Hub command centre</p><h1 className="text-2xl font-bold text-slate-900">{user?.business_name || "Churvox"} · Owner Workspace</h1></div><div className="flex items-center gap-2"><button onClick={() => setModal("ask")} className="rounded-full bg-blue-600 text-white px-4 py-2 text-sm font-semibold">Ask AI</button><NotificationsBell /><details className="relative"><summary className="list-none cursor-pointer rounded-full border px-3 py-2 text-sm font-semibold text-slate-700">{(user?.name || "Account").split(" ")[0]}</summary><div className="absolute right-0 mt-2 w-56 rounded-xl border bg-white p-2 shadow-xl z-20"><p className="px-2 py-1 text-xs text-slate-500">{user?.email}</p><Link to="/account" className="block rounded-lg px-2 py-2 text-sm hover:bg-slate-100">Account</Link><Link to="/settings" className="block rounded-lg px-2 py-2 text-sm hover:bg-slate-100">Settings</Link><button onClick={async () => { await logout(); navigate("/login"); }} className="mt-1 w-full text-left rounded-lg px-2 py-2 text-sm text-red-600 hover:bg-red-50">Log out</button></div></details></div></div></section>
