...
export default function SmartHubBrainPage() {
  ...
  const renderDrawerContent = () => {
    ...
    if (workspaceDrawer === "Jobs") {
      if (workspaceMode === "list") return <div className="space-y-3">{jobs.map((j) => <button key={String(j?.id || j?._id)} type="button" onClick={() => openDetail(j)} className="block w-full rounded border bg-[#ede8df] p-3 text-left"><p className="font-semibold">{safeText(j?.title || j?.name, "Untitled job")}</p><p className="text-sm text-[#5f646b]">{safeText(j?.status, "Unknown")} · {safeText(j?.address || j?.location, "No address")}</p></button>)}</div>;
        ...
        <div className="space-y-4">
          ...
          {!readyToBillJobs.length ? (
            ...
            readyToBillJobs.map((job) => {
              ...
              return (
                <div key={String(job?.id || job?._id || job?.job_id || Math.random())} className="rounded-2xl border border-[#9f978a] bg-[#ede8df] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]">
                  ...
                  <button
                    ...
                    className="mt-4 rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white hover:bg-[#166534] disabled:cursor-not-allowed disabled:opacity-60"
                  >
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => approveMany(selectedReminderIds)} className="rounded-xl bg-[#14532d] px-3 py-2 text-sm font-medium text-white hover:bg-[#166534]">Approve selected reminders</button>
            <button type="button" onClick={() => approveMany(reminderInvoices.map((inv) => String(inv?.id || inv?._id || inv?.invoice_id || "")).filter(Boolean))} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12] font-medium text-[#24272c]">Approve all ready reminders</button>
            <button type="button" onClick={() => setSelectedReminderIds([])} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12] font-medium text-[#24272c]">Reject selected</button>
          </div>
          {!reminderInvoices.length ? <p className="text-sm text-[#5f646b]">No unpaid invoices need reminders right now.</p> : reminderInvoices.map((inv) => {
            ...
            return <article key={id} className="rounded-2xl border border-[#9f978a] bg-[#ede8df] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedReminderIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#5f646b]">{approvedReminderIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#0f1115]">{textOr(client?.name || inv?.client_name || inv?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Invoice: {textOr(inv?.invoice_number || inv?.number || inv?.title, "Untitled invoice")}</p><p className="text-sm text-[#5f646b]">Amount due: {money(invoiceBalance(inv))}</p><p className="text-sm text-[#5f646b]">Due date: {textOr(due, "No due date")}</p><p className="text-sm text-[#5f646b]">Status: {textOr(inv?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Overdue days: {daysOverdue(inv) ?? "—"}</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Warning: missing client contact details. You can save/approve this draft, but it is not ready to send.</p> : null}{isEditing ? <textarea className="mt-3 w-full rounded-lg border border-[#c8c0b2] p-2 text-sm" rows={4} value={reminderDrafts[id] || ""} onChange={(e) => setReminderDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#d8d2c6] p-3 text-sm text-[#24272c]">{reminderDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setEditingDraft((prev) => ({ ...prev, [id]: false })); setReminderDrafts((prev) => ({ ...prev, [id]: buildInvoiceReminderMessage({ client, invoice: inv, business: user, channel: "email" }) })); }} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(inv)} className="rounded bg-[#0f6b57] px-3 py-1 text-sm text-white">Approve reminder draft</button><button type="button" onClick={() => navigate(`/invoices/${id}`)} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Open full invoice page</button></div></article>;
          })}
        </div>
      );
    }
    ...
    if (workspaceDrawer === "Quotes" || workspaceDrawer === "Quote Follow-ups") {
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => approveMany(selectedQuoteIds)} className="rounded-xl bg-[#14532d] px-3 py-2 text-sm font-medium text-white hover:bg-[#166534]">Approve selected follow-ups</button><button type="button" onClick={() => approveMany(waitingQuotes.map((q) => String(q?.id || q?._id || q?.quote_id || "")).filter(Boolean))} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12] font-medium text-[#24272c]">Approve all ready follow-ups</button><button type="button" onClick={() => setSelectedQuoteIds([])} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12] font-medium text-[#24272c]">Reject selected</button></div>
          {!waitingQuotes.length ? <p className="text-sm text-[#5f646b]">No quotes are waiting for follow-up right now.</p> : waitingQuotes.map((quote) => {
            ...
            return <article key={id} className="rounded-2xl border border-[#9f978a] bg-[#ede8df] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedQuoteIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#5f646b]">{approvedQuoteIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#0f1115]">{textOr(client?.name || quote?.client_name || quote?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Quote: {textOr(quote?.quote_number || quote?.number || quote?.title, "Untitled quote")}</p><p className="text-sm text-[#5f646b]">Amount: {money(Number(quote?.total ?? quote?.amount ?? quote?.price))}</p><p className="text-sm text-[#5f646b]">Status: {textOr(quote?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Created/Sent: {textOr(displayDate, "Unknown date")}</p><p className="text-sm text-[#5f646b]">Age: {age ?? "—"} days</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Warning: missing client contact details. You can save this draft, but it is not ready to send.</p> : null}{editingQuoteDraft[id] ? <textarea className="mt-3 w-full rounded-lg border border-[#c8c0b2] p-2 text-sm" rows={4} value={quoteDrafts[id] || ""} onChange={(e) => setQuoteDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#d8d2c6] p-3 text-sm text-[#24272c]">{quoteDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setQuoteDrafts((prev) => ({ ...prev, [id]: quoteDraftOriginals[id] || prev[id] })); setEditingQuoteDraft((prev) => ({ ...prev, [id]: false })); }} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(quote)} className="rounded bg-[#0f6b57] px-3 py-1 text-sm text-white">Approve follow-up draft</button><button type="button" onClick={() => navigate(`/quotes/${id}`)} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Open full quote page</button></div></article>;
          })}
        </div>
      );
    }
    ...
    return (
      <div className="space-y-3">
        ...
        <button
          ...
          className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white hover:bg-[#166534]"
        >
          Open full page
        </button>
      </div>
  ...
  return (
    <Layout title="Smart Hub">
      <div className="min-h-screen bg-[#d8d2c6] text-[#0f1115] tradie-page-bg" style={{ backgroundImage: "linear-gradient(135deg, rgba(15,17,21,0.04) 25%, transparent 25%), linear-gradient(225deg, rgba(15,17,21,0.04) 25%, transparent 25%)", backgroundColor: "#d8d2c6", backgroundSize: "28px 28px" }}>
        <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
          <section className="relative overflow-hidden rounded-3xl border border-white/15 bg-[linear-gradient(135deg,#0d0f12_0%,#15181d_45%,#2a2f36_100%)] p-6 text-white shadow-[0_24px_60px_rgba(0,0,0,0.35)]">
            <p className="text-xs uppercase tracking-[0.2em] text-[#fdba74]">AI Operator Control Centre</p>
            <span className="mt-2 inline-flex rounded-full border border-[#ea580c]/40 bg-[#f97316]/20 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#fdba74]">HARD TRADE THEME v3</span>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button type="button" onClick={() => { openApprovalCentre({ tab: "all" }); }} className="rounded-xl border border-white/15 bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/15">Open Command Queue</button>
              <button type="button" onClick={() => openApprovalCentre({ tab: approvalCounts.needs_decision ? "needs_decision" : "all" })} className={`rounded-full px-3 py-2 text-xs font-semibold ${approvalBadgeTone === "amber" ? "bg-[#f59e0b]/20 text-[#fdba74] border border-[#f59e0b]/40" : approvalBadgeTone === "green" ? "bg-[#bbf7d0] text-[#14532d] border border-[#86efac]" : "bg-white/10 text-white/80 border border-white/10"}`}>{approvalCounts.all ? `${approvalCounts.all} approvals` : "All clear"}</button>
              <div className="relative">
                ...
                {notificationOpen ? <div className="absolute right-0 z-20 mt-2 w-[340px] rounded-xl border border-[#c8c0b2] bg-[#ede8df] p-3 text-[#0f1115] shadow-xl">
                  <p className="text-xs font-semibold uppercase tracking-wide text-[#5f646b]">Notifications</p>
                  {notificationItems.slice(0, 10).map((item) => <button key={item.id} type="button" onClick={() => { setReadNotificationIds((prev) => ({ ...prev, [item.id]: true })); setNotificationOpen(false); item.action?.(); }} className="mt-2 w-full rounded-lg border border-slate-100 p-2 text-left hover:bg-[#d8d2c6]"><p className="text-[11px] font-semibold uppercase text-[#5f646b]">{item.section}</p><p className="text-sm font-medium">{item.title}</p><p className="text-xs text-[#5f646b]">{item.subtitle}</p><p className="text-[11px] text-slate-400">{item.time}</p></button>)}
                </div> : null}
              </div>
            </div>
          </section>
          ...
          <section className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-2xl border border-[#9f978a] bg-[#ede8df] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]">
                <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
          ...
          <section className="mt-6 rounded-3xl border border-[#7b7469] bg-[#ede8df] p-5 shadow-[0_14px_35px_rgba(15,17,21,0.16)] border-l-4 border-l-[#f97316]">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => openApprovalCentre({ tab: "all" })} className={`rounded-full px-3 py-1 text-xs font-medium ${approvalBadgeTone === "amber" ? "bg-[#f59e0b]/20 text-[#fdba74] border border-[#f59e0b]/40" : approvalBadgeTone === "green" ? "bg-[#bbf7d0] text-[#14532d] border border-[#86efac]" : "bg-white/10 text-white/80 border border-white/10"}`}>{approvalCounts.all ? `${approvalCounts.all} approvals` : "All clear"}</button>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => { openApprovalCentre({ tab: "all" }); }} className="rounded-xl bg-[#14532d] px-3 py-2 text-sm font-medium text-white hover:bg-[#166534]">Open Approval Centre</button>
              <button type="button" onClick={runScanNow} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12]">Run today's AI plan</button>
            </div>
          </section>

          <section className="mt-6 grid gap-4 lg:grid-cols-2">
            <article className="rounded-3xl border border-[#7b7469] bg-[#ede8df] p-5 shadow-[0_14px_35px_rgba(15,17,21,0.16)] border-l-4 border-l-[#f97316]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                {[["Jobs today", jobsToday], ["Unassigned jobs", unassignedJobs.length], ["Ready to bill", readyToBillJobs.length], ["Open invoices", openInvoices.length], ["Quotes waiting", waitingQuotes.length], ["Crew available", crewAvailable]].map(([label, value]) => (
                  <button
                    ...
                    className="rounded-lg bg-[#d8d2c6] border border-[#9f978a] px-3 py-2 text-left"
                  >
                    <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
              ...
              <p className="mt-4 rounded-lg bg-[#d8d2c6] px-3 py-2 text-sm text-[#24272c]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                {[["Money waiting", openInvoices.length], ["Billing ready", readyToBillJobs.length], ["Dispatch pressure", unassignedJobs.length], ["Pipeline", waitingQuotes.length], ["Crew", crewAvailable]].map(([label, value]) => (
                  <button
                    ...
                    className="rounded-xl border border-[#c8c0b2] bg-[#ede8df] p-3 text-left"
                  >
                    <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
                    <p className="mt-1 text-xl font-semibold text-[#0f1115]">{value}</p>
                  </button>
          ...
          <section className="mt-6 rounded-3xl border border-[#9f978a] bg-[#ede8df] p-4 shadow-sm">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {workspaceButtons.map((name) => (
                <button
                  ...
                  className="rounded-2xl border border-[#2a2f36] bg-[#1f242b] px-4 py-3 text-left text-sm font-medium text-white shadow transition hover:bg-[#0d0f12]"
                >
                  <span className="block">{name}</span>
                  <span className="block text-xs text-white/70">{workspaceMeta[name] || "Open workspace"}</span>
                </button>
              ))}
              <button type="button" onClick={() => openWorkspace("Payment Reminders", "reminders")} className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#166534]">Prepare reminders</button>
              <button type="button" onClick={() => openWorkspace("Quote Follow-ups", "followUps")} className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#166534]">Review follow-ups</button>
              <button type="button" onClick={() => openWorkspace("AI Dispatch", "assign")} className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#166534]">Assign workers</button>
            </div>
          </section>
          ...
          <section className="mt-4 rounded-3xl border border-[#9f978a] bg-[#ede8df] p-4 shadow-sm">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-[#5f646b]">Recent Smart Hub activity</h3>
            <div className="mt-2 flex gap-2">{[["all","All"],["completed","Completed"],["rejected","Rejected"],["draft_prepared","Drafts"]].map(([k,l]) => <button key={k} type="button" onClick={() => setActivityFilter(k)} className={`rounded px-2 py-1 text-xs ${activityFilter===k?"bg-[#20242a] text-white":"bg-[#d8d2c6] text-[#24272c]"}`}>{l}</button>)}</div>{!activity.length ? <p className="mt-2 text-sm text-[#5f646b]">No AI actions approved yet. Approved work will appear here.</p> : <ul className="mt-3 space-y-2 text-sm text-[#24272c]">{activity.filter((a)=>activityFilter==="all"?true:String(a?.status||"")===activityFilter).map((a) => <li key={String(a?.id||a?._id)} className="rounded-lg border border-[#c8c0b2] p-2"><p>{a?.message || a?.title}</p><p className="text-xs text-[#5f646b]">{textOr(a?.status, "completed")} · {a?.approved_by_name ? `${a.approved_by_name} · ` : ""}{new Date(a?.created_at || Date.now()).toLocaleString()}</p></li>)}</ul>}
          </section>
        </div>

        {workspaceDrawer ? (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#0d0f12]/75 backdrop-blur-sm p-0 sm:items-center sm:p-6">
            <div className="h-[86vh] w-full max-w-3xl rounded-t-2xl bg-[#ede8df] sm:h-auto sm:rounded-2xl">
        ...
        {aiSettingsOpen ? (
          <div className="fixed inset-0 z-[70] bg-[#171717]/70 p-4">
            <div className="mx-auto mt-10 max-w-xl rounded-2xl bg-[#ede8df] p-4">
        ...
        {approvalCentreOpen ? (
          <div className="fixed inset-0 z-[80] overflow-hidden bg-[#0d0f12]/75 backdrop-blur-sm">
            <div className="mx-auto flex h-[100dvh] w-full max-w-6xl flex-col overflow-hidden bg-[#d8d2c6] sm:my-4 sm:h-[94vh] sm:rounded-3xl">
              <div className="flex-none border-b border-[#c8c0b2] bg-[#ede8df] px-5 py-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button type="button" onClick={() => setApprovalCentreOpen(false)} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12]">Close</button>
                </div>
                <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{APPROVAL_GROUPS.map((g)=><button key={g} type="button" onClick={()=>setApprovalFilter(g)} className={`shrink-0 rounded px-3 py-1 text-xs ${approvalFilter===g?"bg-[#f97316] text-white":"bg-[#d8d2c6] text-[#24272c]"}`}>{g === "all" ? "All" : g.replace("_"," ")} ({approvalCounts[g] || 0})</button>)}</div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {filteredApprovalItems.map((item) => {
                    ...
                    <article key={item.id} className="rounded-xl border border-[#c8c0b2] bg-[#ede8df] p-4 shadow-sm">
                      ...
                      <p className="font-semibold text-[#0f1115]">{meta.title}</p><p className="mt-1 text-sm text-[#5f646b]">{meta.subtitle}</p><p className="mt-1 text-sm text-[#24272c]">{meta.reason}</p><p className="mt-1 text-xs text-[#5f646b]">{meta.dataUsed}</p><p className="mt-1 text-xs text-[#5f646b]">{meta.whatHappens}</p><p className="mt-1 text-xs text-[#5f646b]">Risk: {meta.risk}</p>{meta.contactSummary ? <p className="mt-1 text-xs text-[#5f646b]">{meta.contactSummary}</p> : null}
                      {item.type === "invoice_reminder" ? <><div className="mt-2 flex gap-1 text-[11px]"><span className="rounded bg-[#d8d2c6] px-2 py-0.5">Email</span><span className="rounded bg-[#d8d2c6] px-2 py-0.5">SMS</span></div><p className="mt-2 rounded bg-[#d8d2c6] p-2 text-xs text-[#24272c]">{reminderDrafts[item.relatedId] || buildInvoiceReminderMessage({ client: findByIds(clients, [item.invoice?.client_id, item.invoice?.clientId]), invoice: item.invoice, business: user, channel: "email" })}</p></> : null}
                      {item.type === "quote_follow_up" ? <><div className="mt-2 flex gap-1 text-[11px]"><span className="rounded bg-[#d8d2c6] px-2 py-0.5">Email</span><span className="rounded bg-[#d8d2c6] px-2 py-0.5">SMS</span></div><p className="mt-2 rounded bg-[#d8d2c6] p-2 text-xs text-[#24272c]">{quoteDrafts[item.relatedId] || buildQuoteFollowUpMessage({ client: findByIds(clients, [item.quote?.client_id, item.quote?.clientId]), quote: item.quote, business: user, channel: "email" })}</p></> : null}
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button type="button" onClick={() => approveApprovalItem(item)} className="rounded-xl bg-[#14532d] px-3 py-2 text-sm font-medium text-white hover:bg-[#166534]">Approve</button>
                        <button type="button" onClick={()=>setApprovalDetail(item)} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12]">Edit</button>
                        <button type="button" onClick={() => rejectApprovalItem(item)} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12]">Reject</button>
                        <button type="button" onClick={()=>setApprovalDetail(item)} className="rounded-xl bg-[#1f242b] px-3 py-2 text-sm text-white hover:bg-[#0d0f12]">Details</button>
                      </div>
                    </article>
                  );})}
                  {!filteredApprovalItems.length ? <article className="rounded-xl border border-[#c8c0b2] bg-[#ede8df] p-4 shadow-sm"><p className="font-semibold text-[#0f1115]">No actions in this section.</p><div className="mt-3 flex gap-2"><button type="button" onClick={runScanNow} className="rounded-lg bg-[#0f6b57] px-3 py-2 text-sm text-white">Run today's AI plan</button></div></article> : null}
                </div>
              </div>
            </div>
          </div>
