...
export default function SmartHubBrainPage() {
  ...
  const topDecisions = [...inboxGroups.needsDecision, ...inboxGroups.readyDrafts].slice(0, 3);
  return <Layout smartHubMode><div className="min-h-screen bg-[#eceae6] text-slate-900"><div className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6">
    <section className="rounded-3xl border border-slate-300/70 bg-[#f8f6f2] px-5 py-4 shadow-sm"><div className="grid items-center gap-3 lg:grid-cols-[1.1fr_1fr_1.1fr]"><div className="flex items-center gap-3"><ChurvoxLogo size="sm" /><div><h1 className="text-xl font-semibold">Smart Hub</h1><p className="text-xs uppercase tracking-[0.2em] text-slate-500">Owner Workspace</p></div></div><p className="text-center text-sm text-slate-600">{jobsToday.length} jobs today · {completedReadyToBill.length} ready to bill · {openInvoices.length} open invoices · {crewActive} crew active</p><div className="flex items-center justify-end gap-2"><NotificationsBell /><button onClick={() => setModal("ask")} className="rounded-full bg-[#4d5f8f] px-4 py-2 text-sm font-semibold text-white">Ask AI</button><details className="relative"><summary className="list-none cursor-pointer rounded-full border border-slate-300 bg-white px-3 py-2 text-sm">{(user?.name || "Owner").split(" ")[0]}</summary></details></div></div></section>
    <section className="rounded-[28px] border border-slate-700/50 bg-gradient-to-br from-[#1d2430] via-[#222b39] to-[#2d3550] p-6 text-slate-100 shadow-2xl"><div className="grid gap-6 lg:grid-cols-[2fr_1fr]"><div><h2 className="text-3xl font-semibold">AI Operator is running today’s business admin</h2><p className="mt-2 text-sm text-slate-300">Churvox checks jobs, crew, quotes and invoices, prepares the next actions, and waits for your approval.</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{[["Last scan","just now"],["Prepared actions",actionCards.length],["Needs approval",topDecisions.length],["Money waiting",`${openInvoices.length} invoices`],["Ready to bill",completedReadyToBill.length],["Crew available",Math.max(workers.length-crewActive,0)]].map(([k,v]) => <div key={k} className="rounded-2xl border border-white/10 bg-white/10 p-3"><p className="text-xs text-slate-300">{k}</p><p className="text-lg font-semibold">{v}</p></div>)}</div><div className="mt-4 flex flex-wrap gap-2"><button onClick={runDailyCheck} className="rounded-full bg-white px-4 py-2 text-sm text-slate-900">Run AI scan</button><button onClick={prepareToday} className="rounded-full border border-white/40 px-4 py-2 text-sm">Prepare today</button><button onClick={() => openWorkspace("approvals")} className="rounded-full border border-white/40 px-4 py-2 text-sm">Review approvals</button><button onClick={() => setModal("ask")} className="rounded-full border border-white/40 px-4 py-2 text-sm">Ask AI</button></div></div><div className="rounded-3xl border border-white/10 bg-black/20 p-5"><p className="text-xs uppercase tracking-[0.2em] text-slate-300">Best next move</p><p className="mt-2 text-2xl font-semibold">{topDecisions.length ? topDecisions[0].title : "All clear"}</p><p className="mt-2 text-sm text-slate-300">{topDecisions.length ? topDecisions[0].reason : "AI has no urgent approvals waiting."}</p><button onClick={() => topDecisions.length ? handleReviewAction(topDecisions[0]) : openWorkspace("approvals")} className="mt-4 rounded-full bg-[#7ea3a5] px-4 py-2 text-sm font-semibold text-slate-900">{topDecisions.length ? "Review now" : "Open approvals"}</button></div></div></section>
    <section className="grid gap-4 lg:grid-cols-[1.6fr_1fr]"><div className="rounded-3xl border border-slate-300 bg-[#f7f6f4] p-5"><h3 className="text-2xl font-semibold">Immediate decisions</h3><p className="text-sm text-slate-600">AI has prepared the next actions for you.</p><div className="mt-4 space-y-3">{topDecisions.length ? topDecisions.map((a, i) => <div key={a.id || i} className="rounded-2xl border border-slate-300 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-semibold">{a.title}</p><p className="text-sm text-slate-600">{a.reason}</p><p className="text-xs text-slate-500 mt-1">What AI prepared: {a.dataUsed}</p></div><span className="rounded-full bg-amber-100 px-2 py-1 text-xs text-amber-900">Risk: {a.risk || "medium"}</span></div><p className="mt-2 text-sm text-slate-700">If approved: this recommendation is applied and activity is logged.</p><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => handleApproveAction(a)} className="rounded-full bg-[#4d5f8f] px-4 py-2 text-xs font-semibold text-white">Approve</button><button onClick={() => handleEditAction(a)} className="rounded-full border px-3 py-2 text-xs">Edit</button><button onClick={() => handleRejectAction(a)} className="rounded-full border px-3 py-2 text-xs">Reject</button><button onClick={() => handleReviewAction(a)} className="rounded-full border px-3 py-2 text-xs">Details</button></div></div>) : <div className="rounded-2xl border border-slate-300 bg-white p-6 text-slate-600">All clear. AI has no urgent decisions waiting.</div>}</div></div>
    <div className="space-y-4"><div className="rounded-3xl border border-slate-300 bg-[#f8f7f5] p-5"><h4 className="text-xl font-semibold">Operational snapshot</h4><div className="mt-3 grid gap-2">{[["Ready to bill",completedReadyToBill.length],["Open invoices",openInvoices.length],["Quotes waiting",waitingQuotes.length],["Crew available",Math.max(workers.length-crewActive,0)],["Missing details",data.jobs.filter((j)=>!j.address||!j.client_id).length],["Schedule conflicts",Math.max(unassignedJobs.length-(workers.length-crewActive),0)]].map(([label,count])=><div key={label} className="rounded-xl bg-white p-3"><p className="text-sm text-slate-500">{label}</p><p className="text-xl font-semibold">{count}</p></div>)}</div></div></div></section>
  </div></div>
