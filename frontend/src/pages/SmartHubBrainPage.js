...
export default function SmartHubBrainPage() {
  ...
  return <Layout><div className="min-h-screen p-3 sm:p-5" style={{ background: "radial-gradient(circle at top left, rgba(21,94,239,0.10), transparent 34%), linear-gradient(135deg, rgba(21,94,239,0.045) 25%, transparent 25%), linear-gradient(225deg, rgba(21,94,239,0.035) 25%, transparent 25%), linear-gradient(90deg, rgba(15,23,42,0.035) 1px, transparent 1px), linear-gradient(180deg, rgba(15,23,42,0.030) 1px, transparent 1px), #F4F7FB", backgroundSize: "auto,36px 36px,36px 36px,42px 42px,42px 42px,auto", backgroundPosition: "center,0 0,0 0,0 0,0 0,center" }}><div className="mx-auto max-w-7xl space-y-4 relative z-10">
    <section className="bg-white/95 rounded-2xl border border-[#E2E8F0] p-4 sm:p-5 shadow-sm">
      <div className="flex flex-wrap justify-between gap-4 items-start">
        <div>
          <span className="text-xs font-semibold px-3 py-1 rounded-full bg-[#EAF1FF] text-[#155EEF] border border-[#BFD3FF]">AI Command Centre</span>
          <h1 className="mt-2 text-2xl font-semibold text-[#0F172A]">Welcome back, {user?.name || "Random az"}</h1>
          <p className="text-sm text-[#64748B]">{new Date().toLocaleDateString()} • Status: Online and synced</p>
          <p className="text-sm text-[#334155] mt-1">Today: {stats.jobsToday} jobs, {stats.unassigned} unassigned, {stats.openInvoices} invoices open, {stats.crewActive} crew active.</p>
        </div>
    ...
    {canSeeOwnerControls ? <section className="bg-white rounded-2xl border border-[#BFD3FF] p-4 sm:p-5 shadow-sm">
      <div className="flex flex-wrap justify-between gap-3">
        <div className="space-y-2">
          ...
          <p className="text-sm text-[#334155]">I prepare the admin. You approve what happens next.</p>
          <div className="flex items-center gap-2 text-xs"><span className="rounded-full bg-[#DCFCE7] text-[#16A34A] px-2.5 py-1 font-medium">Ready</span><span className="text-[#64748B]">Pending approvals: {data.approvals.length}</span></div>
        </div>
    ...
    <section className="bg-white rounded-2xl border border-[#E2E8F0] p-4 shadow-sm"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-[#6D5DF6]" /><h3 className="font-semibold text-[#0F172A]">Today’s AI Plan</h3></div><p className="text-xs text-[#64748B] mt-1">Here are the first things to do today.</p><div className="mt-3 grid gap-2">{aiPlan.length ? aiPlan.map((item) => <div key={item.label} className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3 flex items-center justify-between gap-3"><div className="flex items-start gap-3"><span className="rounded-lg p-2 bg-[#EAF1FF]"><item.icon className="h-4 w-4 text-[#155EEF]" /></span><div><p className="text-sm font-medium text-[#0F172A]">{item.label}</p><p className="text-xs text-[#64748B]">{item.reason}</p></div></div><div className="flex items-center gap-2"><span className="text-[10px] rounded-full bg-[#F1F5F9] px-2 py-1 text-[#475569]">{item.chip}</span><button className="rounded-full bg-[#155EEF] hover:bg-[#0F46C8] text-white px-3 py-1.5 text-xs" onClick={() => setModal(item.action)}>{item.button}</button></div></div>) : <p className="text-sm text-[#64748B]">No priority actions right now.</p>}</div></section>

    <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">{metricCards.map(([label, count, hint, action, Icon]) => <button key={label} onClick={action} className="bg-white rounded-xl border border-[#E2E8F0] p-3.5 shadow-sm text-left hover:-translate-y-0.5 transition"><div className="flex justify-between items-start"><p className="text-2xl font-semibold text-[#0F172A]">{count}</p><span className="h-8 w-8 rounded-full bg-[#EAF1FF] inline-flex items-center justify-center"><Icon className="h-4 w-4 text-[#155EEF]" /></span></div><p className="text-sm font-medium mt-2 text-[#334155]">{label}</p><p className="text-xs text-[#64748B] mt-0.5">{hint}</p></button>)}</section>
