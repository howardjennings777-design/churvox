...
export default function SmartHubBrainPage() {
        ...
        <div className="space-y-4">
          ...
          {!readyToBillJobs.length ? (
            ...
            readyToBillJobs.map((job) => {
              ...
              return (
                <div key={String(job?.id || job?._id || job?.job_id || Math.random())} className="rounded-2xl border border-[#7f7668] bg-[#d7d0c4] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]">
      ...
      return (
        <div className="space-y-4">
          ...
          {!reminderInvoices.length ? <p className="text-sm text-[#5f646b]">No unpaid invoices need reminders right now.</p> : reminderInvoices.map((inv) => {
            ...
            return <article key={id} className="rounded-2xl border border-[#7f7668] bg-[#d7d0c4] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedReminderIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#5f646b]">{approvedReminderIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#0f1115]">{textOr(client?.name || inv?.client_name || inv?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Invoice: {textOr(inv?.invoice_number || inv?.number || inv?.title, "Untitled invoice")}</p><p className="text-sm text-[#5f646b]">Amount due: {money(invoiceBalance(inv))}</p><p className="text-sm text-[#5f646b]">Due date: {textOr(due, "No due date")}</p><p className="text-sm text-[#5f646b]">Status: {textOr(inv?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Overdue days: {daysOverdue(inv) ?? "—"}</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-[#c8bfb1] px-2 py-1 text-xs text-[#111317]">Warning: missing client contact details. You can save/approve this draft, but it is not ready to send.</p> : null}{isEditing ? <textarea className="mt-3 w-full rounded-lg border border-[#8c8274] p-2 text-sm" rows={4} value={reminderDrafts[id] || ""} onChange={(e) => setReminderDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#c8bfb1] p-3 text-sm text-[#111317]">{reminderDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setEditingDraft((prev) => ({ ...prev, [id]: false })); setReminderDrafts((prev) => ({ ...prev, [id]: buildInvoiceReminderMessage({ client, invoice: inv, business: user, channel: "email" }) })); }} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(inv)} className="rounded bg-[#f97316] px-3 py-1 text-sm text-white">Approve reminder draft</button><button type="button" onClick={() => navigate(`/invoices/${id}`)} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Open full invoice page</button></div></article>;
          })}
        </div>
      );
    }
      ...
      return (
        <div className="space-y-4">
          ...
          {!waitingQuotes.length ? <p className="text-sm text-[#5f646b]">No quotes are waiting for follow-up right now.</p> : waitingQuotes.map((quote) => {
            ...
            return <article key={id} className="rounded-2xl border border-[#7f7668] bg-[#d7d0c4] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedQuoteIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#5f646b]">{approvedQuoteIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#0f1115]">{textOr(client?.name || quote?.client_name || quote?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Quote: {textOr(quote?.quote_number || quote?.number || quote?.title, "Untitled quote")}</p><p className="text-sm text-[#5f646b]">Amount: {money(Number(quote?.total ?? quote?.amount ?? quote?.price))}</p><p className="text-sm text-[#5f646b]">Status: {textOr(quote?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Created/Sent: {textOr(displayDate, "Unknown date")}</p><p className="text-sm text-[#5f646b]">Age: {age ?? "—"} days</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-[#c8bfb1] px-2 py-1 text-xs text-[#111317]">Warning: missing client contact details. You can save this draft, but it is not ready to send.</p> : null}{editingQuoteDraft[id] ? <textarea className="mt-3 w-full rounded-lg border border-[#8c8274] p-2 text-sm" rows={4} value={quoteDrafts[id] || ""} onChange={(e) => setQuoteDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#c8bfb1] p-3 text-sm text-[#111317]">{quoteDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setQuoteDrafts((prev) => ({ ...prev, [id]: quoteDraftOriginals[id] || prev[id] })); setEditingQuoteDraft((prev) => ({ ...prev, [id]: false })); }} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(quote)} className="rounded bg-[#f97316] px-3 py-1 text-sm text-white">Approve follow-up draft</button><button type="button" onClick={() => navigate(`/quotes/${id}`)} className="rounded border border-[#8c8274] px-3 py-1 text-sm">Open full quote page</button></div></article>;
          })}
        </div>
      );
    }
  ...
  return (
    <Layout title="Smart Hub">
      <div className="min-h-screen bg-[#b8afa1] text-[#0f1115] tradie-page-bg" style={{ backgroundImage: "linear-gradient(135deg, rgba(15,17,21,0.07) 25%, transparent 25%), linear-gradient(225deg, rgba(15,17,21,0.06) 25%, transparent 25%)", backgroundColor: "#b8afa1", backgroundSize: "30px 30px" }}>
        <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
          ...
          <section className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-2xl border border-[#7f7668] bg-[#d7d0c4] p-4 shadow-[0_8px_20px_rgba(15,17,21,0.10)]">
                <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
          ...
          <section className="mt-6 grid gap-4 lg:grid-cols-2">
            <article className="rounded-3xl border border-[#7b7469] bg-[#d7d0c4] p-5 shadow-[0_14px_35px_rgba(15,17,21,0.16)] border-l-4 border-l-[#f97316]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                {[["Jobs today", jobsToday], ["Unassigned jobs", unassignedJobs.length], ["Ready to bill", readyToBillJobs.length], ["Open invoices", openInvoices.length], ["Quotes waiting", waitingQuotes.length], ["Crew available", crewAvailable]].map(([label, value]) => (
                  <button
                    ...
                    className="rounded-lg bg-[#c8bfb1] border border-[#7f7668] px-3 py-2 text-left"
                  >
                    <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
          ...
          <section className="mt-6 rounded-3xl border border-[#7f7668] bg-[#d7d0c4] p-4 shadow-[0_12px_30px_rgba(15,17,21,0.18)]">
