...
export default function SmartHubBrainPage() {
  ...
  const renderWorkspace = () => {
    ...
    if (activeWorkspace === "approvals") return pendingApprovals.length ? <div className="space-y-3">{pendingApprovals.slice(0, 5).map((a, idx) => <div key={a.id || idx} className="rounded-2xl border border-[#CFDBEE] bg-white p-4"><button onClick={() => setSelectedContext(a)} className="font-semibold text-[#0F172A] text-left">{a.title || a.action_type || 'Approval item'}</button><p className="text-sm text-[#475569] mt-1">{a.reason || 'Prepared by AI Brain for owner review.'}</p></div>)}</div> : <div className="rounded-2xl border border-dashed border-[#D0DDF1] bg-[#FCFDFF] p-4 text-sm text-[#475569]">No approvals pending. Run brain scan to prepare actions.</div>;
