...
export default function SmartHubBrainPage() {
  ...
  const bestNextMove = unassignedJobs.length > 0
    ? `Approve ${unassignedJobs.length} worker assignment${unassignedJobs.length === 1 ? "" : "s"}`
    : completedReadyToBill.length > 0
      ? `Approve ${completedReadyToBill.length} invoice draft${completedReadyToBill.length === 1 ? "" : "s"}`
      : openInvoices.length > 0
        ? `Prepare reminders for ${openInvoices.length} open invoice${openInvoices.length === 1 ? "" : "s"}`
        : waitingQuotes.length > 0
          ? `Approve ${waitingQuotes.length} quote follow-up${waitingQuotes.length === 1 ? "" : "s"}`
