...
const safeText = (value, fallback = "Not saved") => String(value || "").trim() || fallback;
...
export default function SmartHubBrainPage() {
  ...
  const workers = useMemo(() => [...new Map(safeArray(data.workers).map((w) => [String(w.id || w._id || w.email || w.name), w])).values()], [data.workers]);
  const jobsToday = useMemo(() => safeArray(data.jobs).filter((j) => String(j.scheduled_date || j.date || "").slice(0, 10) === today), [data.jobs, today]);
  ...
  const unassignedJobs = useMemo(() => safeArray(data.jobs).filter((j) => !isAssignedJob(j)), [data.jobs]);
  const completedReadyToBill = useMemo(() => safeArray(data.jobs).filter((j) => ["completed","complete"].includes(normalizeStatus(j?.status)) && !hasDraftOrInvoice(j)), [data.jobs, data.invoices, locallyBilledJobIds]);
  const waitingQuotes = useMemo(() => safeArray(data.quotes).filter((q) => ["sent", "pending"].includes(String(q?.status || "").toLowerCase())), [data.quotes]);
  const openInvoices = useMemo(() => safeArray(data.invoices).filter((i) => ["open", "overdue", "sent"].includes(String(i?.status || "").toLowerCase())), [data.invoices]);
  ...
  const derivedActions = useMemo(() => {
    ...
    if (completedReadyToBill.length) list.push({ id: `draft-invoice-${getAnyId(completedReadyToBill[0])}`, priority: "ready", kind: "create_invoice_draft", action_type: "create_invoice_draft", job_id: getAnyId(completedReadyToBill[0]), title: `Create ${completedReadyToBill.length} draft invoice${completedReadyToBill.length > 1 ? "s" : ""}`, reason: `${completedReadyToBill.length} completed jobs are ready for billing.`, dataUsed: `${completedReadyToBill.length} completed jobs ready to bill`, risk: "medium", primary: "Approve draft", nav: "/invoices" });
    if (waitingQuotes.length) list.push({ id: `quote-followup-${getAnyId(waitingQuotes[0])}`, priority: "draft", kind: "quote_follow_up", action_type: "quote_follow_up", quote_id: getAnyId(waitingQuotes[0]), title: `Follow up ${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""}`, reason: "Quotes are waiting on customer response.", dataUsed: `${waitingQuotes.length} quote waiting for client response`, risk: "low", primary: "Approve draft", nav: "/quotes" });
    if (openInvoices.length) list.push({ id: `invoice-reminder-${getAnyId(openInvoices[0])}`, priority: "draft", kind: "invoice_reminder", action_type: "invoice_reminder", invoice_id: getAnyId(openInvoices[0]), title: `Prepare reminders for ${openInvoices.length} open invoice${openInvoices.length > 1 ? "s" : ""}`, reason: "Unpaid invoices are ready for reminder drafts.", dataUsed: `${openInvoices.length} open invoices waiting for payment`, risk: overdueInvoices.length ? "high" : "medium", primary: "Approve draft", nav: "/invoices" });
  ...
  const openSmartHubAction = (action, mode = "list") => {
  ...
  const invoiceDraftRows = useMemo(() => safeArray(completedReadyToBill).map((job) => {
    ...
    const description = buildSmartInvoiceDescription(job, client);
  ...
  }), [completedReadyToBill, clientsById, buildSmartInvoiceDescription]);
  useEffect(() => {
    const next = {};
    const selected = {};
  ...
  const openReviewJobEdit = (row) => {
    ...
    setReviewEditForm({
      ...
      invoice_description_draft: draftDescriptions[row.jobId] || row.description || buildSmartInvoiceDescription(row.job, { name: row.clientName }) || "",
  ...
  const handleApproveReadyToBillDraft = async (job) => {
    await approveDraftInvoice(job);
  ...
  const handleSaveReadyToBillDescription = async (job, description) => {
    ...
    await post(`/jobs/${job.jobId}`, { ai_invoice_description: value, invoice_description_draft: value });
    ...
    : completedReadyToBill.length > 0
      ...
      : openInvoices.length > 0
        ...
        : waitingQuotes.length > 0
          ...
          : "All clear";
  const decisionCards = [
    completedReadyToBill.length > 0 ? { key: "invoice_drafts", type: "create_invoice_draft", title: `Create ${completedReadyToBill.length} draft invoice${completedReadyToBill.length > 1 ? "s" : ""}`, reason: `${completedReadyToBill.length} completed jobs are ready for billing.`, whatHappens: "Churvox will create editable draft invoices. Nothing is sent until you approve sending.", dataUsed: completedReadyToBill.length > 3 ? `${completedReadyToBill.length} completed jobs ready to bill` : `Jobs ready: ${completedReadyToBill.slice(0, 3).map((j) => j.title || j.name || "Job").join(", ")}`, primary: "Approve drafts", action: () => openSmartHubAction({ id: `draft-invoice-${getAnyId(completedReadyToBill[0])}`, action_type: "create_invoice_draft", kind: "create_invoice_draft", job_id: getAnyId(completedReadyToBill[0]), title: `Create ${completedReadyToBill.length} draft invoice${completedReadyToBill.length > 1 ? "s" : ""}`, reason: `${completedReadyToBill.length} completed jobs are ready for billing.`, dataUsed: `${completedReadyToBill.length} completed jobs ready to bill`, risk: "medium", status: "pending" }), detailAction: () => openSmartHubAction({ id: `draft-invoice-${getAnyId(completedReadyToBill[0])}`, action_type: "create_invoice_draft", kind: "create_invoice_draft", job_id: getAnyId(completedReadyToBill[0]), title: `Create ${completedReadyToBill.length} draft invoice${completedReadyToBill.length > 1 ? "s" : ""}`, reason: `${completedReadyToBill.length} completed jobs are ready for billing.`, dataUsed: `${completedReadyToBill.length} completed jobs ready to bill`, risk: "medium", status: "pending" }) } : null,
    openInvoices.length > 0 ? { key: "invoice_reminders", type: "invoice_reminder", title: `Prepare reminders for ${openInvoices.length} open invoice${openInvoices.length > 1 ? "s" : ""}`, reason: overdueInvoices.length > 0 ? `${overdueInvoices.length} are overdue and need follow-up.` : "Unpaid invoices are ready for reminder drafts.", whatHappens: "AI prepares reminder drafts for owner approval. Messages are not sent automatically.", dataUsed: `${openInvoices.length} open invoices waiting for payment`, primary: "Prepare reminders", action: () => openSmartHubAction({ id: `invoice-reminder-${getAnyId(openInvoices[0])}`, action_type: "invoice_reminder", kind: "invoice_reminder", invoice_id: getAnyId(openInvoices[0]), title: `Prepare reminders for ${openInvoices.length} open invoices`, reason: "Unpaid invoices are ready for reminder drafts.", dataUsed: `${openInvoices.length} open invoices waiting for payment`, risk: "medium", status: "pending" }), detailAction: () => openSmartHubAction({ id: `invoice-reminder-${getAnyId(openInvoices[0])}`, action_type: "invoice_reminder", kind: "invoice_reminder", invoice_id: getAnyId(openInvoices[0]), title: `Prepare reminders for ${openInvoices.length} open invoices`, reason: "Unpaid invoices are ready for reminder drafts.", dataUsed: `${openInvoices.length} open invoices waiting for payment`, risk: "medium", status: "pending" }) } : null,
    waitingQuotes.length > 0 ? { key: "quote_followups", type: "quote_follow_up", title: `Follow up ${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""}`, reason: "Quotes are waiting on customer response.", whatHappens: "AI drafts a follow-up for review. No message is sent without approval.", dataUsed: `${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""} waiting for client response`, primary: "Review follow-ups", action: () => openSmartHubAction({ id: `quote-followup-${getAnyId(waitingQuotes[0])}`, action_type: "quote_follow_up", kind: "quote_follow_up", quote_id: getAnyId(waitingQuotes[0]), title: `Follow up ${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""}`, reason: "Quotes are waiting on customer response.", dataUsed: `${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""} waiting for client response`, risk: "low", status: "pending" }), detailAction: () => openSmartHubAction({ id: `quote-followup-${getAnyId(waitingQuotes[0])}`, action_type: "quote_follow_up", kind: "quote_follow_up", quote_id: getAnyId(waitingQuotes[0]), title: `Follow up ${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""}`, reason: "Quotes are waiting on customer response.", dataUsed: `${waitingQuotes.length} quote${waitingQuotes.length > 1 ? "s" : ""} waiting for client response`, risk: "low", status: "pending" }) } : null,
  ...
  <SmartModal open={Boolean(workspaceDrawer)} wide title={`${activeSmartHubSection || ""} workspace`} onClose={() => { setActiveSmartHubSection(null); setWorkspaceDrawer(null); setWorkspaceMode("list"); setWorkspaceRecord(null); }} actions={<button onClick={() => navigate(workspaceRoute(activeSmartHubSection, workspaceMode, workspaceRecord))} className="rounded-full border px-3 py-1 text-xs">Open full page</button>}>
    ...
    <div className="space-y-4 text-sm">
      <div className="rounded-2xl border border-slate-700/80 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 text-slate-100 shadow-lg">
        <p className="text-lg font-semibold capitalize">{activeSmartHubSection === "invoices" ? "Invoices" : (activeSmartHubSection || "Workspace")}</p>
        <p className="mt-1 text-sm text-slate-300">{activeSmartHubSection === "invoices" ? "Review ready-to-bill jobs, draft invoices and payment reminders." : "Embedded Smart Hub workspace with no duplicate sidebar or route shell."}</p>
      </div>
      ...
      {workspaceMode === "list" ? <div className="space-y-3">
        {activeSmartHubSection === "invoices" ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[["Ready to bill", completedReadyToBill.length],["Draft invoices", data.invoices.filter((i)=>String(i.status||"").toLowerCase()==="draft").length],["Open invoices", openInvoices.length],["Overdue", overdueInvoices.length]].map(([k,v]) => <div key={k} className="rounded-xl border border-slate-200 bg-[#f9f6ef] px-3 py-2"><p className="text-xs uppercase tracking-wide text-slate-500">{k}</p><p className="text-lg font-semibold text-slate-900">{v}</p></div>)}
        </div> : null}
        {activeSmartHubSection === "invoices" ? <div className="flex flex-wrap gap-2">
          <button onClick={() => setWorkspaceFilter("ready_to_bill")} className="rounded-full bg-teal-700 text-white px-3 py-1.5 text-xs">Review ready-to-bill</button>
          <button onClick={async()=>{const rows=invoiceDraftRows.filter((r)=>selectedDrafts[r.jobId] && !r.missingPrice);const skipped=invoiceDraftRows.filter((r)=>selectedDrafts[r.jobId] && r.missingPrice).length;let created=0;for (const r of rows){await approveDraftInvoice(r);created++;} await load(); toast.success(`${created} draft invoices created.${skipped?` ${skipped} need missing prices.`:''}`);}} className="rounded-full border px-3 py-1.5 text-xs">Approve selected drafts</button>
          <button onClick={async()=>{const rows=reminderRows.filter((r)=>selectedReminderDrafts[r.id]);await approveReminderDrafts(rows);}} className="rounded-full border px-3 py-1.5 text-xs">Prepare reminders</button>
          <button onClick={() => setWorkspaceMode("create")} className="rounded-full border px-3 py-1.5 text-xs">Create invoice</button>
        </div> : null}
        ...
        {activeSmartHubSection === "invoices" ? null : filteredWorkspaceRows.map((row, idx) => <div key={asId(row) || row.id || idx} className="rounded-xl border p-3">
  ...
  <SmartModal open={isActionModalOpen} wide title={String((selectedSmartHubAction || selectedAction)?.action_type || (selectedSmartHubAction || selectedAction)?.kind || "").toLowerCase() === "create_invoice_draft" ? "Review invoice drafts" : ((selectedSmartHubAction || selectedAction)?.title || "Action details")} onClose={() => setIsActionModalOpen(false)}>
    {String((selectedSmartHubAction || selectedAction)?.action_type || (selectedSmartHubAction || selectedAction)?.kind || (selectedSmartHubAction || selectedAction)?.type || "").toLowerCase() === "create_invoice_draft" ? <div className="space-y-4 text-sm"> 
  <SmartModal open={isActionModalOpen} wide title={String((selectedSmartHubAction || selectedAction)?.action_type || (selectedSmartHubAction || selectedAction)?.kind || (selectedSmartHubAction || selectedAction)?.type || "").toLowerCase() === "create_invoice_draft" ? "Review invoice drafts" : ((selectedSmartHubAction || selectedAction)?.title || "Action details")} onClose={() => setIsActionModalOpen(false)}>
    {(() => { const action = selectedSmartHubAction || selectedAction; const actionType = String(action?.action_type || action?.kind || action?.type || "").toLowerCase(); return actionType === "create_invoice_draft" ? <div className="space-y-4 text-sm"> 
      ...
      <div className="mt-3 flex flex-wrap gap-2"><button disabled={row.missingPrice || draftApprovalState[row.jobId]==='approved'} onClick={async()=>handleApproveReadyToBillDraft(row)} className="rounded-full bg-teal-700 px-3 py-1.5 text-white disabled:opacity-50">{draftApprovalState[row.jobId]==='approved'?'Approved':'Approve draft'}</button><button onClick={()=>handleEditReadyToBillDescription(row)} className="rounded-full border px-3 py-1.5">Edit description</button><button onClick={()=>setSelectedDrafts((s)=>({...s,[row.jobId]:false}))} className="rounded-full border px-3 py-1.5">Reject</button><button onClick={()=>handleOpenReadyToBillJob(row)} className="rounded-full border px-3 py-1.5">Open job</button></div></div>) : <div className="rounded-xl border bg-white p-3 text-slate-600">No completed jobs are ready to bill.</div>}</div> : null}
      {reviewMode === "jobDetail" && selectedReviewItem ? <div className="rounded-2xl border bg-[#fdfaf4] p-4 space-y-2"><p className="text-lg font-semibold">{selectedReviewItem.job.title || selectedReviewItem.job.name || "Job detail"}</p><p>Client: {selectedReviewItem.clientName}</p><p>Address: {selectedReviewItem.job.address || selectedReviewItem.job.job_address || "n/a"}</p><p>Status: {textOr(selectedReviewItem.job.status, "completed")}</p><p>Scheduled: {fmtDateTime(selectedReviewItem.job.scheduled_date || selectedReviewItem.job.date)}</p><p>Completed: {fmtDateTime(selectedReviewItem.job.completed_at || selectedReviewItem.job.completed_date || selectedReviewItem.job.updated_at)}</p><p>Assigned worker: {textOr(workers.find((w) => sameId(asId(w), selectedReviewItem.job.assigned_worker_id || selectedReviewItem.job.worker_id))?.name, "Unassigned")}</p><p>Pricing: Subtotal {fmtMoney(selectedReviewItem.subtotal)} · GST {fmtMoney(selectedReviewItem.gst)} · Total {fmtMoney(selectedReviewItem.total)}</p><p>Notes: {textOr(selectedReviewItem.job.notes || selectedReviewItem.job.job_notes || selectedReviewItem.job.description, "n/a")}</p><p>Completion notes: {textOr(selectedReviewItem.job.completion_notes || selectedReviewItem.job.worker_completion_notes || selectedReviewItem.job.worker_notes, "n/a")}</p><div className="flex flex-wrap gap-2 pt-2"><button onClick={() => openReviewJobEdit(selectedReviewItem)} className="rounded-full bg-teal-700 px-3 py-1.5 text-white">Edit job details</button><button onClick={handleBackToReadyToBill} className="rounded-full border px-3 py-1.5">Back to ready-to-bill</button><button onClick={() => navigate(`/jobs/${selectedReviewItem.jobId}`)} className="rounded-full border px-3 py-1.5">Open full job page</button></div></div> : null}
      {reviewMode === "jobEdit" && selectedReviewItem && reviewEditForm ? <div className="rounded-2xl border bg-[#fdfaf4] p-4 space-y-3"><p className="text-lg font-semibold">Edit job details</p><input className="w-full rounded-lg border p-2" value={reviewEditForm.title} onChange={(e)=>setReviewEditForm((s)=>({...s,title:e.target.value}))} placeholder="Job title" /><input className="w-full rounded-lg border p-2" value={reviewEditForm.address} onChange={(e)=>setReviewEditForm((s)=>({...s,address:e.target.value}))} placeholder="Address" /><input type="datetime-local" className="w-full rounded-lg border p-2" value={String(reviewEditForm.scheduled_date || "").slice(0,16)} onChange={(e)=>setReviewEditForm((s)=>({...s,scheduled_date:e.target.value}))} /><select className="w-full rounded-lg border p-2" value={reviewEditForm.assigned_worker_id} onChange={(e)=>setReviewEditForm((s)=>({...s,assigned_worker_id:e.target.value}))}><option value="">Unassigned</option>{workers.map((w)=><option key={asId(w)} value={asId(w)}>{w.name || w.email}</option>)}</select><div className="grid gap-2 sm:grid-cols-2"><input className="rounded-lg border p-2" value={reviewEditForm.pricing_type} onChange={(e)=>setReviewEditForm((s)=>({...s,pricing_type:e.target.value}))} placeholder="Pricing type" /><input type="number" className="rounded-lg border p-2" value={reviewEditForm.subtotal} onChange={(e)=>setReviewEditForm((s)=>({...s,subtotal:e.target.value}))} placeholder="Subtotal" /></div><input type="number" className="w-full rounded-lg border p-2" value={reviewEditForm.gst} onChange={(e)=>setReviewEditForm((s)=>({...s,gst:e.target.value}))} placeholder="GST" /><label className="text-xs font-semibold uppercase tracking-wide text-slate-500">AI invoice description</label><textarea className="w-full rounded-lg border p-2" rows={3} value={reviewEditForm.invoice_description_draft} onChange={(e)=>setReviewEditForm((s)=>({...s,invoice_description_draft:e.target.value}))} placeholder="Invoice description draft" /><div className="flex flex-wrap gap-2"><button onClick={async ()=>{await handleSaveReadyToBillDescription(selectedReviewItem, reviewEditForm.invoice_description_draft);}} className="rounded-full bg-teal-700 px-3 py-1.5 text-white">Save description</button><button onClick={saveReviewJobEdit} className="rounded-full border px-3 py-1.5">Save job changes</button><button onClick={handleBackToReadyToBill} className="rounded-full border px-3 py-1.5">Cancel</button><button onClick={() => navigate(`/jobs/${selectedReviewItem.jobId}/edit`)} className="rounded-full border px-3 py-1.5">Open full edit page</button></div></div> : null}
    ...
    </div> : <div className="space-y-3 text-sm"><div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-amber-900"><p className="font-semibold">{selectedAction ? `Unsupported action type: ${String(selectedAction?.action_type || selectedAction?.kind || selectedAction?.type || "unknown").toLowerCase() || "unknown"}` : "Action details could not load."}</p><p className="text-xs mt-1">{selectedAction ? "This action type is not recognised by the details renderer." : "Close this window and try again."}</p></div><div className="rounded-xl border p-3"><p><span className="font-semibold">Title:</span> {textOr(selectedAction?.title, "n/a")}</p><p><span className="font-semibold">Reason:</span> {textOr(selectedAction?.reason, "n/a")}</p><p><span className="font-semibold">Data used:</span> {textOr(selectedAction?.dataUsed || selectedAction?.summary, "n/a")}</p><p><span className="font-semibold">Risk:</span> {textOr(selectedAction?.risk, "n/a")}</p><p><span className="font-semibold">Status:</span> {textOr(selectedAction?.status, "pending")}</p></div><div className="flex gap-2"><button onClick={() => setIsActionModalOpen(false)} className="rounded-full border px-3 py-1.5">Close</button><button onClick={() => load()} className="rounded-full bg-teal-700 px-3 py-1.5 text-white">Reload Smart Hub</button></div></div>}
    </div> : <div className="space-y-3 text-sm"><div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-amber-900"><p className="font-semibold">{action ? `Unsupported action type: ${actionType || "unknown"}` : "Action details could not load."}</p><p className="text-xs mt-1">{action ? "This action type is not recognised by the details renderer." : "Close this window and try again."}</p></div><div className="rounded-xl border p-3"><p><span className="font-semibold">Title:</span> {textOr(action?.title, "n/a")}</p><p><span className="font-semibold">Reason:</span> {textOr(action?.reason, "n/a")}</p><p><span className="font-semibold">Data used:</span> {textOr(action?.dataUsed || action?.summary, "n/a")}</p><p><span className="font-semibold">Risk:</span> {textOr(action?.risk, "n/a")}</p><p><span className="font-semibold">Status:</span> {textOr(action?.status, "pending")}</p></div><div className="flex gap-2"><button onClick={() => setIsActionModalOpen(false)} className="rounded-full border px-3 py-1.5">Close</button><button onClick={() => window.location.reload()} className="rounded-full bg-teal-700 px-3 py-1.5 text-white">Reload Smart Hub</button></div></div>; })()}
  </SmartModal>
  </Layout></>);
}
