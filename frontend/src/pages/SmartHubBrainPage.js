...
export default function SmartHubBrainPage() {
  ...
  return <Layout smartHubMode><div className="min-h-screen bg-[#f3f6fb]"><div className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6">
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-xl font-semibold">Today’s Plan</h3><p className="mt-2 text-sm text-slate-700">{missionText}</p><p className="mt-2 text-xs text-slate-500">AI prepares actions. You stay in control.</p></section>
    {needsSetup ? <section className="rounded-2xl border border-blue-200 bg-blue-50/60 p-5 shadow-sm"><h3 className="text-lg font-semibold text-slate-900">Get AI ready</h3><p className="mt-1 text-sm text-slate-700">AI needs a few basics before it can run the day. Add a client and worker to unlock assignment recommendations.</p><ol className="mt-3 list-decimal space-y-1 pl-5 text-sm text-slate-700"><li>Add business type / industry if missing.</li><li>Add first client.</li><li>Add first worker.</li><li>Create first job.</li><li>Complete first job so AI can prepare invoices.</li></ol><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => openWorkspace("clients", "create")} className="rounded-full bg-blue-600 px-3 py-2 text-xs font-semibold text-white">Add client</button><button onClick={() => openWorkspace("crew")} className="rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700">Add worker</button><button onClick={() => openWorkspace("jobs", "create")} className="rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700">Create job</button></div></section> : null}
    <section className="grid gap-4 lg:grid-cols-[2fr_1fr]"><div className="space-y-4"><div className="rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-xl font-semibold">Owner Decision Queue</h3><p className="text-sm text-slate-600">AI has prepared these actions. Approve, edit, or reject.</p></div>{[["Needs decision", inboxGroups.needsDecision],["Ready drafts", inboxGroups.readyDrafts],["Monitoring", inboxGroups.monitoring],["Completed by AI", inboxGroups.completedToday]].map(([g, items]) => <div key={g} className="rounded-2xl border bg-white p-4 shadow-sm"><h4 className="text-lg font-semibold">{g}</h4><div className="mt-3 space-y-3">{items.length ? items.map((a, i) => <div key={a.id || a._id || i} className="rounded-2xl border p-4 shadow-sm"><p className="font-semibold">{a.title}</p><p className="text-sm mt-1">{g === "Completed by AI" ? `What happened: ${a.completedLabel || "Owner approved action."}` : `What AI prepared: ${a.reason}`}</p><p className="text-xs text-slate-500 mt-1">Real data used: {a.dataUsed}</p>{g === "Completed by AI" ? <p className="text-xs text-slate-500 mt-1">Approved by: {user?.name || "Owner"} · {new Date().toLocaleString()}</p> : null}<div className="mt-2 flex flex-wrap gap-2"><span className="text-xs border rounded-full px-2 py-1">{g === "Completed by AI" ? "Status: Completed" : `Risk: ${a.risk || "medium"}`}</span>{g !== "Completed by AI" ? <><button onClick={() => handleApproveAction(a)} className="rounded-full bg-blue-600 text-white px-3 py-1.5 text-xs">{approvingActionId === String(a.id || a._id || a.title) ? "Approving…" : (a.primary || "Approve")}</button><button onClick={() => handleEditAction(a)} className="rounded-full border px-3 py-1.5 text-xs">Edit</button><button onClick={() => handleRejectAction(a)} className="rounded-full border px-3 py-1.5 text-xs">Reject</button></> : null}<button onClick={() => handleReviewAction(a)} className="rounded-full border px-3 py-1.5 text-xs">View details</button></div></div>) : <p className="text-sm text-slate-500">{g === "Completed by AI" ? "No approved AI actions yet. When you approve work, it will appear here." : "All clear. AI has no urgent decisions waiting."}</p>}</div></div>)}</div>
