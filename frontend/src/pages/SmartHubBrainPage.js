...
export default function SmartHubBrainPage() {
  ...
  const renderDrawerContent = () => {
    ...
    if (workspaceDrawer === "Invoices") {
      return (
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-semibold text-[#121417]">Invoices Workspace</h3>
          ...
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-xl border border-[#c8c0b2] tradie-panel p-3">
                <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
                <p className="mt-1 text-xl font-semibold text-[#121417]">{value}</p>
              </article>
            ))}
          </section>
          {!readyToBillJobs.length ? (
            ...
            readyToBillJobs.map((job) => {
              ...
              return (
                <div key={String(job?.id || job?._id || job?.job_id || Math.random())} className="rounded-xl border border-[#c8c0b2] tradie-panel p-4">
                  <p className="font-semibold text-[#121417]">{textOr(job?.title || job?.name, "Untitled job")}</p>
                  ...
                  <p className="mt-2 text-sm text-[#24272c]">{buildInvoiceDescription({ job, client })}</p>
                  {Number.isFinite(subtotal) ? (
                    <div className="mt-3 grid grid-cols-1 gap-1 text-sm text-[#24272c] sm:grid-cols-3">
                  ...
                  <button
                    ...
                    className="mt-4 rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white hover:bg-[#0b4f42] disabled:cursor-not-allowed disabled:opacity-60"
                  >
    ...
    if (workspaceDrawer === "Payment Reminders") {
      ...
      return (
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-semibold text-[#121417]">Payment Reminders</h3>
            <p className="text-sm text-[#5f646b]">Review AI-prepared reminder drafts for unpaid invoices.</p>
          </div>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[['Open invoices', reminderInvoices.length], ['Overdue invoices', overdueCount], ['Draft reminders', draftCount], ['Missing contact details', missingContactCount]].map(([label, value]) => <article key={label} className="rounded-xl border border-[#c8c0b2] tradie-panel p-3"><p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p><p className="mt-1 text-xl font-semibold text-[#121417]">{value}</p></article>)}
          </section>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => approveMany(selectedReminderIds)} className="rounded-full bg-[#0f6b57] px-3 py-2 text-sm font-medium text-white hover:bg-[#0b4f42]">Approve selected reminders</button>
            <button type="button" onClick={() => approveMany(reminderInvoices.map((inv) => String(inv?.id || inv?._id || inv?.invoice_id || "")).filter(Boolean))} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm font-medium text-[#24272c]">Approve all ready reminders</button>
            <button type="button" onClick={() => setSelectedReminderIds([])} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm font-medium text-[#24272c]">Reject selected</button>
          </div>
          {!reminderInvoices.length ? <p className="text-sm text-[#5f646b]">No unpaid invoices need reminders right now.</p> : reminderInvoices.map((inv) => {
            ...
            return <article key={id} className="rounded-xl border border-[#c8c0b2] tradie-panel p-4"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedReminderIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#5f646b]">{approvedReminderIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#121417]">{textOr(client?.name || inv?.client_name || inv?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Invoice: {textOr(inv?.invoice_number || inv?.number || inv?.title, "Untitled invoice")}</p><p className="text-sm text-[#5f646b]">Amount due: {money(invoiceBalance(inv))}</p><p className="text-sm text-[#5f646b]">Due date: {textOr(due, "No due date")}</p><p className="text-sm text-[#5f646b]">Status: {textOr(inv?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Overdue days: {daysOverdue(inv) ?? "—"}</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Warning: missing client contact details. You can save/approve this draft, but it is not ready to send.</p> : null}{isEditing ? <textarea className="mt-3 w-full rounded-lg border border-[#c8c0b2] p-2 text-sm" rows={4} value={reminderDrafts[id] || ""} onChange={(e) => setReminderDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#efede8] p-3 text-sm text-[#24272c]">{reminderDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setEditingDraft((prev) => ({ ...prev, [id]: false })); setReminderDrafts((prev) => ({ ...prev, [id]: buildInvoiceReminderMessage({ client, invoice: inv, business: user, channel: "email" }) })); }} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(inv)} className="rounded bg-[#0f6b57] px-3 py-1 text-sm text-white">Approve reminder draft</button><button type="button" onClick={() => navigate(`/invoices/${id}`)} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Open full invoice page</button></div></article>;
          })}
        </div>
      );
    }
    ...
    if (workspaceDrawer === "Quotes" || workspaceDrawer === "Quote Follow-ups") {
      ...
      return (
        <div className="space-y-4">
          <div><h3 className="text-lg font-semibold text-[#121417]">Quote Follow-ups</h3><p className="text-sm text-[#5f646b]">Review AI-prepared quote follow-up drafts before anything is sent.</p></div>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Quotes waiting", waitingQuotes.length], ["Follow-ups prepared", preparedCount], ["Missing contact details", missingContactCount], ["Oldest waiting quote", oldestWaiting ? `${oldestWaiting}d` : "—"]].map(([label, value]) => <article key={label} className="rounded-xl border border-[#c8c0b2] tradie-panel p-3"><p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p><p className="mt-1 text-xl font-semibold text-[#121417]">{value}</p></article>)}</section>
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => approveMany(selectedQuoteIds)} className="rounded-full bg-[#0f6b57] px-3 py-2 text-sm font-medium text-white hover:bg-[#0b4f42]">Approve selected follow-ups</button><button type="button" onClick={() => approveMany(waitingQuotes.map((q) => String(q?.id || q?._id || q?.quote_id || "")).filter(Boolean))} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm font-medium text-[#24272c]">Approve all ready follow-ups</button><button type="button" onClick={() => setSelectedQuoteIds([])} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm font-medium text-[#24272c]">Reject selected</button></div>
          {!waitingQuotes.length ? <p className="text-sm text-[#5f646b]">No quotes are waiting for follow-up right now.</p> : waitingQuotes.map((quote) => {
            ...
            return <article key={id} className="rounded-xl border border-[#c8c0b2] tradie-panel p-4"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedQuoteIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#5f646b]">{approvedQuoteIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#121417]">{textOr(client?.name || quote?.client_name || quote?.customer_name, "Unknown client")}</p><p className="text-sm text-[#5f646b]">Quote: {textOr(quote?.quote_number || quote?.number || quote?.title, "Untitled quote")}</p><p className="text-sm text-[#5f646b]">Amount: {money(Number(quote?.total ?? quote?.amount ?? quote?.price))}</p><p className="text-sm text-[#5f646b]">Status: {textOr(quote?.status, "unknown")}</p><p className="text-sm text-[#5f646b]">Created/Sent: {textOr(displayDate, "Unknown date")}</p><p className="text-sm text-[#5f646b]">Age: {age ?? "—"} days</p><p className="text-sm text-[#5f646b]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Warning: missing client contact details. You can save this draft, but it is not ready to send.</p> : null}{editingQuoteDraft[id] ? <textarea className="mt-3 w-full rounded-lg border border-[#c8c0b2] p-2 text-sm" rows={4} value={quoteDrafts[id] || ""} onChange={(e) => setQuoteDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#efede8] p-3 text-sm text-[#24272c]">{quoteDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setQuoteDrafts((prev) => ({ ...prev, [id]: quoteDraftOriginals[id] || prev[id] })); setEditingQuoteDraft((prev) => ({ ...prev, [id]: false })); }} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(quote)} className="rounded bg-[#0f6b57] px-3 py-1 text-sm text-white">Approve follow-up draft</button><button type="button" onClick={() => navigate(`/quotes/${id}`)} className="rounded border border-[#c8c0b2] px-3 py-1 text-sm">Open full quote page</button></div></article>;
          })}
        </div>
      );
    }
    if (workspaceDrawer === "AI Dispatch") {
      ...
      return <div className="space-y-4"><div><h3 className="text-lg font-semibold text-[#121417]">AI Dispatch</h3><p className="text-sm text-[#5f646b]">Review recommended worker assignments before jobs are updated.</p></div>
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Unassigned jobs", dispatchRecs.length],["Crew available", crewAvailable],["Schedule conflicts", conflicts],["Missing job details", missingData]].map(([l,v])=><article key={l} className="rounded-xl border border-[#c8c0b2] tradie-panel p-3"><p className="text-xs uppercase tracking-wide text-[#5f646b]">{l}</p><p className="mt-1 text-xl font-semibold text-[#121417]">{v}</p></article>)}</section>
      {!dispatchRecs.length ? <p className="text-sm text-[#5f646b]">No unassigned jobs require approval right now.</p> : dispatchRecs.map(({ job, jobId, recommendation, selectedWorkerId }) => {
        ...
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={!selectedWorkerId || savingJobId===jobId} onClick={() => applyAssign(job, selectedWorkerId)} className="rounded bg-[#0f6b57] px-3 py-1 text-sm text-white">Approve assignment</button><button type="button" onClick={() => navigate(`/jobs/${jobId}`)} className="rounded border px-3 py-1 text-sm">Open full job page</button><button type="button" onClick={async () => { setRejectedDispatchIds((prev) => ({ ...prev, [jobId]: true })); await logActivity({ action_type: "recommendation_rejected", title: "Recommendation rejected", message: `AI recommendation rejected: ${textOr(job?.title, "Job")}`, related_type: "job", related_id: jobId, status: "rejected" }); }} className="rounded border px-3 py-1 text-sm">Reject recommendation</button></div></article>;
      })}</div>;
    }

    return (
      <div className="space-y-3">
        <h3 className="text-lg font-semibold text-[#121417]">{workspaceDrawer} Workspace</h3>
        <p className="text-sm text-[#5f646b]">Nothing needs attention here.</p>
        <button
          type="button"
          onClick={() => navigate(`/${workspaceDrawer.toLowerCase()}`)}
          className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white hover:bg-[#0b4f42]"
        >
          Open full page
        </button>
      </div>
  ...
  return (
    <Layout title="Smart Hub">
      <div className="min-h-screen bg-[#efede8] text-[#121417] tradie-page-bg">
        <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
          <section className="rounded-3xl border border-white/10 bg-[linear-gradient(135deg,#111317_0%,#171a1f_55%,#20242a_100%)] border border-white/10 shadow-[0_24px_60px_rgba(0,0,0,0.35)] p-6 text-slate-100 shadow-2xl">
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-[#f9f7f2]/5 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" onClick={runScanNow} className="rounded-lg bg-amber-600 text-[#121417] px-4 py-2 text-sm font-medium text-white hover:bg-[#0f6b57]">Run today's AI plan</button>
              ...
              <button type="button" onClick={() => openApprovalCentre({ tab: approvalCounts.needs_decision ? "needs_decision" : "all" })} className={`rounded-full px-3 py-2 text-xs font-semibold ${approvalBadgeTone === "amber" ? "bg-[#f59e0b]/15 text-[#ffb15c]" : approvalBadgeTone === "green" ? "bg-[#d8f3e8] text-[#0b4f42]" : "bg-[#efede8] text-[#5f646b]"}`}>{approvalCounts.all ? `${approvalCounts.all} approvals` : "All clear"}</button>
              <div className="relative">
                ...
                {notificationOpen ? <div className="absolute right-0 z-20 mt-2 w-[340px] rounded-xl border border-[#c8c0b2] bg-[#f9f7f2] p-3 text-[#121417] shadow-xl">
                  <p className="text-xs font-semibold uppercase tracking-wide text-[#5f646b]">Notifications</p>
                  {notificationItems.slice(0, 10).map((item) => <button key={item.id} type="button" onClick={() => { setReadNotificationIds((prev) => ({ ...prev, [item.id]: true })); setNotificationOpen(false); item.action?.(); }} className="mt-2 w-full rounded-lg border border-slate-100 p-2 text-left hover:bg-[#efede8]"><p className="text-[11px] font-semibold uppercase text-[#5f646b]">{item.section}</p><p className="text-sm font-medium">{item.title}</p><p className="text-xs text-[#5f646b]">{item.subtitle}</p><p className="text-[11px] text-slate-400">{item.time}</p></button>)}
                </div> : null}
              </div>
            </div>
          </section>
          ...
          <section className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-xl border border-[#c8c0b2] tradie-panel p-4">
                <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
          ...
          <section className="mt-6 rounded-2xl border border-[#c8c0b2] tradie-panel p-5">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => openApprovalCentre({ tab: "all" })} className={`rounded-full px-3 py-1 text-xs font-medium ${approvalBadgeTone === "amber" ? "bg-[#f59e0b]/15 text-[#ffb15c]" : approvalBadgeTone === "green" ? "bg-[#d8f3e8] text-[#0b4f42]" : "bg-[#efede8] text-[#5f646b]"}`}>{approvalCounts.all ? `${approvalCounts.all} approvals` : "All clear"}</button>
          ...
          <section className="mt-6 grid gap-4 lg:grid-cols-2">
            <article className="rounded-2xl border border-[#c8c0b2] tradie-panel p-5">
              ...
              <p className="mt-4 rounded-lg bg-[#efede8] px-3 py-2 text-sm text-[#24272c]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                {[["Money waiting", openInvoices.length], ["Billing ready", readyToBillJobs.length], ["Dispatch pressure", unassignedJobs.length], ["Pipeline", waitingQuotes.length], ["Crew", crewAvailable]].map(([label, value]) => (
                  <button
                    ...
                    className="rounded-xl border border-[#c8c0b2] bg-[#f9f7f2] p-3 text-left"
                  >
                    <p className="text-xs uppercase tracking-wide text-[#5f646b]">{label}</p>
                    <p className="mt-1 text-xl font-semibold text-[#121417]">{value}</p>
                  </button>
          ...
          <section className="mt-6 rounded-2xl border border-[#c8c0b2] tradie-panel p-4">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <button type="button" onClick={() => openWorkspace("Payment Reminders", "reminders")} className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#0b4f42]">Prepare reminders</button>
              <button type="button" onClick={() => openWorkspace("Quote Follow-ups", "followUps")} className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#0b4f42]">Review follow-ups</button>
              <button type="button" onClick={() => openWorkspace("AI Dispatch", "assign")} className="rounded-lg bg-[#0f6b57] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#0b4f42]">Assign workers</button>
            </div>
          </section>
          ...
          <section className="mt-6 rounded-2xl border border-[#c8c0b2] tradie-panel p-4">
            ...
            <div className="mt-2 text-sm text-[#24272c] space-y-1">
            ...
            <button type="button" onClick={() => setAiSettingsOpen(true)} className="mt-3 rounded-lg bg-[#0f6b57] px-3 py-2 text-sm text-white">Open AI Settings</button>
          </section>

          <section className="mt-4 rounded-2xl border border-[#c8c0b2] tradie-panel p-4">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-[#5f646b]">Recent Smart Hub activity</h3>
            <div className="mt-2 flex gap-2">{[["all","All"],["completed","Completed"],["rejected","Rejected"],["draft_prepared","Drafts"]].map(([k,l]) => <button key={k} type="button" onClick={() => setActivityFilter(k)} className={`rounded px-2 py-1 text-xs ${activityFilter===k?"bg-[#20242a] text-white":"bg-[#efede8] text-[#24272c]"}`}>{l}</button>)}</div>{!activity.length ? <p className="mt-2 text-sm text-[#5f646b]">No AI actions approved yet. Approved work will appear here.</p> : <ul className="mt-3 space-y-2 text-sm text-[#24272c]">{activity.filter((a)=>activityFilter==="all"?true:String(a?.status||"")===activityFilter).map((a) => <li key={String(a?.id||a?._id)} className="rounded-lg border border-[#c8c0b2] p-2"><p>{a?.message || a?.title}</p><p className="text-xs text-[#5f646b]">{textOr(a?.status, "completed")} · {a?.approved_by_name ? `${a.approved_by_name} · ` : ""}{new Date(a?.created_at || Date.now()).toLocaleString()}</p></li>)}</ul>}
          </section>
        </div>

        {workspaceDrawer ? (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#171717]/60 p-0 sm:items-center sm:p-6">
            <div className="h-[86vh] w-full max-w-3xl rounded-t-2xl bg-[#f9f7f2] sm:h-auto sm:rounded-2xl">
              <div className="flex items-center justify-between border-b border-[#c8c0b2] px-4 py-3">
                <h2 className="font-semibold text-[#121417]">{workspaceDrawer}</h2>
                <button type="button" onClick={() => { setWorkspaceDrawer(""); setWorkspaceMode("list"); setWorkspaceRecord(null); }} className="rounded-md border border-[#c8c0b2] px-3 py-1 text-sm text-[#24272c]">
                  Close
                </button>
              </div>
        ...
        {aiSettingsOpen ? (
          <div className="fixed inset-0 z-[70] bg-[#171717]/70 p-4">
            <div className="mx-auto mt-10 max-w-xl rounded-2xl bg-[#f9f7f2] p-4">
              ...
              <div className="mt-4 flex gap-2"><button type="button" className="rounded bg-[#0f6b57] px-3 py-2 text-white" onClick={async ()=>{ try { const res = await patch('/api/ai-operator/settings', aiSettings); setAiSettings((p)=>({...p,...(res?.settings||{})})); setToast({kind:'success',message:'AI settings saved.'}); setAiSettingsOpen(false);} catch { localStorage.setItem("smart_hub_ai_settings_local", JSON.stringify(aiSettings)); setToast({kind:'success',message:'Saved locally until backend setting is added.'}); setAiSettingsOpen(false); } }}>Save</button><button type="button" className="rounded border px-3 py-2" onClick={()=>setAiSettingsOpen(false)}>Close</button></div>
            </div>
          </div>
        ) : null}
        ...
        {approvalCentreOpen ? (
          <div className="fixed inset-0 z-[80] overflow-hidden bg-[#171717]/75">
            <div className="mx-auto flex h-[100dvh] w-full max-w-6xl flex-col overflow-hidden bg-stone-50 sm:my-4 sm:h-[94vh] sm:rounded-3xl">
              <div className="flex-none border-b border-[#c8c0b2] bg-[#f9f7f2] px-5 py-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <h2 className="text-xl font-semibold text-[#121417]">AI Approval Centre</h2>
                    <p className="mt-1 text-sm text-[#5f646b]">Review, edit or approve everything AI prepared.</p>
                  </div>
                  <button type="button" onClick={() => setApprovalCentreOpen(false)} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm">Close</button>
                </div>
                <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{APPROVAL_GROUPS.map((g)=><button key={g} type="button" onClick={()=>setApprovalFilter(g)} className={`shrink-0 rounded px-3 py-1 text-xs ${approvalFilter===g?"bg-slate-900 text-white ring-2 ring-slate-500":"bg-[#efede8] text-[#24272c]"}`}>{g === "all" ? "All" : g.replace("_"," ")} ({approvalCounts[g] || 0})</button>)}</div>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-5 py-4 pb-24">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {filteredApprovalItems.map((item) => {
                    ...
                    return (
                    <article key={item.id} className="rounded-xl border border-[#c8c0b2] bg-[#f9f7f2] p-4 shadow-sm">
                      ...
                      <p className="font-semibold text-[#121417]">{meta.title}</p><p className="mt-1 text-sm text-[#5f646b]">{meta.subtitle}</p><p className="mt-1 text-sm text-[#24272c]">{meta.reason}</p><p className="mt-1 text-xs text-[#5f646b]">{meta.dataUsed}</p><p className="mt-1 text-xs text-[#5f646b]">{meta.whatHappens}</p><p className="mt-1 text-xs text-[#5f646b]">Risk: {meta.risk}</p>{meta.contactSummary ? <p className="mt-1 text-xs text-[#5f646b]">{meta.contactSummary}</p> : null}
                      {item.type === "invoice_reminder" ? <><div className="mt-2 flex gap-1 text-[11px]"><span className="rounded bg-[#efede8] px-2 py-0.5">Email</span><span className="rounded bg-[#efede8] px-2 py-0.5">SMS</span></div><p className="mt-2 rounded bg-[#efede8] p-2 text-xs text-[#24272c]">{reminderDrafts[item.relatedId] || buildInvoiceReminderMessage({ client: findByIds(clients, [item.invoice?.client_id, item.invoice?.clientId]), invoice: item.invoice, business: user, channel: "email" })}</p></> : null}
                      {item.type === "quote_follow_up" ? <><div className="mt-2 flex gap-1 text-[11px]"><span className="rounded bg-[#efede8] px-2 py-0.5">Email</span><span className="rounded bg-[#efede8] px-2 py-0.5">SMS</span></div><p className="mt-2 rounded bg-[#efede8] p-2 text-xs text-[#24272c]">{quoteDrafts[item.relatedId] || buildQuoteFollowUpMessage({ client: findByIds(clients, [item.quote?.client_id, item.quote?.clientId]), quote: item.quote, business: user, channel: "email" })}</p></> : null}
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button type="button" onClick={() => approveApprovalItem(item)} className="rounded-full bg-[#0f6b57] px-3 py-2 text-sm font-medium text-white hover:bg-[#0b4f42]">Approve</button>
                        <button type="button" onClick={()=>setApprovalDetail(item)} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm">Edit</button>
                        <button type="button" onClick={() => rejectApprovalItem(item)} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm">Reject</button>
                        <button type="button" onClick={()=>setApprovalDetail(item)} className="rounded-lg border border-[#c8c0b2] px-3 py-2 text-sm">Details</button>
                      </div>
                    </article>
                  );})}
                  {!filteredApprovalItems.length ? <article className="rounded-xl border border-[#c8c0b2] bg-[#f9f7f2] p-4 shadow-sm"><p className="font-semibold text-[#121417]">No actions in this section.</p><div className="mt-3 flex gap-2"><button type="button" onClick={runScanNow} className="rounded-lg bg-[#0f6b57] px-3 py-2 text-sm text-white">Run today's AI plan</button></div></article> : null}
                </div>
              </div>
            </div>
          </div>
