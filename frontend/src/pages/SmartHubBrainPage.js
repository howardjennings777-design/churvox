...
export default function SmartHubBrainPage() {
  ...
  const renderDrawerContent = () => {
    ...
    if (workspaceDrawer === "Clients") {
      ...
      return <div className="space-y-4"><div><h3 className="text-lg font-semibold text-[#171717]">Clients Workspace</h3><p className="text-sm text-[#4b4f53]">Search, review and update clients without leaving Smart Hub.</p></div><div className="flex flex-wrap gap-2"><input className="min-w-[220px] flex-1 rounded border p-2" value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} placeholder="Search clients" /><button type="button" className="rounded bg-[#c65a1e] px-3 py-2 text-sm text-white" onClick={() => { setWorkspaceEditForm({}); setWorkspaceMode("add"); }}>Add client</button><button type="button" className="rounded border px-3 py-2 text-sm" onClick={() => navigate("/clients")}>Open full clients page</button></div>{!filteredClients.length ? <p className="text-sm text-[#4b4f53]">No clients found.</p> : filteredClients.map((client) => { const id = String(client?.id || client?._id || ""); const counts = makeClientCounts(client); return <article key={id} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4"><p className="font-semibold text-[#171717]">{safeText(client?.name, "Unknown client")}</p><p className="text-sm text-[#4b4f53]">{safeText(client?.email)}</p><p className="text-sm text-[#4b4f53]">{safeText(client?.phone)}</p><p className="text-sm text-[#4b4f53]">{safeText(client?.address)}</p><p className="mt-1 text-xs text-[#4b4f53]">Active/open jobs: {counts.activeJobs} · Open invoices: {counts.openInvoices} · Quotes: {counts.quotesCount}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="rounded border px-3 py-1 text-sm" onClick={() => openDetail(client)}>View details</button><button type="button" className="rounded border px-3 py-1 text-sm" onClick={() => startEdit(client)}>Edit</button><button type="button" className="rounded border px-3 py-1 text-sm" onClick={() => navigate(`/jobs/new?client_id=${id}`)}>New job (open full page)</button><button type="button" className="rounded border px-3 py-1 text-sm" onClick={() => navigate(`/quotes/new?client_id=${id}`)}>New quote (open full page)</button><button type="button" className="rounded border px-3 py-1 text-sm" onClick={() => navigate(`/clients/${id}`)}>Open full client page</button></div></article>; })}</div>;
    }
      ...
      return (
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[#4b4f53]">Review ready-to-bill jobs, draft invoices and payment reminders.</p>
          </div>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-3">
                <p className="text-xs uppercase tracking-wide text-[#636a70]">{label}</p>
                <p className="mt-1 text-xl font-semibold text-[#171717]">{value}</p>
              </article>
            ))}
          </section>
          {!readyToBillJobs.length ? (
            <p className="text-sm text-[#4b4f53]">No ready-to-bill jobs right now.</p>
          ) : (
            readyToBillJobs.map((job) => {
              ...
              return (
                <div key={String(job?.id || job?._id || job?.job_id || Math.random())} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4">
                  <p className="font-semibold text-[#171717]">{textOr(job?.title || job?.name, "Untitled job")}</p>
                  <p className="text-sm text-[#4b4f53]">Client: {textOr(client?.name, "Unknown client")}</p>
                  <p className="text-sm text-[#4b4f53]">Address: {textOr(job?.address || job?.location, "No address saved")}</p>
                  <p className="text-sm text-[#4b4f53]">Completed: {textOr(job?.completed_at || job?.updated_at, "Unknown date")}</p>
                  ...
                  <button
                    ...
                    className="mt-4 rounded-lg bg-[#c65a1e] px-4 py-2 text-sm font-medium text-white hover:bg-[#a64917] disabled:cursor-not-allowed disabled:opacity-60"
                  >
      ...
      return (
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[#4b4f53]">Review AI-prepared reminder drafts for unpaid invoices.</p>
          </div>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[['Open invoices', reminderInvoices.length], ['Overdue invoices', overdueCount], ['Draft reminders', draftCount], ['Missing contact details', missingContactCount]].map(([label, value]) => <article key={label} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-3"><p className="text-xs uppercase tracking-wide text-[#636a70]">{label}</p><p className="mt-1 text-xl font-semibold text-[#171717]">{value}</p></article>)}
          </section>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => approveMany(selectedReminderIds)} className="rounded-lg bg-[#c65a1e] px-3 py-2 text-sm font-medium text-white">Approve selected reminders</button>
          ...
          {!reminderInvoices.length ? <p className="text-sm text-[#4b4f53]">No unpaid invoices need reminders right now.</p> : reminderInvoices.map((inv) => {
            const id = String(inv?.id || inv?._id || inv?.invoice_id || "");
            const client = findByIds(clients, [inv?.client_id, inv?.clientId], ["id", "_id", "client_id"]);
            ...
            return <article key={id} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedReminderIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#636a70]">{approvedReminderIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#171717]">{textOr(client?.name || inv?.client_name || inv?.customer_name, "Unknown client")}</p><p className="text-sm text-[#4b4f53]">Invoice: {textOr(inv?.invoice_number || inv?.number || inv?.title, "Untitled invoice")}</p><p className="text-sm text-[#4b4f53]">Amount due: {money(invoiceBalance(inv))}</p><p className="text-sm text-[#4b4f53]">Due date: {textOr(due, "No due date")}</p><p className="text-sm text-[#4b4f53]">Status: {textOr(inv?.status, "unknown")}</p><p className="text-sm text-[#4b4f53]">Overdue days: {daysOverdue(inv) ?? "—"}</p><p className="text-sm text-[#4b4f53]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Warning: missing client contact details. You can save/approve this draft, but it is not ready to send.</p> : null}{isEditing ? <textarea className="mt-3 w-full rounded-lg border border-slate-300 p-2 text-sm" rows={4} value={reminderDrafts[id] || ""} onChange={(e) => setReminderDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#ece7dc] p-3 text-sm text-slate-800">{reminderDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-slate-300 px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-slate-300 px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setEditingDraft((prev) => ({ ...prev, [id]: false })); setReminderDrafts((prev) => ({ ...prev, [id]: buildInvoiceReminderMessage({ client, invoice: inv, business: user, channel: "email" }) })); }} className="rounded border border-slate-300 px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(inv)} className="rounded bg-[#c65a1e] px-3 py-1 text-sm text-white">Approve reminder draft</button><button type="button" onClick={() => navigate(`/invoices/${id}`)} className="rounded border border-slate-300 px-3 py-1 text-sm">Open full invoice page</button></div></article>;
          })}
        </div>
      );
    }
    ...
    if (workspaceDrawer === "Quotes" || workspaceDrawer === "Quote Follow-ups") {
      ...
      return (
        <div className="space-y-4">
          <div><h3 className="text-lg font-semibold text-[#171717]">Quote Follow-ups</h3><p className="text-sm text-[#4b4f53]">Review AI-prepared quote follow-up drafts before anything is sent.</p></div>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Quotes waiting", waitingQuotes.length], ["Follow-ups prepared", preparedCount], ["Missing contact details", missingContactCount], ["Oldest waiting quote", oldestWaiting ? `${oldestWaiting}d` : "—"]].map(([label, value]) => <article key={label} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-3"><p className="text-xs uppercase tracking-wide text-[#636a70]">{label}</p><p className="mt-1 text-xl font-semibold text-[#171717]">{value}</p></article>)}</section>
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => approveMany(selectedQuoteIds)} className="rounded-lg bg-[#c65a1e] px-3 py-2 text-sm font-medium text-white">Approve selected follow-ups</button><button type="button" onClick={() => approveMany(waitingQuotes.map((q) => String(q?.id || q?._id || q?.quote_id || "")).filter(Boolean))} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700">Approve all ready follow-ups</button><button type="button" onClick={() => setSelectedQuoteIds([])} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700">Reject selected</button></div>
          {!waitingQuotes.length ? <p className="text-sm text-[#4b4f53]">No quotes are waiting for follow-up right now.</p> : waitingQuotes.map((quote) => {
            const id = String(quote?.id || quote?._id || quote?.quote_id || "");
            const client = findByIds(clients, [quote?.client_id, quote?.clientId], ["id", "_id", "client_id"]);
            ...
            return <article key={id} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedQuoteIds.includes(id)} onChange={() => toggleSelected(id)} />Select</label><p className="text-xs font-medium uppercase tracking-wide text-[#636a70]">{approvedQuoteIds[id] ? "Approved draft" : "Pending approval"}</p></div><p className="mt-2 font-semibold text-[#171717]">{textOr(client?.name || quote?.client_name || quote?.customer_name, "Unknown client")}</p><p className="text-sm text-[#4b4f53]">Quote: {textOr(quote?.quote_number || quote?.number || quote?.title, "Untitled quote")}</p><p className="text-sm text-[#4b4f53]">Amount: {money(Number(quote?.total ?? quote?.amount ?? quote?.price))}</p><p className="text-sm text-[#4b4f53]">Status: {textOr(quote?.status, "unknown")}</p><p className="text-sm text-[#4b4f53]">Created/Sent: {textOr(displayDate, "Unknown date")}</p><p className="text-sm text-[#4b4f53]">Age: {age ?? "—"} days</p><p className="text-sm text-[#4b4f53]">Contact: {contactEmail || "—"} {contactPhone ? ` / ${contactPhone}` : ""}</p>{missingContact ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Warning: missing client contact details. You can save this draft, but it is not ready to send.</p> : null}{editingQuoteDraft[id] ? <textarea className="mt-3 w-full rounded-lg border border-slate-300 p-2 text-sm" rows={4} value={quoteDrafts[id] || ""} onChange={(e) => setQuoteDrafts((prev) => ({ ...prev, [id]: e.target.value }))} /> : <p className="mt-3 rounded-lg bg-[#ece7dc] p-3 text-sm text-slate-800">{quoteDrafts[id]}</p>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: true }))} className="rounded border border-slate-300 px-3 py-1 text-sm">Edit message</button><button type="button" onClick={() => setEditingQuoteDraft((prev) => ({ ...prev, [id]: false }))} className="rounded border border-slate-300 px-3 py-1 text-sm">Save message</button><button type="button" onClick={() => { setQuoteDrafts((prev) => ({ ...prev, [id]: quoteDraftOriginals[id] || prev[id] })); setEditingQuoteDraft((prev) => ({ ...prev, [id]: false })); }} className="rounded border border-slate-300 px-3 py-1 text-sm">Cancel</button><button type="button" onClick={() => approveOne(quote)} className="rounded bg-[#c65a1e] px-3 py-1 text-sm text-white">Approve follow-up draft</button><button type="button" onClick={() => navigate(`/quotes/${id}`)} className="rounded border border-slate-300 px-3 py-1 text-sm">Open full quote page</button></div></article>;
          })}
        </div>
      );
    }
    if (workspaceDrawer === "AI Dispatch") {
      ...
      return <div className="space-y-4"><div><h3 className="text-lg font-semibold text-[#171717]">AI Dispatch</h3><p className="text-sm text-[#4b4f53]">Review recommended worker assignments before jobs are updated.</p></div>
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Unassigned jobs", dispatchRecs.length],["Crew available", crewAvailable],["Schedule conflicts", conflicts],["Missing job details", missingData]].map(([l,v])=><article key={l} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-3"><p className="text-xs uppercase tracking-wide text-[#636a70]">{l}</p><p className="mt-1 text-xl font-semibold text-[#171717]">{v}</p></article>)}</section>
      {!dispatchRecs.length ? <p className="text-sm text-[#4b4f53]">No unassigned jobs require approval right now.</p> : dispatchRecs.map(({ job, jobId, recommendation, selectedWorkerId }) => {
        ...
        return <article key={jobId} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4"><p className="font-semibold">{textOr(job?.title, "Untitled job")}</p><p className="text-sm text-[#4b4f53]">Client: {textOr(job?.client_name || job?.customer_name, "Unknown")}</p><p className="text-sm text-[#4b4f53]">Address: {textOr(job?.address || job?.location, "No address")}</p><p className="text-sm text-[#4b4f53]">Scheduled: {textOr(job?.scheduled_date || job?.date || job?.scheduled_at, "Unscheduled")}</p><p className="text-sm text-[#4b4f53]">Priority/Status: {textOr(job?.priority, "normal")} / {textOr(job?.status, "new")}</p><p className="mt-2 text-sm text-slate-700">{reasoning}</p>{recommendation?.conflict ? <p className="mt-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-700">Possible schedule conflict: this worker already has another job scheduled that day.</p> : null}
    ...
    return (
      <div className="space-y-3">
        ...
        <p className="text-sm text-[#4b4f53]">Nothing needs attention here.</p>
        <button
          type="button"
          onClick={() => navigate(`/${workspaceDrawer.toLowerCase()}`)}
          className="rounded-lg bg-[#c65a1e] px-4 py-2 text-sm font-medium text-white hover:bg-[#a64917]"
        >
          Open full page
        </button>
      </div>
  ...
  return (
    <Layout title="Smart Hub">
      <div className="min-h-screen tradie-page-bg bg-[#efefec]">
        <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
          <section className="rounded-3xl border border-[#3a3f46] bg-[linear-gradient(145deg,#16181c_0%,#1c1f24_45%,#22262c_100%)] p-6 text-slate-100 shadow-2xl">
            ...
            <div className="mt-4 rounded-xl border border-[#3f454d] bg-[#1a1d22]/90 p-4 shadow-inner">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" onClick={runScanNow} className="rounded-lg bg-amber-600 text-[#171717] px-4 py-2 text-sm font-medium text-white hover:bg-[#c65a1e]">Run today's AI plan</button>
              ...
              <button type="button" onClick={() => openApprovalCentre({ tab: approvalCounts.needs_decision ? "needs_decision" : "all" })} className={`rounded-full px-3 py-2 text-xs font-semibold ${approvalBadgeTone === "amber" ? "bg-[#f0c56a] text-[#3f2a04]" : approvalBadgeTone === "green" ? "bg-[#b8c9b8] text-[#1f3529]" : "bg-[#d7dadc] text-[#3b4248]"}`}>{approvalCounts.all ? `${approvalCounts.all} approvals` : "All clear"}</button>
              <div className="relative">
                ...
                {notificationOpen ? <div className="absolute right-0 z-20 mt-2 w-[340px] rounded-xl border border-[#737a74] bg-white p-3 text-[#171717] shadow-xl">
                  <p className="text-xs font-semibold uppercase tracking-wide text-[#636a70]">Notifications</p>
                  {notificationItems.slice(0, 10).map((item) => <button key={item.id} type="button" onClick={() => { setReadNotificationIds((prev) => ({ ...prev, [item.id]: true })); setNotificationOpen(false); item.action?.(); }} className="mt-2 w-full rounded-lg border border-slate-100 p-2 text-left hover:bg-[#ece7dc]"><p className="text-[11px] font-semibold uppercase text-[#636a70]">{item.section}</p><p className="text-sm font-medium">{item.title}</p><p className="text-xs text-[#4b4f53]">{item.subtitle}</p><p className="text-[11px] text-slate-400">{item.time}</p></button>)}
                </div> : null}
              </div>
            </div>
          </section>
          ...
          <section className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4">
                <p className="text-xs uppercase tracking-wide text-[#636a70]">{label}</p>
          ...
          <section className="mt-6 rounded-2xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-5">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[#4b4f53]">AI Approval Centre</h2>
            <p className="mt-1 text-sm text-[#4b4f53]">AI has prepared today&apos;s admin. Review everything before anything changes.</p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => openApprovalCentre({ tab: "all" })} className={`rounded-full px-3 py-1 text-xs font-medium ${approvalBadgeTone === "amber" ? "bg-amber-100 text-amber-800" : approvalBadgeTone === "green" ? "bg-emerald-100 text-emerald-800" : "bg-[#d7dadc] text-[#3b4248]"}`}>{approvalCounts.all ? `${approvalCounts.all} approvals` : "All clear"}</button>
            ...
            <p className="mt-2 text-sm text-[#4b4f53]">{approvalCounts.needs_decision || 0} need decision · {approvalCounts.ready || 0} ready · {approvalCounts.drafts || 0} drafts · {approvalCounts.watching || 0} watching</p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => { openApprovalCentre({ tab: "all" }); }} className="rounded-lg bg-[#c65a1e] px-3 py-2 text-sm font-medium text-white">Open Approval Centre</button>
          ...
          <section className="mt-6 grid gap-4 lg:grid-cols-2">
            <article className="rounded-2xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-5">
              <h2 className="text-sm font-semibold uppercase tracking-wide text-[#4b4f53]">Today&apos;s Plan</h2>
              <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                {[["Jobs today", jobsToday], ["Unassigned jobs", unassignedJobs.length], ["Ready to bill", readyToBillJobs.length], ["Open invoices", openInvoices.length], ["Quotes waiting", waitingQuotes.length], ["Crew available", crewAvailable]].map(([label, value]) => (
                  <button
                    ...
                    className="rounded-lg border border-[#c8c3b7] bg-[#f3f0e8] px-3 py-2 text-left shadow-sm"
                  >
                    <p className="text-xs uppercase tracking-wide text-[#636a70]">{label}</p>
              ...
              <p className="mt-4 rounded-lg bg-[#ece7dc] px-3 py-2 text-sm text-slate-700">
            ...
            <article className="rounded-2xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-5">
              <h2 className="text-sm font-semibold uppercase tracking-wide text-[#4b4f53]">Business Pulse</h2>
              <div className="mt-3 grid grid-cols-2 gap-3">
                {[["Money waiting", openInvoices.length], ["Billing ready", readyToBillJobs.length], ["Dispatch pressure", unassignedJobs.length], ["Pipeline", waitingQuotes.length], ["Crew", crewAvailable]].map(([label, value]) => (
                  <button
                    ...
                    className="rounded-xl border border-[#b8b2a6] bg-[#f3f0e8] p-3 text-left shadow-sm"
                  >
                    <p className="text-xs uppercase tracking-wide text-[#636a70]">{label}</p>
                    <p className="mt-1 text-xl font-semibold text-[#171717]">{value}</p>
                  </button>
          ...
          <section className="mt-6 rounded-2xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[#4b4f53]">Workspace Dock</h2>
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {workspaceButtons.map((name) => (
                <button
                  ...
                  className="rounded-lg border border-[#3f474f] bg-[linear-gradient(140deg,#2a3138,#1f252b)] px-4 py-3 text-left text-sm font-semibold text-[#e5e7e9] shadow-sm transition hover:border-[#c65a1e] hover:text-white"
                >
                  <span className="block">{name}</span>
                  <span className="block text-xs text-[#9da7ae]">{workspaceMeta[name] || "Open workspace"}</span>
                </button>
              ))}
              <button type="button" onClick={() => openWorkspace("Payment Reminders", "reminders")} className="rounded-lg bg-[#2f3d35] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#243129]">Prepare reminders</button>
              <button type="button" onClick={() => openWorkspace("Quote Follow-ups", "followUps")} className="rounded-lg bg-[#2f3d35] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#243129]">Review follow-ups</button>
              <button type="button" onClick={() => openWorkspace("AI Dispatch", "assign")} className="rounded-lg bg-[#2f3d35] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#243129]">Assign workers</button>
            </div>
          </section>


          <section className="mt-6 rounded-2xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[#4b4f53]">AI Operator Settings</h2>
            <p className="mt-1 text-sm text-[#4b4f53]">Control what AI can prepare, approve, and send.</p>
            ...
            <button type="button" onClick={() => setAiSettingsOpen(true)} className="mt-3 rounded-lg bg-[#c65a1e] px-3 py-2 text-sm text-white">Open AI Settings</button>
          </section>

          <section className="mt-4 rounded-2xl border border-[#a8acae] bg-[#f8f7f4] shadow-[0_10px_24px_-18px_rgba(22,24,28,0.45)] p-4">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-[#4b4f53]">Recent Smart Hub activity</h3>
            <div className="mt-2 flex gap-2">{[["all","All"],["completed","Completed"],["rejected","Rejected"],["draft_prepared","Drafts"]].map(([k,l]) => <button key={k} type="button" onClick={() => setActivityFilter(k)} className={`rounded px-2 py-1 text-xs ${activityFilter===k?"bg-slate-800 text-white":"bg-[#ece7dc] text-slate-700"}`}>{l}</button>)}</div>{!activity.length ? <p className="mt-2 text-sm text-[#636a70]">No AI actions approved yet. Approved work will appear here.</p> : <ul className="mt-3 space-y-2 text-sm text-slate-700">{activity.filter((a)=>activityFilter==="all"?true:String(a?.status||"")===activityFilter).map((a) => <li key={String(a?.id||a?._id)} className="rounded-lg border border-[#737a74] p-2"><p>{a?.message || a?.title}</p><p className="text-xs text-[#636a70]">{textOr(a?.status, "completed")} · {a?.approved_by_name ? `${a.approved_by_name} · ` : ""}{new Date(a?.created_at || Date.now()).toLocaleString()}</p></li>)}</ul>}
          </section>
        </div>

        {workspaceDrawer ? (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#171717]/60 p-0 sm:items-center sm:p-6">
            <div className="h-[86vh] w-full max-w-3xl rounded-t-2xl bg-[#f6f2e8] sm:h-auto sm:rounded-2xl">
        ...
        {aiSettingsOpen ? (
          <div className="fixed inset-0 z-[70] bg-[#171717]/70 p-4">
            <div className="mx-auto mt-10 max-w-xl rounded-2xl bg-white p-4">
              ...
              <div className="mt-4 flex gap-2"><button type="button" className="rounded bg-[#c65a1e] px-3 py-2 text-white" onClick={async ()=>{ try { const res = await patch('/api/ai-operator/settings', aiSettings); setAiSettings((p)=>({...p,...(res?.settings||{})})); setToast({kind:'success',message:'AI settings saved.'}); setAiSettingsOpen(false);} catch { localStorage.setItem("smart_hub_ai_settings_local", JSON.stringify(aiSettings)); setToast({kind:'success',message:'Saved locally until backend setting is added.'}); setAiSettingsOpen(false); } }}>Save</button><button type="button" className="rounded border px-3 py-2" onClick={()=>setAiSettingsOpen(false)}>Close</button></div>
            </div>
          </div>
        ) : null}
              ...
              <div className="flex-none border-b border-[#737a74] bg-white px-5 py-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#4b4f53]">Review, edit or approve everything AI prepared.</p>
                  </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {filteredApprovalItems.map((item) => {
                    ...
                    <article key={item.id} className="rounded-xl border border-[#737a74] bg-[#f6f2e8] p-4 shadow-sm">
                      ...
                      <p className="font-semibold text-[#171717]">{meta.title}</p><p className="mt-1 text-sm text-[#4b4f53]">{meta.subtitle}</p><p className="mt-1 text-sm text-slate-700">{meta.reason}</p><p className="mt-1 text-xs text-[#636a70]">{meta.dataUsed}</p><p className="mt-1 text-xs text-[#636a70]">{meta.whatHappens}</p><p className="mt-1 text-xs text-[#636a70]">Risk: {meta.risk}</p>{meta.contactSummary ? <p className="mt-1 text-xs text-[#636a70]">{meta.contactSummary}</p> : null}
                      {item.type === "invoice_reminder" ? <><div className="mt-2 flex gap-1 text-[11px]"><span className="rounded bg-[#ece7dc] px-2 py-0.5">Email</span><span className="rounded bg-[#ece7dc] px-2 py-0.5">SMS</span></div><p className="mt-2 rounded bg-[#ece7dc] p-2 text-xs text-slate-700">{reminderDrafts[item.relatedId] || buildInvoiceReminderMessage({ client: findByIds(clients, [item.invoice?.client_id, item.invoice?.clientId]), invoice: item.invoice, business: user, channel: "email" })}</p></> : null}
                      {item.type === "quote_follow_up" ? <><div className="mt-2 flex gap-1 text-[11px]"><span className="rounded bg-[#ece7dc] px-2 py-0.5">Email</span><span className="rounded bg-[#ece7dc] px-2 py-0.5">SMS</span></div><p className="mt-2 rounded bg-[#ece7dc] p-2 text-xs text-slate-700">{quoteDrafts[item.relatedId] || buildQuoteFollowUpMessage({ client: findByIds(clients, [item.quote?.client_id, item.quote?.clientId]), quote: item.quote, business: user, channel: "email" })}</p></> : null}
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button type="button" onClick={() => approveApprovalItem(item)} className="rounded-lg bg-[#c65a1e] px-3 py-2 text-sm font-medium text-white">Approve</button>
                        ...
                        <button type="button" onClick={() => rejectApprovalItem(item)} className="rounded-lg border border-[#9f6a56] px-3 py-2 text-sm text-[#7d2f1f]">Reject</button>
                  ...
                  {!filteredApprovalItems.length ? <article className="rounded-xl border border-[#737a74] bg-[#f6f2e8] p-4 shadow-sm"><p className="font-semibold text-[#171717]">No actions in this section.</p><div className="mt-3 flex gap-2"><button type="button" onClick={runScanNow} className="rounded-lg bg-[#c65a1e] px-3 py-2 text-sm text-white">Run today's AI plan</button></div></article> : null}
                </div>
              </div>
            </div>
          </div>
        ...
        {loading ? <p className="mx-auto max-w-6xl px-4 pb-6 text-sm text-[#636a70] sm:px-6 lg:px-8">Loading Smart Hub...</p> : null}
      </div>
    </Layout>
  );
}
