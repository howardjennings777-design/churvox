  ...
  return (
    <main className="opdesk" data-version="CHURVOX_OPERATOR_DESK_V2_20260524">
      ...
      <section className="opdesk-grid">
        <aside className="opdesk-card opdesk-lanes"><p className="opdesk-kicker">Operating lanes</p>{laneRows.map(([key, label, count]) => <button key={key} className={lane === key ? "active" : ""} onClick={() => { setLane(key); setSelected(null); }}><span>{label}</span><strong>{count}</strong></button>)}</aside>
        ...
        <aside className="opdesk-card opdesk-safety"><p className="opdesk-kicker">Owner control</p><strong>{approve.length + fix.length + field.length + moneyItems.length}</strong><span>prepared items</span><ul><li>Messages stay approval-first</li><li>Pricing and payroll stay locked</li><li>Accounting/payment changes need approval</li><li>AI setup is {setup?.ai?.ready ? "ready" : "waiting"}</li></ul></aside>
      </section>
