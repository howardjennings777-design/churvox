...
export default function SafeRecordsWorkbenchPage({ type }) {
  ...
  return <main className="srRoot"><Style /><section className="srWrap"><article className="srHero"><small>{page.title}</small><h1>{page.title} that stay tied to the right business.</h1><p>This page reads through the safe business-record route first, then falls back only if needed.</p></article><section className="srGrid"><section className="srPanel"><small>Working form</small><h2>{selectedId ? `Edit ${page.singular}` : `New ${page.singular}`}</h2><div className="srFields">{page.fields.map((field) => <Field key={field[0]} field={field} form={form} setForm={setForm} />)}</div></section></section></section></main>;
}
