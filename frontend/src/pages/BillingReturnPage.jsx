  ...
  React.useEffect(() => {
    ...
    async function run() {
      ...
      if (addon && sessionId) {
        ...
        const res = await post("/billing/confirm-addon-checkout", { addon, session_id: sessionId });
        if (!alive) return;
        if (res?.success) {
