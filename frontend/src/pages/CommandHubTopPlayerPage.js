...
export default function CommandHubTopPlayerPage() {
  ...
  const workspaceTiles = ["Jobs", "Clients", "Quotes", "Invoices", "Team", "Assign crew", "Proof-to-Paid", "AI Receptionist", "Recurring", "Customer updates", "Quote builder", "Client Memory", "Plans / Billing", "Settings", "Contact", "Notifications", "Integrations", "Privacy", "Terms", "Account Removal"];
  ...
  return <Layout smartHubMode><main className="smart-command-system"><div className="command-real-shell">
    ...
    <section className="command-safe-strip">Auto-send is OFF. AI prepares drafts for review. · No auto-charge · No MYOB write · No payroll changes · No deletion without owner approval.</section>
    ...
    <section className="command-next-grid"><button className="command-next-move orange" onClick={() => setDrawer({ kind: "workspace", title: "Assign crew", subtitle: "Assign and balance today" })}><span>Assign crew the day</span><strong>{groups.dispatch.length}</strong><small>AI worker suggestions</small></button><button className="command-next-move blue" onClick={() => setDrawer({ kind: "workspace", title: "Invoices", subtitle: "Move money safely" })}><span>Move money</span><strong>{groups.revenue.length + groups.follow.length}</strong><small>Draft invoices and reminders</small></button><button className="command-next-move dark" onClick={() => setDrawer({ kind: "workspace", title: "Proof-to-Paid", subtitle: "Prepare proof and updates" })}><span>Proof & updates</span><strong>{groups.proof.length}</strong><small>Proof packs and updates</small></button></section>
