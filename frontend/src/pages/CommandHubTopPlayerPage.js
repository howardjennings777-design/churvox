...
function WorkspaceEditor({ title, data, actions, openJob, execute, askAi, saveSettings }) {
  ...
  if (lower.includes("integration")) return <div className="command-drawer-stack"><div className="command-editor-card"><h3>Integrations workspace</h3><p>Review MYOB, SMS and payment tools. AI will not write to external systems without approval.</p></div><Button onClick={() => askAi("Check integration readiness for MYOB SMS and payments")}>Check integration readiness</Button></div>;
  if (lower.includes("privacy") || lower.includes("terms") || lower.includes("removal")) return <div className="command-drawer-stack"><div className="command-editor-card"><h3>{title}</h3><p>Read and stage account/legal actions safely. AI will never delete or change legal/account status automatically.</p></div><textarea placeholder="Owner notes" /><Button>Save note</Button></div>;
