...
function Drawer({ drawer, close, children }) {
  ...
  return <div className="command-drawer-backdrop" onClick={close}></div>;
}
...
function WorkspaceEditor({ title, data, actions, openJob, execute, askAi, saveSettings }) {
  ...
  if (lower.includes("privacy") || lower.includes("terms") || lower.includes("removal")) return <div className="command-drawer-stack"><div className="command-editor-card"><h3>{title}</h3><p>Read and stage account/legal actions safely. AI checked and will never delete or change legal/account status automatically.</p></div><textarea placeholder="Owner notes" /><Button onClick={() => set("noteSavedAt", new Date().toISOString())}>Save note</Button></div>;
...
export default function TopPlayerPage() {
  const { user } = useAuth();
  ...
  if (!canUse(user?.role)) return <Layout><main className="smart-command-system"><section className="command-panel"><h2>Command is owner/admin only.</h2><p>Workers and payroll users stay in focused workspaces.</p></section></main></Layout>;
  ...
  return <Layout smartHubMode><main className="smart-command-system"><div className="command-real-shell">
    <section className="command-hero"><div className="command-hero-brand"><ChurvoxLogo size="hero" /></div><div className="command-hero-copy"><p className="smart-command-kicker">AI Operator</p><h1>Command</h1><p>AI prepares dispatch, invoices, proof, follow-ups, recurring work, customers, settings and account actions. Owner works and approves inside drawers.</p><div className="command-hero-actions"><Button onClick={runScan}>Run AI scan</Button><Button tone="light" onClick={() => askAi("What should I do first today?")}>Ask AI</Button><Button tone="dark" onClick={() => setDrawer({ kind: "approvals", title: "AI Approval Control", subtitle: "Approve what AI prepared" })}>Open approvals</Button></div></div></section>
    <section className="command-safe-strip">Approval-only mode is ON. AI prepares drafts for review. · No auto-charge · No MYOB write · No payroll changes · No deletion without owner approval.</section>
