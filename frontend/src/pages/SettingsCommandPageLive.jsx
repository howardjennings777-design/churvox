...
export default function SettingsCommandPageLive() {
  ...
  return <main className="slRoot"><Style /><section className="slWrap"><article className="slHero"><small>Settings</small><h1>Set the business up once. Keep it clean.</h1><p>Business profile and branding now save to the backend so invoices, quotes and customer messages can use the same real details.</p></article><section className="slTopGrid"><section className="slPanel"><h2>Business profile</h2><div className="slFields">{FIELDS.map((field) => <Field key={field[0]} field={field} form={form} setForm={setForm} />)}</div></section><aside className="slPanel slControls"><h2>Owner controls</h2><p>{message}</p><button className="slSave" disabled={busy} onClick={saveProfile}>{busy ? "Saving..." : "Save business profile"}</button><button className="slReload" onClick={() => loadProfile()} disabled={busy}>Reload profile</button></aside></section><section className="slXeroSection"><XeroConnectionPanel /></section></section></main>;
}
