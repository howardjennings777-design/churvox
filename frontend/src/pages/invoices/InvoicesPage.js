...
import {
  ...
  AlertTriangle, Link2, Sparkles, Receipt, Zap, ArrowRight, RefreshCw
} from "lucide-react";
...
export default function InvoicesPage() {
  ...
  const runInvoiceAutomation = async (mode = "run") => {
    ...
    if (r.success || r.ok) {
      const data = r.data || r;
      toast.success(data.message || "Invoice check complete");
      await loadInvoices();
    } else {
      toast.error(r.error || r.message || "Invoice check failed");
    }
  ...
  const collectionHealth = missingJobInvoices > 0 ? "Drafts ready" : m.collectableCount === 0 ? "All clear" : m.overdue > 0 ? "Needs chase" : "Ready to collect";
  ...
  return (
    <Layout>
      <PremiumPage>
        <div className="invoice-v5">
          <PremiumHero
            className="invoice-v5-hero"
            icon={<Receipt className="h-7 w-7" />}
            eyebrow={<><Receipt className="h-3 w-3" /> Cashflow</>}
            title="Invoices"
            subtitle="Create, review, send and collect invoices. Background checks can prepare draft work, but the owner stays in control of sending, reminders and paid status."
            actions={
              <>
                {mode === "myob_external" ? (
                ...
                <PremiumButton variant="secondary" onClick={loadInvoices} iconLeft={<RefreshCw className="h-4 w-4" />}>Refresh</PremiumButton>
          ...
          <section className="invoice-command-strip invoice-command-strip--clean">
            <div className="invoice-command-main">
              <span><DollarSign className="h-4 w-4" /> Cashflow status</span>
            ...
            <div className="invoice-command-actions invoice-command-actions--clean">
              <button onClick={() => runInvoiceAutomation("drafts")} disabled={automationBusy}><ReceiptText className="h-4 w-4" /> Prepare drafts</button>
