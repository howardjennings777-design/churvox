// CHURVOX_FIRST_INVOICE_FLOW_STABLE_20260601
...
function readInvoice(payload) {
  ...
  return data?.invoice || data?.item || data?.record || data || {};
}
...
export default function InvoiceFormPage() {
    ...
    async function load() {
      ...
      if (invoiceRes?.success) {
        ...
        setFormData({
          ...
          due_date: String(invoice.due_date || "").slice(0, 10) || todayPlus(7),
          payment_terms: invoice.payment_terms || "Payment due within 7 days.",
          ...
          line_items: Array.isArray(invoice.line_items) && invoice.line_items.length
            ...
            : [{ description: invoice.description || "Service work", quantity: 1, unit_price: invoice.subtotal || invoice.total || invoice.amount || "", amount: invoice.subtotal || invoice.total || invoice.amount || 0 }],
        });
      } else {
        if (clientFromQuery) {
  ...
  }, [api, id, isEdit, jobFromQuery, clientFromQuery]);
  ...
  async function save(event) {
    ...
    const res = isEdit
      ? await api.patch(`/invoices/${encodeURIComponent(id)}`, payload)
  ...
  return (
    <Layout>
      <PremiumPage maxWidth={1080}>
        <button type="button" onClick={() => navigate("/invoices")} className="mb-3 inline-flex items-center gap-2 text-sm font-black text-slate-300 hover:text-white">
          <ArrowLeft size={16} /> Back to invoices
        </button>

        <PremiumHero
          eyebrow={isEdit ? "Edit invoice" : "First invoice"}
          title={isEdit ? "Edit invoice" : "Create an invoice ready to send"}
          subtitle="Invoices now save through the stable invoice endpoint and pull from clients/jobs without calling special missing business-grade routes."
          icon={<Receipt className="h-6 w-6" />}
        />

        {loadingData ? (
          <PremiumCard><div className="p-8 text-center font-bold text-slate-300">Loading invoice workspace…</div></PremiumCard>
        ) : (
          <form onSubmit={save} className="space-y-6" data-testid="stable-invoice-form" data-version="CHURVOX_FIRST_INVOICE_FLOW_STABLE_20260601">
            <PremiumCard title="Customer and linked work">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <label className="space-y-2">
                  <span className={labelClass}>Saved client</span>
                  <select value={formData.client_id} onChange={(e) => applyClient(e.target.value)} className={inputClass}>
                ...
                <label className="space-y-2">
                  ...
                  <select value={formData.job_id} onChange={(e) => applyJobId(e.target.value)} className={inputClass}>
                    <option value="">No linked job</option>
                    {jobs.map((job) => <option key={jobId(job)} value={jobId(job)}>{jobTitle(job)} — {job.status || "open"}</option>)}
                  </select>
                </label>
                ...
                {[
                  ...
                  <label className="space-y-2" key={key}>
                    ...
                    <input value={formData[key] || ""} onChange={(e) => update(key, e.target.value)} className={inputClass} />
                  </label>
                ))}
              </div>
            </PremiumCard>
            ...
            <PremiumCard title="Invoice lines">
              <div className="space-y-3">
                {formData.line_items.map((line, index) => (
                  <div key={index} className="grid grid-cols-12 gap-3 items-end rounded-2xl border border-slate-700 bg-slate-950/50 p-3">
                    <label className="col-span-12 space-y-1 md:col-span-5">
                      <span className="text-xs font-black text-slate-300">Description</span>
                      <input value={line.description} onChange={(e) => updateLine(index, "description", e.target.value)} className={inputClass} />
                    </label>
                    <label className="col-span-4 space-y-1 md:col-span-2">
                      <span className="text-xs font-black text-slate-300">Qty</span>
                      <input type="number" step="0.01" value={line.quantity} onChange={(e) => updateLine(index, "quantity", e.target.value)} className={inputClass} />
                    </label>
                    <label className="col-span-4 space-y-1 md:col-span-2">
                      <span className="text-xs font-black text-slate-300">Unit price</span>
                      <input type="number" step="0.01" value={line.unit_price} onChange={(e) => updateLine(index, "unit_price", e.target.value)} className={inputClass} />
                    </label>
                    <label className="col-span-4 space-y-1 md:col-span-2">
                      <span className="text-xs font-black text-slate-300">Line total</span>
                      <input type="number" step="0.01" value={line.amount} onChange={(e) => updateLine(index, "amount", e.target.value)} className={inputClass} />
                    </label>
                ...
                <button type="button" onClick={addLine} className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-black text-slate-950"><Plus size={16} /> Add line item</button>
              </div>
            </PremiumCard>
            ...
            <PremiumCard title="Terms, tax and payment">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                {[
                  ...
                  <label className="space-y-2" key={key}>
                    <span className={labelClass}>{label}</span>
                    <input type={type} value={formData[key] || ""} onChange={(e) => update(key, e.target.value)} className={inputClass} />
                  </label>
                ))}
                <label className="space-y-2 md:col-span-3">
                  <span className={labelClass}>Customer description / public notes</span>
                  <textarea value={formData.description} onChange={(e) => update("description", e.target.value)} rows={3} className={inputClass} />
                </label>
                <label className="space-y-2 md:col-span-3">
                  <span className={labelClass}>Internal notes</span>
                  <textarea value={formData.internal_notes} onChange={(e) => update("internal_notes", e.target.value)} rows={2} className={inputClass} />
                </label>
              </div>
            </PremiumCard>
            ...
            <PremiumCard title="Invoice preview">
              <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-5">
                <div><span className="block text-slate-300">Subtotal</span><b className="text-white">{money(subtotal)}</b></div>
                <div><span className="block text-slate-300">Discount</span><b className="text-white">{money(discount)}</b></div>
                <div><span className="block text-slate-300">GST</span><b className="text-white">{money(gstAmount)}</b></div>
                <div><span className="block text-slate-300">Total</span><b className="text-cyan-300">{money(total)}</b></div>
                <div><span className="block text-slate-300">Amount due</span><b className="text-lime-300">{money(due)}</b></div>
              </div>
            </PremiumCard>

            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => navigate("/invoices")} className="rounded-full border border-slate-600 px-5 py-3 font-black text-slate-100">Cancel</button>
