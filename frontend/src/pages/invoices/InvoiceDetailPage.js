// CHURVOX_INVOICE_DETAIL_STABLE_ACTIONS_20260601
...
export default function InvoiceDetailPage() {
  ...
  async function patchInvoice(label, payload, message = "Invoice updated") {
    setBusy(label);
    const res = await api.patch(`/invoices/${encodeURIComponent(id)}`, { ...payload, updated_at: new Date().toISOString() });
    setBusy("");
    if (res.success) {
  ...
  async function markPaid() {
    ...
    await patchInvoice("paid", { status: "paid", amount_paid: total, amount_due: 0, balance_due: 0, payment_history: [...(invoice?.payment_history || []), payment] }, "Invoice marked paid");
  }
    ...
    <section className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
      <PremiumCard>
        <article className="rounded-3xl bg-white p-6 text-[#0d1b34] shadow-sm" data-testid="business-grade-invoice-preview">
          ...
          <section className="grid grid-cols-1 gap-4 py-5 md:grid-cols-2"><div className="rounded-2xl bg-[#f6faff] p-4"><p className="text-xs font-black uppercase text-[#5b6c87]">Bill to</p><h3 className="mt-1 font-black">{invoice.customer_name}</h3><p className="text-sm text-[#5b6c87]">{invoice.customer_email}</p><p className="text-sm text-[#5b6c87]">{invoice.customer_phone}</p><p className="text-sm text-[#5b6c87]">{invoice.billing_address || invoice.address}</p></div><div className="rounded-2xl bg-[#f6faff] p-4"><p className="text-xs font-black uppercase text-[#5b6c87]">Job / payment</p><p className="text-sm text-[#0d1b34]">{invoice.site_address || invoice.address || "No site address saved"}</p><p className="mt-2 text-sm text-[#5b6c87]">{invoice.payment_terms || "Payment terms not set"}</p>{invoice.job_id || invoice.linked_job_id ? <Link className="mt-2 inline-block text-sm font-black text-[#2563eb]" to={`/jobs/${invoice.job_id || invoice.linked_job_id}`}>Open linked job</Link> : null}</div></section>
