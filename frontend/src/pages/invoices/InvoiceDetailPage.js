...
export default function InvoiceDetailPage() {
  ...
  return <Layout><PremiumPage maxWidth={1080}>
    ...
    <section className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <aside className="space-y-4"><PremiumCard title="Owner actions"><div className="grid gap-3">{["draft", "sent", "overdue"].includes(status) ? <PremiumButton onClick={sendInvoice} disabled={busy === "send"}><Send size={16} className="mr-2" /> Open email + mark sent</PremiumButton> : null}{!['paid', 'cancelled'].includes(status) ? <PremiumButton variant="success" onClick={markPaid} disabled={busy === "paid"}><CreditCard size={16} className="mr-2" /> Mark paid</PremiumButton> : null}{publicLink ? <a href={publicLink} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-700 px-4 py-3 text-sm font-black text-slate-100"><Eye size={16} /> View public invoice</a> : null}{publicLink ? <button type="button" onClick={copyPublicLink} className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-700 px-4 py-3 text-sm font-black text-slate-100"><Copy size={16} /> Copy public link</button> : null}{!['paid', 'cancelled'].includes(status) ? <button type="button" onClick={cancelInvoice} className="inline-flex items-center justify-center gap-2 rounded-full bg-red-500/10 px-4 py-3 text-sm font-black text-red-200"><Trash2 size={16} /> Cancel invoice</button> : null}</div></PremiumCard><PremiumCard title="Linked records"><div className="grid gap-2 text-sm font-bold">{jobId ? <Link className="text-cyan-300" to={`/jobs/${jobId}`}>Open linked job</Link> : <span className="text-slate-400">No linked job</span>}{quoteId ? <Link className="text-cyan-300" to={`/quotes/${quoteId}`}>Open linked quote</Link> : <span className="text-slate-400">No linked quote</span>}</div></PremiumCard></aside>
    </section>
  </PremiumPage></Layout>;
}
