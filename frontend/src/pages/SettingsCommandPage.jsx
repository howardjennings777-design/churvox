...
const sections = [
  { id: "profile", title: "Business profile", badge: "Launch basics", color: "#fb923c", href: "/settings-board", copy: "Business name, contact details, address, GST settings and launch basics.", details: [["Purpose", "Keep the business identity and customer-facing details correct."], ["Check", "Business name, email, phone, address, GST and invoice details."], ["Rule", "Only show fields here when there is a real save form wired."]] },
  { id: "branding", title: "Branding", badge: "Keep consistent", color: "#22d3ee", href: "/settings-board", copy: "Logo, colours, customer-facing wording and invoice/quote presentation.", details: [["Purpose", "Keep customer-facing quotes, invoices and screens looking consistent."], ["Check", "Logo, colour, business wording and document presentation."], ["Rule", "No random old themes or mismatched branding."]] },
  { id: "accounting", title: "Xero accounting", badge: "Approval first", color: "#34d399", href: "/support-board", copy: "Xero is the visible accounting direction for launch. Sync remains staged and approval-first.", details: [["Purpose", "Prepare invoice/payment sync direction without overpromising."], ["Check", "Accounting wording, support path and approval-first sync copy."], ["Rule", "No automatic accounting changes without owner approval."]] },
  ...
  { id: "team", title: "Team access", badge: "Role safe", color: "#a78bfa", href: "/team-board", copy: "Roles, invites, worker access and payroll permissions.", details: [["Purpose", "Keep staff access safe and simple."], ["Check", "Owner, Manager, Office Admin, Worker and Payroll roles."], ["Rule", "Payroll stays locked down. Workers do not see pricing or GPS evidence."]] },
  { id: "legal", title: "Legal and support", badge: "Live links", color: "#f43f5e", href: "/support-board", copy: "Privacy, terms, account deletion, help and setup support.", details: [["Purpose", "Make support and legal pages easy to find."], ["Check", "Privacy, terms, account deletion and help links."], ["Rule", "Legal/support links must be tappable on mobile."]] },
];
...
function SettingsSlip({ section, reviewed, onClose, onReview }) {
    ...
    <div className="fixed inset-0 z-[2147483600] overflow-y-auto bg-slate-950/92 p-3 text-white backdrop-blur-xl md:p-6" role="dialog" aria-modal="true">
      <div className="mx-auto flex min-h-[calc(100vh-24px)] max-w-6xl flex-col overflow-hidden rounded-[34px] border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 shadow-2xl md:min-h-[calc(100vh-48px)]">
        ...
        <div className="grid flex-1 gap-5 p-5 md:grid-cols-[1.15fr_.85fr] md:p-7">
          ...
          <aside className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200">Owner action</div><p className="mt-3 text-sm font-bold leading-6 text-slate-300">Review the setting area here. Open the related page only when there is a real save, billing, support, or full-editor action.</p>{reviewed ? <div className="mt-4 rounded-3xl border border-emerald-300/25 bg-emerald-300/10 p-4 text-sm font-black text-emerald-100">Reviewed. This settings area has been checked.</div> : null}<div className="mt-5 grid gap-3"><button type="button" onClick={onReview} className="rounded-2xl bg-emerald-300 px-5 py-4 text-sm font-black text-slate-950">Mark setting reviewed</button><Link to={section.href} onClick={onClose} className="rounded-2xl bg-white px-5 py-4 text-center text-sm font-black text-slate-950 no-underline">Open related page</Link><button type="button" onClick={onClose} className="rounded-2xl bg-white/10 px-5 py-4 text-sm font-black text-white ring-1 ring-white/10">Back to settings</button></div></aside>
        </div>
      </div>
    </div>
  );
...
export default function SettingsCommandPage() {
  ...
  const [reviewed, setReviewed] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem("churvox_settings_reviewed") || "{}"); } catch { return {}; }
  });
  const selectedId = selectedSection?.id || "current";

  function markReviewed(section) {
    const next = { ...reviewed, [section.id]: true };
    setReviewed(next);
    localStorage.setItem("churvox_settings_reviewed", JSON.stringify(next));
    toast.success("Setting marked reviewed");
  }

  const reviewedCount = sections.filter((section) => reviewed[section.id]).length;
    ...
    <main className={industrialPageShell} data-industrial-simple-page="settings" data-command-canvas>
      <section className={`${industrialContentLane} space-y-5`}>
        <DarkCard className="p-6 pl-9 md:p-8 md:pl-10">
          ...
          <div className="mt-6 flex flex-wrap gap-3"><Link to="/plans" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialAction}`}>View plans</Link><Link to="/support-board" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Get support</Link><Link to="/dashboard" className={`rounded-2xl px-5 py-3 text-sm font-black ${industrialGhost}`}>Command Board</Link></div>
        </DarkCard>

        <section className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(340px,.9fr)]">
          <section className="grid gap-4 md:grid-cols-2">
            {sections.map((section) => <DarkCard key={section.id} color={section.color} className="min-h-[180px]"><div className="flex items-start justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-amber-300">{section.badge}</div><h2 className="mt-2 text-2xl font-black tracking-[-.05em] text-white">{section.title}</h2></div>{reviewed[section.id] ? <span className="rounded-full bg-emerald-300 px-3 py-1 text-[10px] font-black uppercase tracking-[.14em] text-slate-950">Reviewed</span> : null}</div><p className="mt-3 text-sm font-bold leading-6 text-slate-300">{section.copy}</p><button type="button" onClick={() => setSelectedSection(section)} className="mt-4 inline-flex rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950">Review slip</button></DarkCard>)}
          </section>
          ...
          <aside className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,.07)] md:p-6">
            ...
            <p className="mt-2 text-sm font-bold leading-6 text-slate-600">{reviewedCount}/{sections.length} settings areas reviewed.</p>
        ...
        <section className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,.07)] md:p-6"><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-700">Accounting direction</div><h2 className="mt-2 text-3xl font-black tracking-[-.06em]">Xero visible. Approval-first sync.</h2><p className="mt-3 max-w-3xl text-sm font-bold leading-7 text-slate-600">For launch polish, Churvox should talk about Xero where accounting is visible. Accounting sync should stay approval-first: Churvox prepares the admin, the owner approves it.</p><div className="mt-5 flex flex-wrap gap-3"><Link to="/support-board" className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white no-underline">Ask about Xero setup</Link><Link to="/invoices-board" className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-950 no-underline">Review invoices</Link></div></section>
      </section>
      <SettingsSlip section={selectedSection} reviewed={Boolean(reviewed[selectedId])} onClose={() => setSelectedSection(null)} onReview={() => selectedSection && markReviewed(selectedSection)} />
    </main>
  );
}
