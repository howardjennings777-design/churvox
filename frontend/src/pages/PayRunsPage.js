...
export default function PayRunsPage() {
  ...
  const stats = [
    { label: "Active run", value: activePeriod?.name || "None", help: activePeriod ? `${activePeriod.start_date} → ${activePeriod.end_date}` : "Create a run from the full setup flow", icon: CalendarRange },
    { label: "Approved hours", value: Number(summary?.approved_hours || 0).toFixed(2), help: "Ready for export", icon: Clock3 },
    { label: "Pending review", value: Number(summary?.pending_review_count || pendingTimesheets.length), help: "Needs approval first", icon: FileClock },
    { label: "Workers included", value: Number(summary?.workers_included || workerSummaries.length), help: "In this run", icon: UsersRound },
  ];

  const downloadCsv = async (path, filename, label) => {
    if (!activePeriodId) return toast.error("Select a pay run first");
  ...
  const bulkApprove = async () => withAction("bulk-approve", async () => {
  ...
  const unlockRun = async () => withAction("unlock-run", async () => {
    ...
    if (!res?.success) return toast.error(res?.error || "Could not unlock run");
  ...
  const markExported = async () => withAction("mark-exported", async () => {
    ...
    toast.success("Run marked exported");
    await loadInitial();
  });
    ...
    <Layout>
      <div className="cx-page space-y-6">
        <section className="rounded-3xl border border-border bg-gradient-to-br from-white via-[#f6f9ff] to-[#eef6ff] p-6 shadow-[0_10px_30px_rgba(16,24,40,0.08)]">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-700">Payroll prep + export</p>
          <h1 className="cx-page-title mt-2">Pay Runs & Timesheets</h1>
          <p className="cx-page-subtitle max-w-4xl">
            Prepare approved hours, review worker summaries, lock the run, and export payroll-ready CSV files for MYOB, Xero Payroll, PayHero, iPayroll, PaySauce, Smartly, your accountant, or your bookkeeper.
          </p>
          <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-medium text-blue-800">
            Churvox prepares the run and export files. Final PAYE, KiwiSaver, leave, payslips, payday filing, and payments stay in your payroll provider or accountant workflow.
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 gap-6 xl:grid-cols-[0.9fr_1.4fr]">
          <div className="rounded-2xl border border-border bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-bold text-slate-950">Runs</h2>
              <span className={statusBadge(activePeriod?.status)}>{activePeriod?.status || "open"}</span>
            </div>

            <div className="mt-3 space-y-2">
              {loading ? <p className="text-sm text-slate-500">Loading runs...</p> : null}
              {!loading && !periods.length ? (
                <div className="rounded-xl border border-dashed border-border bg-slate-50 p-4 text-sm text-slate-500">
                  No pay runs found yet. Create one from the existing setup flow, then come back here to approve and export.
                </div>
              ) : null}
              {periods.map((period) => (
                <button key={period.id} type="button" onClick={() => setActivePeriodId(period.id)} className={`w-full rounded-xl border p-3 text-left ${activePeriodId === period.id ? "border-blue-500 bg-blue-50" : "border-border bg-slate-50"}`}>
                  ...
                  <p className="text-xs text-slate-500">{period.start_date} → {period.end_date} · Pay date {period.pay_date}</p>
                </button>
              ))}
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {readOnly ? (
                <button className="cx-button-secondary" disabled={!activePeriodId || actionLoading["unlock-run"]} onClick={unlockRun}><Unlock size={14} className="mr-2" />Unlock</button>
              ) : (
                <button className="cx-button-secondary" disabled={!activePeriodId || actionLoading["lock-run"]} onClick={lockRun}><Lock size={14} className="mr-2" />Lock run</button>
              )}
          ...
          <div className="rounded-2xl border border-border bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-bold text-slate-950">Timesheet approval</h2>
                <p className="text-sm text-slate-500">Only approved hours should be included in your export.</p>
              </div>
              <button className="cx-button-secondary" disabled={!activePeriodId || readOnly || !pendingTimesheets.length || actionLoading["bulk-approve"]} onClick={bulkApprove}>Approve all pending</button>
            </div>

            <div className="mt-3 space-y-2">
              {!timesheets.length ? (
                <div className="rounded-xl border border-dashed border-border bg-slate-50 p-4 text-sm text-slate-500">No timesheets found for this run.</div>
              ) : null}
        ...
        <section className="rounded-2xl border border-border bg-white p-4 shadow-sm">
          ...
          <p className="text-sm text-slate-500">Download clean CSV files for your payroll provider, accountant, or bookkeeper.</p>
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            <button className="cx-button-secondary justify-center" disabled={!activePeriodId} onClick={() => downloadCsv(`/payroll/periods/${activePeriodId}/export/payroll-summary.csv`, `churvox-pay-run-summary-${payRunPart}.csv`, "Pay run summary")}><Download size={14} className="mr-2" />Pay run summary CSV</button>
            <button className="cx-button-secondary justify-center" disabled={!activePeriodId} onClick={() => downloadCsv(`/payroll/periods/${activePeriodId}/export/timesheets.csv`, `churvox-timesheets-${payRunPart}.csv`, "Timesheet detail")}><Download size={14} className="mr-2" />Timesheet detail CSV</button>
            <button className="cx-button-secondary justify-center" disabled={!activePeriodId} onClick={() => downloadCsv(`/payroll/periods/${activePeriodId}/export/worker-pay.csv`, `churvox-worker-pay-${payRunPart}.csv`, "Worker pay summary")}><Download size={14} className="mr-2" />Worker pay summary CSV</button>
          </div>
        </section>

        <section className="rounded-2xl border border-border bg-white p-4 shadow-sm">
          <h2 className="text-lg font-bold text-slate-950">Worker summaries</h2>
            ...
            {workerSummaries.map((worker) => (
              <div key={worker.worker_id || worker.name} className="rounded-xl border border-border bg-slate-50 p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-600">
                  ...
                  <p>Estimate: <b>{formatCurrency(worker.gross_pay || 0)}</b></p>
                </div>
              </div>
            ))}
