...
const workspaces = [
  ...
  ["Assign crew", "/dispatch?embedded=1", "♙", "#1165ff", "#edf4ff"],
          ...
          <section style={s.twoCol}>
            ...
            <article style={s.card} onClick={() => openPanel({ key: "approvalControl" })}>
              ...
              <div style={s.approvalGrid}>
                ...
                <Approval label="Assign crew" value={String(stats.needCrew || 0)} icon="▣" onClick={(event) => { event.stopPropagation(); openPanel({ key: "approvalTile", label: "Assign crew", value: stats.needCrew || 0 }); }} />
...
function ControlRoomPanel({ panel, draft, setDraft, onClose, onSave, updateDraft, navigate, stats, data, notice, setNotice, rows }) {
  ...
  const content = {
    ...
    team: <><p style={s.modalLine}>Unassigned jobs: {unassigned.length}</p>{list(data.workers,(w)=><div key={w.id||w._id} style={s.panelRow}><strong>{w.name||w.full_name||"Worker"}</strong><span>{w.status||"active"}</span></div>)}<label style={s.modalLabel}>Assign crew instruction</label><textarea style={s.modalTextarea} value={draft.dispatchInstruction||""} onChange={(e)=>updateDraft("dispatchInstruction",e.target.value)} /><label style={s.modalLabel}>Assignment suggestion</label><textarea style={s.modalTextarea} value={draft.assignmentSuggestion||""} onChange={(e)=>updateDraft("assignmentSuggestion",e.target.value)} /></>,
    dispatch: <><p style={s.modalLine}>Worker availability summary: {stats.workers} active</p>{list(unassigned,(j)=><div key={j.id||j._id} style={s.panelRow}><strong>{j.title||j.service}</strong><span>{j.client_name||j.client}</span></div>)}<label style={s.modalLabel}>Assign crew plan</label><textarea style={s.modalTextarea} value={draft.dispatchPlan||""} onChange={(e)=>updateDraft("dispatchPlan",e.target.value)} /><label style={s.modalLabel}>Priority</label><select style={s.modalInput} value={draft.priority||"Normal"} onChange={(e)=>updateDraft("priority",e.target.value)}><option>Low</option><option>Normal</option><option>High</option></select></>,
