...
const supportCards = [
  ["Launch setup path", "Add business details, create a client, create a job, create an invoice, then invite crew.", "/settings-board"],
  ["Jobs and crew", "Use Jobs and Crew Dispatch for work that needs assigning, starting, finishing or invoicing.", "/jobs-board"],
  ["Invoices and quotes", "Use approval-first slips before sending invoices, quote follow-ups or customer messages.", "/invoices-board"],
  ["Xero direction", "Xero is the visible accounting direction for launch polish.", "/settings-board"],
  ...
  ["Privacy Policy", "Open the privacy page.", "/privacy-policy"],
  ["Terms", "Open the terms page.", "/terms-of-service"],
...
function HelpSlip({ item, reviewed, onClose, onReview }) {
  if (!item) return null;
  return (
...
export default function SupportCommandPage() {
  ...
  const [reviewed, setReviewed] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem("churvox_support_reviewed") || "{}"); } catch { return {}; }
  });
  ...
  const submit = async (event) => {
    ...
    setSending(true);
    const payload = { type, message: message.trim(), page: "/support-board", source: "support_board", created_at: new Date().toISOString() };
    let res = null;
    try {
  ...
  function markReviewed(item) {
    const next = { ...reviewed, [item.id]: true };
    setReviewed(next);
    localStorage.setItem("churvox_support_reviewed", JSON.stringify(next));
    toast.success("Support path marked reviewed");
  }
  ...
  return (
    <main className={industrialPageShell} data-industrial-simple-page="support" data-command-canvas>
      <section className={`${industrialContentLane} space-y-5`}>
        ...
        <section className="grid gap-5 xl:grid-cols-[minmax(0,.95fr)_minmax(360px,1.05fr)]">
          ...
          <section className="grid gap-4 md:grid-cols-2">
            {quickHelp.map((item) => <DarkCard key={item.id} color={item.color} className="min-h-[150px]"><div className="flex items-start justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-amber-300">Help area</div><h3 className="mt-2 text-xl font-black tracking-[-.04em] text-white">{item.title}</h3></div>{reviewed[item.id] ? <span className="rounded-full bg-emerald-300 px-3 py-1 text-[10px] font-black uppercase tracking-[.14em] text-slate-950">Reviewed</span> : null}</div><p className="mt-2 text-sm font-bold leading-6 text-slate-300">{item.copy}</p><button type="button" onClick={() => setSelectedHelp(item)} className="mt-4 inline-flex rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950">Review slip</button></DarkCard>)}
          </section>
        </section>
      ...
      <HelpSlip item={selectedHelp} reviewed={Boolean(reviewed[selectedId])} onClose={() => setSelectedHelp(null)} onReview={() => selectedHelp && markReviewed(selectedHelp)} />
    </main>
  );
}
