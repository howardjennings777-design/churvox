...
export default function WorkerSafeSettingsPage() {
  ...
  const activeNav = location.hash === "#help" ? "messages" : "profile";

  return <div id="top" className="px-app min-h-screen pb-28" data-marker="CHURVOX_WORKER_SAFE_SETTINGS_20260608">
    ...
    <main className="mx-auto max-w-2xl space-y-4 px-4 py-5">
      <section className="px-hero" style={{ padding: "20px" }}><span className="px-hero__eyebrow"><Settings className="h-3 w-3" /> Worker profile</span><h1 className="px-hero__title" style={{ fontSize: "24px" }}>Your field app.</h1><p className="px-hero__sub">Assigned jobs, messages, help, and basic account info. Owner pricing, billing, payroll admin and accounting controls stay hidden.</p></section>
      ...
      <Card><p className="text-xs font-black uppercase tracking-[.16em] text-[var(--cx-accent)]">Quick actions</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><Link to="/worker/jobs" className="rounded-2xl bg-[var(--cx-accent)] px-4 py-4 text-center text-sm font-black text-slate-950 no-underline"><Briefcase className="mx-auto mb-1 h-5 w-5" /> Open my jobs</Link><button type="button" onClick={() => setContactOpen(true)} className="rounded-2xl border border-[var(--cx-border)] bg-[var(--cx-surface-2)] px-4 py-4 text-sm font-black text-[var(--cx-text)]"><MessageCircle className="mx-auto mb-1 h-5 w-5" /> Message boss</button><button type="button" onClick={() => window.location.reload()} className="rounded-2xl border border-[var(--cx-border)] bg-[var(--cx-surface-2)] px-4 py-4 text-sm font-black text-[var(--cx-text)]"><RefreshCw className="mx-auto mb-1 h-5 w-5" /> Refresh app</button><button type="button" onClick={logout} className="rounded-2xl border border-red-300/30 bg-red-500/10 px-4 py-4 text-sm font-black text-red-200"><LogOut className="mx-auto mb-1 h-5 w-5" /> Log out</button></div></Card>

      <Card id="worker-help"><p className="text-xs font-black uppercase tracking-[.16em] text-[var(--cx-accent)]">Messages and help</p><div className="mt-3 grid gap-3"><InfoRow icon={Briefcase} label="1. Open job" value="Only work assigned to you should appear in your list." /><InfoRow icon={CheckCircle2} label="2. Status" value="Use On my way, Started, Paused, Need help, and Complete from the job screen." /><InfoRow icon={Camera} label="3. Notes and photos" value="Add clear worker notes and completion photos for owner review." /><InfoRow icon={CheckCircle2} label="4. Complete" value="Finish the job so time, checklist, materials and proof are ready for the owner." /></div></Card>

      <Card><div className="flex items-start gap-3"><HelpCircle className="mt-1 h-5 w-5 text-[var(--cx-accent)]" /><div><h2 className="text-lg font-black text-[var(--cx-text)]">Need help?</h2><p className="mt-1 text-sm font-bold leading-6 text-[var(--cx-muted)]">Message the boss for job access, instructions, schedule changes, or app issues.</p><button type="button" onClick={() => setContactOpen(true)} className="mt-3 rounded-2xl bg-[var(--cx-accent)] px-4 py-3 text-sm font-black text-slate-950">Message boss</button></div></div></Card>
    </main>
    ...
    <WorkerBottomNav active={activeNav} />
  </div>;
}
