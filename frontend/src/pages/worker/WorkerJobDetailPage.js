  ...
  return (
    <div className="px-app min-h-screen pb-28">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        ...
        <PremiumCard><div className="px-card__body space-y-3"><p className="text-sm font-semibold text-[var(--cx-text)]">Notes</p>{job.notes ? <p className="text-sm text-[var(--cx-muted)] whitespace-pre-wrap">{job.notes}</p> : <p className="text-sm text-[var(--cx-muted-2)]">No job notes yet.</p>}<textarea value={workerNotes} onChange={(e) => setWorkerNotes(e.target.value)} rows={4} placeholder="Add your worker notes..." className="px-input" /><PremiumButton onClick={() => handleSaveNotes()} disabled={savingNotes}>{savingNotes ? "Saving..." : "Save worker notes"}</PremiumButton></div></PremiumCard>
        ...
        <PremiumCard><div className="px-card__body space-y-2"><p className="text-sm font-semibold text-[var(--cx-text)]">Completion</p><textarea className="px-input" rows={3} value={finalNote} onChange={(e) => setFinalNote(e.target.value)} placeholder="Final completion note..." /><p className="text-xs text-[var(--cx-muted)]">Reminder: add at least one final photo where possible before completion.</p><PremiumButton className="w-full" onClick={async () => { await handleSaveNotes(finalNote); await handleStatus("completed"); }} disabled={saving || savingNotes || status === "completed"}>Complete job now</PremiumButton></div></PremiumCard>
