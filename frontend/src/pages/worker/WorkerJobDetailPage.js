...
const statusOf = (job) => String(job?.status || "").toLowerCase().replaceAll(" ", "_");
...
export default function WorkerJobDetailPage() {
  ...
  const checkAnotherActiveJob = async () => {
    ...
    const list = Array.isArray(res?.data) ? res.data : Array.isArray(res?.data?.jobs) ? res.data.jobs : [];
    return list.find((item) => activeStatuses.has(statusOf(item)) && jobIdOf(item) !== String(id));
  };
  ...
  const handleStatus = async (status) => {
    ...
    try {
      ...
      const isColdStart = status === "in_progress" && currentStatus !== "paused";
      if (isColdStart) {
        const otherActive = await checkAnotherActiveJob();
        if (otherActive) {
          toast.error(`Finish or pause your active job first: ${otherActive?.title || "current job"}`);
          setSaving(false);
          return;
        }
      }

      const needsLocation = ["in_progress", "completed"].includes(status);
      const geo = needsLocation ? await getGeo() : null;
      if (!geo && needsLocation) toast.warning("Location unavailable. Time was still saved.");

      const payload = { status };
      if (status === "in_progress" && geo) {
        payload.start_lat = geo.latitude;
        payload.start_lng = geo.longitude;
        payload.start_accuracy_meters = geo.accuracy;
      }
      if (status === "in_progress" && !geo) payload.location_status = "location_denied";
      if (status === "completed" && geo) {
        payload.end_lat = geo.latitude;
        payload.end_lng = geo.longitude;
        payload.end_accuracy_meters = geo.accuracy;
      }
      if (status === "completed" && !geo) payload.location_status = "location_denied";
      ...
      if (res?.success) {
        if (status === "in_progress") toast.success(currentStatus === "paused" ? "Job resumed" : "Job started — Crew Map is active");
        else if (status === "paused") toast.success("Job paused — pause time will be deducted");
        else if (status === "completed") toast.success("Job finished — time added to timesheet review");
        else toast.success(`Job ${status.replace(/_/g, " ")}`);
        await loadJob();
      } else {
  ...
  return (
    <div className="px-app min-h-screen pb-28" data-marker="CHURVOX_ACTIVE_JOB_TIMESHEET_LOGIC_20260601">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        ...
        <PremiumCard><div className="px-card__body space-y-2"><p className="text-sm font-semibold text-[var(--cx-text)]">Active job & timesheet logic</p><div className="grid grid-cols-2 gap-2 text-xs text-[var(--cx-muted)]"><div className="rounded-xl border border-[var(--cx-border)] p-2">Crew Map: <b className="text-[var(--cx-text)]">{activeStatuses.has(normalizedStatus) ? "Showing" : "Hidden"}</b></div><div className="rounded-xl border border-[var(--cx-border)] p-2">Timesheet: <b className="text-[var(--cx-text)]">{timeLabel}</b></div></div><p className="text-xs text-[var(--cx-muted)]">Start shows you on Crew Map. Pause deducts unpaid paused time. Finish closes the map session and sends the time to timesheet review.</p></div></PremiumCard>
        ...
        <PremiumCard><div className="px-card__body space-y-2"><p className="text-sm font-semibold text-[var(--cx-text)]">Time & work controls</p><div className="grid grid-cols-2 gap-2"><PremiumButton onClick={handleAcknowledge} disabled={saving || status !== "assigned"} iconLeft={<Hand className="h-4 w-4" />}>Acknowledge</PremiumButton><PremiumButton onClick={() => handleStatus("in_progress")} disabled={saving || (normalizedStatus !== "assigned" && normalizedStatus !== "acknowledged" && normalizedStatus !== "paused")} iconLeft={<Play className="h-4 w-4" />}>{normalizedStatus === "paused" ? "Resume" : "Start"}</PremiumButton><PremiumButton variant="secondary" onClick={() => handleStatus("paused")} disabled={saving || normalizedStatus !== "in_progress"} iconLeft={<Pause className="h-4 w-4" />}>Pause</PremiumButton><PremiumButton variant="secondary" onClick={() => handleStatus("in_progress")} disabled={saving || normalizedStatus !== "paused"} iconLeft={<RotateCcw className="h-4 w-4" />}>Resume</PremiumButton><div className="col-span-2"><PremiumButton className="w-full" onClick={() => handleStatus("completed")} disabled={saving || (normalizedStatus === "completed" && !sentBack)} iconLeft={<CheckCircle className="h-4 w-4" />}>{sentBack ? "Send back to owner review" : "Finish job & save time"}</PremiumButton></div></div></div></PremiumCard>
