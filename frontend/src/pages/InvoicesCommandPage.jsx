...
export default function InvoicesCommandPage() {
  ...
  const load = React.useCallback(async () => { setLoading(true); try { const res = await get("/invoices"); setInvoices(listFrom(res)); } catch (error) { console.warn("Invoices page load failed", error); setInvoices([]); } finally { setLoading(false); } }, [get]);
