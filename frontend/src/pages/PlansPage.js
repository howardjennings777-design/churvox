...
export default function PlansPage() {
    ...
    const handleCheckoutReturn = async () => {
      ...
      if (checkout === "success") {
        try {
          ...
          if (smsPack) {
            setNotice({ type: "success", title: "SMS credits purchased", text: "Your SMS credit checkout completed. Refresh if the balance does not update straight away." });
          } else if (addon === "command_growth_pack") {
            setNotice({ type: "success", title: "Growth Pack added", text: "Your Command Growth Pack checkout completed." });
          } else if (addon === "myob_addon") {
            setNotice({ type: "success", title: "MYOB add-on added", text: "Your MYOB add-on checkout completed." });
          } else {
      ...
      if (checkout === "cancelled") {
        setNotice({ type: "warning", title: "Checkout cancelled", text: "No changes were made." });
      }
  ...
  const openCheckout = async (payload) => {
    const res = await api.post("/billing/unified-checkout", {
  ...
  const handleSelectPlan = async (planKey) => {
    ...
    try {
      ...
      await openCheckout({
        checkout_type: "plan",
        plan_type: planKey,
        success_path: `/plans?checkout=success&plan=${encodeURIComponent(planKey)}`,
        cancel_path: `/plans?checkout=cancelled&plan=${encodeURIComponent(planKey)}`,
      });
    } catch (err) {
  ...
  const handleBuyMyobAddon = async () => {
    if (busyPlan || busyAddon || busySms) return;
    if (isNewUser) {
      toast.error("Choose a Churvox plan before adding MYOB.");
      return;
    }

    if (currentPlan === "enterprise") {
      toast.success("MYOB is already included in Command.");
      return;
    }

    try {
      setBusyAddon("myob_addon");
      await openCheckout({
        checkout_type: "myob_addon",
        addon_type: "myob_addon",
        quantity: 1,
        success_path: "/plans?checkout=success&addon=myob_addon",
        cancel_path: "/plans?checkout=cancelled&addon=myob_addon",
      });
    } catch (err) {
      toast.error(err?.response?.data?.detail || err?.data?.detail || err?.message || "Failed to open MYOB checkout");
    } finally {
      setBusyAddon("");
    }
  };
  ...
  const handleBuySmsPack = async (pack) => {
    ...
    if (isNewUser) {
      toast.error("Choose a Churvox plan before buying SMS credits.");
      return;
    }
    ...
    try {
      ...
      await openCheckout({
        ...
        sms_pack: pack.key,
        quantity: 1,
        success_path: `/plans?checkout=success&sms_pack=${encodeURIComponent(pack.key)}`,
        cancel_path: `/plans?checkout=cancelled&sms_pack=${encodeURIComponent(pack.key)}`,
      });
    } catch (err) {
      toast.error(err?.response?.data?.detail || err?.data?.detail || err?.message || "Failed to open SMS checkout");
    } finally {
      setBusySms("");
    }
  };
  ...
  if (loading) {
    return (
      <main className="cv-plans">
        <div className="cv-plans-shell">
          <section className="cv-plans-hero"><p>Loading plans…</p></section>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="cv-plans" data-version="CHURVOX_PLANS_SMS_BLOCKS_CHECKOUT_20260601" data-audit-markers={CHURVOX_AUDIT_MARKERS}>
      <div className="cv-plans-shell">
        ...
        <section className="cv-user-blocks">
          <div>
            <small>Command Growth Pack</small>
            <b>+50 active team members</b>
            <span>Add more crew, jobs and AI Operator capacity as your business grows. Built for Command customers who need more capacity without changing the whole plan.</span>
          </div>
          <article>
            <small>Growth Pack</small>
            <strong>$99<em> /month + GST</em></strong>
            <p>Each block adds 50 more active team members.</p>
            <button className="cv-user-block-buy" type="button" onClick={handleBuyGrowthPack} disabled={Boolean(busyPlan || busyAddon || busySms)} data-testid="buy-command-growth-pack">
              {busyAddon === "command_growth_pack" ? "Opening checkout…" : "Buy Growth Pack"}
            </button>
          </article>
          <ul>{userBlocks.map((item) => <li key={item}>{item}</li>)}</ul>
        </section>

        <section className="cv-myob-addon">
          <div>
            <small>MYOB add-on</small>
            <b>MYOB sync for Operator</b>
            <span>Operator can add MYOB for $39/month + GST. Command includes MYOB by default.</span>
          </div>
          <button type="button" onClick={handleBuyMyobAddon} disabled={Boolean(busyPlan || busyAddon || busySms || currentPlan === "enterprise")} data-testid="buy-myob-addon">
            {currentPlan === "enterprise" ? "Included in Command" : busyAddon === "myob_addon" ? "Opening checkout…" : "Add MYOB — $39/month"}
          </button>
        </section>

        <section className="cv-sms-pricing">
          <div>
            <b>SMS credit blocks</b>
            <span>SMS is separate so you only buy what you use. Customer reminders and follow-ups stay approval-first.</span>
          </div>
          <div className="cv-sms-grid">
            {smsBlocks.map((pack) => (
