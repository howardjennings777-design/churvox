...
export default function PlansPage() {
    ...
    const handleCheckoutReturn = async () => {
      ...
      if (checkout === "success") {
        try {
          ...
          else if (addon === "myob_addon" || addon === "xero_addon" || addon === "accounting_sync_addon") setNotice({ type: "success", title: "Accounting Sync Add-on added", text: "Your accounting sync add-on checkout completed." });
          else {
  ...
  const handleBuyMyobAddon = async () => {
    ...
    try { setBusyAddon("xero_addon"); await openCheckout({ checkout_type: "xero_addon", addon_type: "xero_addon", quantity: 1, success_path: "/plans?checkout=success&addon=xero_addon", cancel_path: "/plans?checkout=cancelled&addon=xero_addon" }); }
