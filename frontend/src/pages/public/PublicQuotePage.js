  ...
  return (
    <main className="cpd-shell" data-version="CHURVOX_PUBLIC_DOCUMENT_TOTAL_FALLBACK_HARDENING_20260529 CHURVOX_PUBLIC_DOCUMENT_LINE_ITEMS_TOTALS_20260529">
      ...
      <article className="cpd-document">
        <header className="cpd-head"><div><small>Quote</small><h1>{quoteNumber}</h1><p>{customer}</p></div><aside className="cpd-status"><small>Status</small><b>{status}</b><p>{validUntil ? `Valid until ${String(validUntil).split("T")[0]}` : "Prepared through Churvox."}</p></aside></header>
