...
export default function PublicInvoicePage() {
  ...
  const rows = useMemo(() => {
    ...
    if (!invoice) return [];
    return [{ description: first(invoice.description, invoice.invoice_description, invoice.notes, "Service work completed."), quantity: 1, rate: moneyNumber(first(invoice.subtotal, invoice.amount, invoice.price, invoice.total)) }];
  }, [invoice]);
  ...
  const subtotal = moneyNumber(first(invoice.subtotal, rows.reduce((sum, row) => sum + lineAmount(row), 0)));
  const gstRate = moneyNumber(first(invoice.gst_rate, invoice.tax_rate, 15));
  const gstAmount = moneyNumber(first(invoice.gst_amount, invoice.tax_amount, subtotal * (gstRate / 100)));
  const total = moneyNumber(first(invoice.total, invoice.amount, subtotal + gstAmount));
  ...
  return (
    <main className="cpd-shell" data-version="CHURVOX_PUBLIC_INVOICE_TEMPLATE_20260528 CHURVOX_PUBLIC_DOCUMENT_IMPORT_SAFETY_20260528 CHURVOX_PUBLIC_DOCUMENT_LINE_ITEMS_TOTALS_20260529">
      ...
      <article className="cpd-document">
        <header className="cpd-head">
          <div><small>Invoice</small><h1>{invoiceNumber}</h1><p>{customer}</p></div>
        ...
        <section className="cpd-body">
          <div className="cpd-grid">
            <div className="cpd-card"><small>Bill to</small><h2>{customer}</h2><p>{invoice.email || invoice.customer_email || invoice.address || "Customer details saved by the business."}</p></div>
            <div className="cpd-card"><small>Description</small><h2>Work completed</h2><p>{description}</p></div>
          </div>

          <table className="cpd-line-table">
            <thead><tr><th>Item</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead>
            <tbody>{rows.map((line, index) => <tr key={index}><td>{lineDescription(line, description)}</td><td>{lineQty(line)}</td><td>{formatCurrency(lineRate(line))}</td><td>{formatCurrency(lineAmount(line))}</td></tr>)}</tbody>
          </table>

          <div className="cpd-total-card">
            <small>Total due</small>
            <div className="cpd-total-lines"><span>Subtotal</span><b>{formatCurrency(subtotal)}</b><span>GST ({gstRate}%)</span><b>{formatCurrency(gstAmount)}</b></div>
            <h2>{formatCurrency(total)}</h2>
            <p>{paymentLink ? "Use Pay now to complete payment securely." : paymentDetails || "Payment details are provided by the business."}</p>
            {paymentDetails ? <p className="cpd-payment-note">{paymentDetails}</p> : null}
            {publicNotes && publicNotes !== description ? <p className="cpd-payment-note">{publicNotes}</p> : null}
