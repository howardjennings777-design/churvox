...
function mount() {
  ...
  const html = rows.length ? rows.map((x) => `<div class="row"><span><b>${esc(x.title || x.recordId || x.type || 'Activity')}</b>${esc(x.detail || x.status || '')}<br>${esc(x.at || x.created_at || '')}</span><em>${esc(String(x.type || 'activity').replace(/-/g,' '))}</em></div>`).join('') : '<div class="row"><span><b>No timeline yet</b>Save or approve a record and Churvox will show the history here.</span><em>empty</em></div>';
  panel.innerHTML = `<h3>Record timeline</h3><p>History for autofill, saved records, prepared Command items and owner decisions.</p><div class="rows">${html}</div>`;
}
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && !window.__CHURVOX_OWNER_TIMELINE_PANEL__) {
  ...
  document.addEventListener('click', () => setTimeout(mount, 600), true);
