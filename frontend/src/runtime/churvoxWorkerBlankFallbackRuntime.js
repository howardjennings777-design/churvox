...
function liveWorkerOk() {
  ...
  return /Worker|Today|Jobs|Messages|Help|Start current job|All jobs done|Message office/i.test(value) && value.length > 12;
}
...
function ensureStyle() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    html.churvox-worker-fallback-active body{background:#f5f7f1!important}
    #${FALLBACK_ID}{min-height:100dvh;background:#f5f7f1;color:#111815;font-family:Inter,system-ui,sans-serif;padding:18px 18px 92px;box-sizing:border-box;display:grid!important;gap:14px;align-content:start;visibility:visible!important;opacity:1!important}
    #${FALLBACK_ID},#${FALLBACK_ID} *{visibility:visible!important;-webkit-text-fill-color:initial!important}
    ...
    #${FALLBACK_ID} .swHero span{display:inline-flex;border-radius:999px;background:#ea580c;padding:6px 10px;font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.08em;color:#fff}
    #${FALLBACK_ID} h1{margin:10px 0 4px;font-size:36px;line-height:1;font-weight:1000;letter-spacing:-.05em;color:#fff}
    #${FALLBACK_ID} p{margin:0;color:#52605a;font-size:14px;font-weight:850;line-height:1.45}
    #${FALLBACK_ID} .swHero p{color:rgba(255,255,255,.82)}
    ...
    #${FALLBACK_ID} .swPrimary{display:inline-flex;justify-content:center;align-items:center;text-decoration:none;border:0;border-radius:999px;background:#ea580c;color:#fff!important;padding:12px 14px;font-weight:950}
    ...
    #${FALLBACK_ID} .swNav a{text-align:center;text-decoration:none;border-radius:12px;color:#fff!important;padding:10px 4px;font-size:12px;font-weight:950}
...
function body(tab) {
  if (tab === 'Messages') return '<section class="swCard"><span>Messages</span><h2>Message office</h2><p>Send office updates. Sent messages stay visible.</p><a class="swPrimary" href="/worker/help">Need help</a></section>';
  if (tab === 'Help') return '<section class="swCard"><span>Help</span><h2>Message office</h2><p>Wrong address, customer issue, unsafe work, need more info, or other.</p><a class="swPrimary" href="/worker/messages">Message office</a></section>';
  if (tab === 'Me') return '<section class="swCard"><span>Me</span><h2>Worker profile</h2><p>Today, Jobs, Messages, Help and Me stay simple.</p></section>';
  if (tab === 'Jobs') return '<section class="swCard"><span>Jobs</span><h2>One job at a time</h2><p>Start current job, finish job, send to office, or all jobs done.</p><a class="swPrimary" href="/worker/today">Back to today</a></section>';
...
function renderFallback(force = false) {
  if (!isWorkerRoute()) {
    document.getElementById(FALLBACK_ID)?.remove();
    document.documentElement.classList.remove('churvox-worker-fallback-active');
    return;
  }
  if (!force && liveWorkerOk()) {
    document.getElementById(FALLBACK_ID)?.remove();
    document.documentElement.classList.remove('churvox-worker-fallback-active');
    return;
  }
  ...
  ensureStyle();
  document.documentElement.classList.add('churvox-worker-fallback-active');
  let node = current;
  if (!node) {
  ...
  node.innerHTML = `<section class="swHero"><span>${tab}</span><h1>Worker</h1><p>Today, jobs and messages stay clear.</p></section>${body(tab)}<nav class="swNav">${nav(tab)}</nav>`;
}

function schedule() {
  if (!isWorkerRoute()) return;
  renderFallback(false);
  setTimeout(() => renderFallback(false), 120);
  setTimeout(() => renderFallback(false), 400);
  setTimeout(() => renderFallback(false), 900);
  setTimeout(() => renderFallback(false), 1800);
}
