...
function valuesFor(page, item) {
  if (page === 'clients') return {
    name: item.name || item.client_name || '', phone: item.phone || item.mobile || '', email: item.email || '', address: item.address || item.service_address || '', defaultPrice: item.default_price || item.price || '', serviceMemory: item.notes || item.serviceMemory || '', access: item.access || item.access_notes || '',
  };
  if (page === 'jobs') return {
    client: item.client || item.client_name || '', site: item.address || item.site || '', worker: item.worker || item.assigned_worker || '', date: item.date || item.scheduled_date || '', time: item.time || item.start_time || '', status: item.status || 'Draft', price: item.price || item.amount || '', repeat: item.repeat || item.recurring || 'None', scope: item.scope || item.description || item.notes || '', proof: item.proof || item.proof_status || '',
  };
  if (page === 'quotes') return {
    client: item.client || item.client_name || '', quoteNo: item.quote_number || item.quoteNo || '', status: item.status || 'Draft', validUntil: item.valid_until || '', total: item.total || item.amount || '', scope: item.scope || item.description || '', followUp: item.follow_up || '',
  };
  if (page === 'invoices') return {
    client: item.client || item.client_name || '', invoiceNo: item.invoice_number || item.invoiceNo || '', job: item.job || item.job_title || '', status: item.status || 'Draft', amount: item.amount || item.total || '', due: item.due || item.due_date || '', sync: item.sync || 'Draft sync only', paidRule: item.paidRule || 'Only mark paid after accounting refresh confirms paid', lineItems: item.line_items || item.description || '',
  };
  if (page === 'workers' || page === 'team') return {
    worker: item.name || item.worker_name || item.email || '', currentJob: item.current_job || '', status: item.status || item.invite_status || '', gps: item.gps || item.location || '', timer: item.timer || '', proof: item.proof || '', message: item.message || item.notes || '', role: item.role || '', email: item.email || '',
  };
  return item || {};
}
...
async function hydrateSource(source, records) {
  ...
  const rows = pickArray(data).slice(0, 80);
  let count = 0;
  rows.forEach((item) => {
    ...
    const record = { id, page: source.page, title: source.title(item), values: valuesFor(source.page, item), source: 'backend-hydration', backendId: rawId, updatedAt: new Date().toISOString() };
...
async function run() {
  ...
  try {
    ...
    for (const source of SOURCES) {
      try { total += await hydrateSource(source, records); } catch (_) {}
    }
    if (total) {
      write(RECORD_KEY, records);
      dispatchEvent(new CustomEvent('churvox:owner-backend-hydrated', { detail: { total } }));
    }
  } finally {
    busy = false;
  }
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && !window.__CHURVOX_OWNER_BACKEND_HYDRATION__) {
  ...
  addEventListener('hashchange', () => setTimeout(run, 2000));
  addEventListener('churvox:owner-record-api-synced', () => setTimeout(run, 2200));
