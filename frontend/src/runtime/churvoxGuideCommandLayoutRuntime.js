...
// Stable launch version: no 1s remount loop, Command-only decisions preserved.
...
function openSlip(title) {
  ...
  const rows = [['Page', page === 'command' ? 'Command approval desk' : 'AI Guide / Smart Hub'], ['Item', title], ['Prepared status', 'Ready for owner review'], ['Owner decision', page === 'command' ? 'Approve, edit, park or send back' : 'Review in Command when owner decision is needed'], ['Owner rule', 'Churvox does the admin. You approve.']].map(([k,v]) => `<label>${esc(k)}${/decision|rule/i.test(k) ? `<textarea rows="3">${esc(v)}</textarea>` : `<input value="${esc(v)}" />`}</label>`).join('');
  modal.innerHTML = `<section class="box"><header><div><small>${esc(page)}</small><h2>${esc(title)}</h2><p>Review the prepared Churvox item before deciding.</p></div><button type="button" class="close" data-gc-close>×</button></header><div class="body"><div class="grid">${rows}</div><div class="actions"><button type="button" data-gc-close>Keep</button><button type="button" data-gc-command>Review in Command</button><button type="button" data-gc-close>Park</button><button type="button" data-gc-close>Close</button></div></div></section>`;
...
function mount() {
  ...
  if (!root || !page) { old?.remove(); document.getElementById('churvox-owner-page-recovery')?.style.removeProperty('display'); lastMountSig = ''; return; }
  installStyle();
  document.getElementById('churvox-owner-page-recovery')?.style.setProperty('display', 'none', 'important');
  let node = old;
  if (!node) { node = document.createElement('section'); node.id = LAYOUT_ID; root.prepend(node); lastMountSig = ''; }
  const html = page === 'command' ? commandLayout() : guideLayout();
  const sig = `${page}:${html}`;
  if (sig === lastMountSig && node.dataset.page === page) return;
  lastMountSig = sig;
...
function clickHandler(event) {
  ...
  if (route) { event.preventDefault(); const next = `/dashboard#${route.dataset.gcRoute}`; if (`${location.pathname}${location.hash}` !== next) { history.replaceState({}, document.title, next); dispatchEvent(new HashChangeEvent('hashchange')); } return; }
  ...
  if (modal?.classList.contains('open')) {
    ...
    if (event.target.closest('[data-gc-command]')) { modal.classList.remove('open'); const next = '/dashboard#command'; if (`${location.pathname}${location.hash}` !== next) { history.replaceState({}, document.title, next); dispatchEvent(new HashChangeEvent('hashchange')); } }
  }
}
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && !window.__CHURVOX_GUIDE_COMMAND_LAYOUT__) {
  ...
  window.addEventListener('popstate', () => setTimeout(mount, 120));
  window.addEventListener('churvox:fresh-data-updated', () => setTimeout(mount, 300));
