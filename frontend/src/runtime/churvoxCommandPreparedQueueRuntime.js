...
function writeItems(items) {
  try { localStorage.setItem(STORE_KEY, JSON.stringify(items.slice(0, 20))); } catch (_) {}
}
...
function prepareItem(title, sourcePage) {
  const item = {
    ...
    title: title || 'Prepared item',
    sourcePage: sourcePage || pageKey(),
    status: 'Waiting owner approval',
    note: 'Churvox prepared this for owner review. You approve.',
...
function installStyle() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    #${PANEL_ID}{display:grid;gap:10px;border:1px solid rgba(239,85,60,.18);border-radius:17px;background:linear-gradient(135deg,#fff,#fff7f0);box-shadow:0 14px 30px rgba(16,21,19,.06);padding:14px;color:#111815}
    #${PANEL_ID} header{display:flex;justify-content:space-between;gap:12px;align-items:center}
    #${PANEL_ID} h3{margin:0;color:#111815;font-size:17px;font-weight:950}
    #${PANEL_ID} small{color:#64706b;font-size:11px;font-weight:900}
    #${PANEL_ID} .items{display:grid;gap:8px}
    #${PANEL_ID} .item{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center;border:1px solid rgba(16,21,19,.08);border-radius:13px;background:#fff;padding:10px}
    #${PANEL_ID} b{display:block;color:#111815;font-size:13px;font-weight:950}
    #${PANEL_ID} span{display:block;color:#52605a;font-size:11px;font-weight:850;line-height:1.35}
    #${PANEL_ID} .actions{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}
    #${PANEL_ID} button{border:0;border-radius:999px;min-height:30px;padding:6px 10px;background:#111815;color:#fff;font-size:10px;font-weight:950}
    #${PANEL_ID} button:nth-child(2){background:#ef553c}
    #${PANEL_ID} button:nth-child(3){background:#e4e7e7;color:#111815}
  `;
  document.head.appendChild(style);
}

function mountPanel() {
  if (pageKey() !== 'command') { document.getElementById(PANEL_ID)?.remove(); return; }
  ...
  if (!panel) { panel = document.createElement('section'); panel.id = PANEL_ID; host.prepend(panel); }
  ...
  panel.innerHTML = `<header><div><h3>Prepared for Command</h3><small>Churvox prepares it here. You approve.</small></div><button type="button" data-command-clear>Clear done</button></header><div class="items">${rows}</div>`;
}
...
function commandActions(event) {
  ...
  if (event.target.closest('[data-command-clear]')) { writeItems(readItems().filter((x) => !/approved|parked/i.test(x.status || ''))); mountPanel(); return; }
  if (!row) return;
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && !window.__CHURVOX_COMMAND_PREPARED_QUEUE__) {
  ...
  window.addEventListener('churvox:command-prepared', () => setTimeout(run, 80));
  setInterval(run, 900);
  run();
}
