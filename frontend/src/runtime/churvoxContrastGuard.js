(function churvoxContrastGuard() {
  ...
  if (window.__CHURVOX_CONTRAST_GUARD_V5__) return;
  window.__CHURVOX_CONTRAST_GUARD_V5__ = true;

  const TEXT_SELECTOR = 'h1,h2,h3,h4,h5,h6,p,span,small,label,strong,em,li,td,th,button,a,b';
  ...
  function colorsFromBackgroundImage(value) {
    ...
    const out = [];
    (text.match(/rgba?\([^)]+\)/gi) || []).forEach((item) => { const rgb = parseRgb(item); if (rgb) out.push(rgb); });
    (text.match(/#[0-9a-f]{3,6}\b/gi) || []).forEach((item) => { const rgb = parseHex(item); if (rgb) out.push(rgb); });
    return out;
  }
  ...
  function effectiveBackground(el) {
    ...
    while (node && node !== document.documentElement) {
      ...
      if (imageColors.length) {
        ...
        const lightest = imageColors.slice().sort((a, b) => luminance(b) - luminance(a))[0];
        if (luminance(lightest) > 0.68) return lightest;
      }
  ...
  function skip(el) {
    ...
    if (el.closest('.freshCommandPreparedPage')) return true;
    const style = window.getComputedStyle(el);
    return style.display === 'none' || style.visibility === 'hidden';
  }

  function computedTextColor(el) {
    const style = window.getComputedStyle(el);
    const fill = style.webkitTextFillColor && style.webkitTextFillColor !== 'currentcolor' ? style.webkitTextFillColor : style.color;
    return parseRgb(fill) || parseRgb(style.color) || { r: 17, g: 24, b: 39, a: 1 };
  }

  function fixText(el) {
    if (skip(el) || !hasText(el)) return;
    const style = window.getComputedStyle(el);
    const fg = computedTextColor(el);
    const bg = effectiveBackground(el);
    const ratio = contrastRatio(fg, bg);
    const opacity = Number.parseFloat(style.opacity || '1');
    const bgIsLight = luminance(bg) > 0.52;
    if (ratio < 4.5 || opacity < 0.75) forceColor(el, bgIsLight ? '#111827' : '#f9fafb');
  }

  function forceForm(el) {
    if (!el || !el.style || skip(el)) return;
    const bg = effectiveBackground(el);
    const bgIsLight = luminance(bg) > 0.52;
    if (el.closest('.cvPublicAuth,.wh-auth')) {
      el.style.setProperty('background', '#ffffff', 'important');
      el.style.setProperty('background-color', '#ffffff', 'important');
      forceColor(el, '#000000');
      el.style.setProperty('caret-color', '#000000', 'important');
      return;
    }
    forceColor(el, bgIsLight ? '#111827' : '#f9fafb');
    el.style.setProperty('caret-color', bgIsLight ? '#111827' : '#f9fafb', 'important');
  }
  ...
  function run() {
    ...
    document.querySelectorAll(FORM_SELECTOR).forEach(forceForm);
    forceKnownShell();
  }
  ...
  ['pushState', 'replaceState'].forEach((name) => {
    ...
    window.history[name] = function patchedHistoryMethod() { const result = original.apply(this, arguments); setTimeout(schedule, 80); return result; };
  });
