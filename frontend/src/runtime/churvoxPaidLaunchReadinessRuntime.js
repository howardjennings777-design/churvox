...
function mount() {
  ...
  if (!root || !['aiguide','command','plans','settings'].includes(page)) return;
  installStyle();
  let panel = document.getElementById(PANEL_ID);
  if (!panel) { panel = document.createElement('section'); panel.id = PANEL_ID; root.appendChild(panel); lastHtml = ''; }
  const rows = checks().map((x) => `<div class="row"><span><b>${esc(x.name)}</b>${esc(x.note)}</span><em class="${x.ok ? '' : 'no'}">${x.ok ? 'ready' : 'check'}</em></div>`).join('');
