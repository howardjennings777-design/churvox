...
import { Bot, Briefcase, CheckCircle2, ChevronRight, ClipboardCheck, DollarSign, FileText, Hammer, Home, LogOut, MapPinned, MessageSquare, Plug, RefreshCw, Settings, ShieldCheck, Sparkles, Timer, UserPlus, Users, Wand2, X, Zap } from "lucide-react";
import { useAuth } from "../../context/AuthContext";
import { get } from "../../lib/api";
import { approveAiAction, loadAiOperatorQueue, prepareTodayWithAi, runAiDailyCheck } from "../../lib/aiOperator";
import V3Brand from "../../v3/components/V3Brand";
...
const nav = [
  ["smart-hub", "Smart Hub", Home, "AI command centre"],
  ["operator", "AI Operator", Bot, "Prepared admin"],
  ["decisions", "Decisions", Sparkles, "Approve actions"],
  ["jobs", "Jobs", Briefcase, "Run sheet"],
  ["dispatch", "Dispatch", MapPinned, "Crew match"],
  ["clients", "Clients", Users, "Customer base"],
  ...
  ["rules", "Auto Rules", Zap, "Safe automation"],
  ["reports", "Reports", ClipboardCheck, "Insights"],
  ["messages", "Messages", MessageSquare, "AI drafts"],
  ["integrations", "Sync", Plug, "MYOB etc"],
  ["settings", "Settings", Settings, "Business setup"],
];
...
function Status({ value }) { const s = lower(value); const cls = s.includes("overdue") || s.includes("cancel") ? "danger" : s.includes("complete") || s.includes("paid") || s.includes("accept") ? "good" : s.includes("progress") || s.includes("sent") ? "blue" : "neutral"; return <span className={`v4-status ${cls}`}>{value || "Open"}</span>; }
...
export default function V4AppPage({ initialSection = "smart-hub" }) {
  ...
  const openQuotes = state.quotes.filter((q) => ["draft", "sent", "open", "pending", ""].includes(lower(statusOf(q))));
  ...
  const openSection = (key) => navigate(key === "smart-hub" ? "/v4" : `/v4/${key}`);
  ...
  const SmartHub = () => <>
    <section className="v4-hero"><div className="v4-hero-main"><span className="v4-kicker"><Bot size={15}/> Churvox AI Operator</span><h1>Run the trade business from one clean cockpit.</h1><p>Churvox watches jobs, crew, proof, quotes, invoices and messages. It prepares real admin, explains why, then waits for owner approval.</p><div className="v4-hero-actions"><button className="v4-btn primary" onClick={() => runAi("prepare")} disabled={aiRunning}><Wand2 size={18}/> {aiRunning ? "Preparing…" : "Prepare next moves"}</button><button className="v4-btn dark" onClick={() => runAi("check")} disabled={aiRunning}><RefreshCw size={18}/> Full business check</button><button className="v4-btn light" onClick={() => openSection("decisions")}>Review decisions</button></div></div><aside className="v4-next-card"><small>Next best move</small><b>{pending.length ? "Approve the first prepared action." : unassigned.length ? "Assign unassigned work." : proofNeeded.length ? "Check proof-to-paid." : "Run the AI check."}</b><span>Tap cards for detail first. Full workspace only when needed.</span></aside></section>
    <section className="v4-metric-grid">{[["Owner decisions", pending.length, "Approve AI work", Sparkles, "decisions"], ["Unassigned jobs", unassigned.length, "AI crew match", MapPinned, "dispatch"], ["Proof-to-paid", proofNeeded.length, "Ready to invoice", CheckCircle2, "jobs"], ["Money moves", money.length, "Invoices and follow-ups", DollarSign, "invoices"]].map(([label, value, copy, Icon, target]) => <button className="v4-metric" key={label} onClick={() => openSection(target)}><Icon size={22}/><span>{label}</span><b>{loading ? "—" : value}</b><small>{copy}</small></button>)}</section>
    <section className="v4-board two"><article className="v4-panel"><div className="v4-panel-head"><div><small>Owner approval queue</small><h2>AI prepared actions</h2></div><strong>{pending.length}</strong></div>{prepared.length ? prepared.map((item) => <button className="v4-decision" key={actionId(item.raw)} onClick={() => setSelected(item)}><span><Sparkles size={18}/></span><div><b>{item.title}</b><small>{item.copy}</small></div><ChevronRight size={18}/></button>) : <Empty title="No decisions waiting" copy="Run the AI check to prepare assignments, invoice drafts and follow-ups."/>}</article><article className="v4-panel"><div className="v4-panel-head"><div><small>Operator workflow</small><h2>Not a dashboard. An operator.</h2></div><Bot size={25}/></div><div className="v4-flow-grid">{[[MapPinned,"Crew Match","Recommend the worker by area, workload, availability and fit.","dispatch"],[CheckCircle2,"Proof-to-Paid","Completed jobs checked for photos, time and invoice readiness.","jobs"],[DollarSign,"Money Board","Draft, chase and clear invoice actions from one place.","invoices"],[Zap,"Auto Rules","Safe admin runs automatically. Risky work stays approval-first.","rules"]].map(([Icon,title,copy,target]) => <button className="v4-flow" key={title} onClick={() => openSection(target)}><Icon size={20}/><b>{title}</b><span>{copy}</span></button>)}</div></article></section>
  </>;
  ...
  const Records = ({ kind, title, kicker, copy, items, type }) => <section className="v4-panel full"><div className="v4-section-title"><small>{kicker}</small><h1>{title}</h1><p>{copy}</p></div><div className="v4-record-grid">{items.length ? items.slice(0, 24).map((item) => <RecordCard key={item.id || item._id || item.email || item.invoice_number || item.quote_number || titleOf(item)} item={item} type={type} onOpen={setSelected}/>) : <Empty title={`No ${title.toLowerCase()} loaded`} copy="When live data is available it will show here."/>}</div><button className="v4-btn secondary" onClick={() => navigate(`/v3/${kind}`)}>Open current full {title} workspace</button></section>;
  ...
  const Simple = () => { const data = { operator:[Bot,"AI Operator","Prepare work the owner can approve","Run checks, find admin gaps and prepare actions."], payroll:[Timer,"Payroll","Approved time and pay run review","Payroll stays locked down and approval-first."], rules:[Zap,"Auto Rules","Safe automation for repeat admin","Safe patterns can run. Risky changes need approval."], reports:[ClipboardCheck,"Reports","Owner insight without clutter","Track jobs, money, proof and workload."], messages:[MessageSquare,"Messages","AI drafted customer communication","Draft reminders and follow-ups before sending."], integrations:[Plug,"Sync","MYOB and external connections","Clear sync for invoices, payments and contacts."], settings:[Settings,"Settings","Business setup and preferences","Trade type, roles, plan and AI approval behaviour."] }[section] || [ShieldCheck,"Workspace","Churvox workspace","This area uses the same V4 system."]; const [Icon,k,t,c]=data; return <section className="v4-panel full v4-placeholder"><Icon size={42}/><small>{k}</small><h1>{t}</h1><p>{c}</p><div><button className="v4-btn primary" onClick={() => runAi("prepare")} disabled={aiRunning}><Wand2 size={18}/> Prepare actions</button><button className="v4-btn secondary" onClick={() => navigate(`/v3/${section}`)}>Open current full workspace</button></div></section>; };

  const content = section === "smart-hub" ? <SmartHub/> : section === "decisions" ? <Decisions/> : section === "dispatch" ? <Dispatch/> : section === "jobs" ? <Records kind="jobs" title="Jobs" kicker="AI Run Sheet" copy="All work with proof, worker and billing readiness." items={state.jobs} type="job"/> : section === "clients" ? <Records kind="clients" title="Clients" kicker="Customers" copy="Clean customer base for jobs, quotes and invoices." items={state.clients} type="client"/> : section === "quotes" ? <Records kind="quotes" title="Quotes" kicker="Quote Desk" copy="Draft, sent and follow-up quotes." items={state.quotes} type="quote"/> : section === "invoices" ? <Records kind="invoices" title="Invoices" kicker="Money Board" copy="Draft, unpaid, overdue and paid invoices." items={state.invoices} type="invoice"/> : section === "team" ? <Records kind="team" title="Team" kicker="Crew" copy="Workers, roles, workload and dispatch readiness." items={state.workers} type="worker"/> : <Simple/>;

  return <div className="v4-app"><aside className="v4-sidebar"><button className="v4-brand" onClick={() => openSection("smart-hub")}><V3Brand/></button><nav>{visibleNav.map(([key,label,Icon,hint]) => <button key={key} className={section === key ? "active" : ""} onClick={() => openSection(key)}><Icon size={18}/><span><b>{label}</b><small>{hint}</small></span></button>)}</nav></aside><main className="v4-main"><header className="v4-topbar"><div><span>Churvox V4</span><b>{nav.find(([k]) => k === section)?.[1] || "Smart Hub"}</b></div><div className="v4-top-actions">{notice ? <em>{notice}</em> : null}<button onClick={load}><RefreshCw size={16}/> Refresh</button><button onClick={signOut}><LogOut size={16}/> Log out</button><div className="v4-user"><strong>{initials(user)}</strong><span>{user?.business_name || user?.email || "Churvox"}</span></div></div></header>{content}</main><nav className="v4-mobile-nav">{["smart-hub","decisions","jobs","dispatch","invoices"].map((key) => { const found = nav.find(([k]) => k === key); const Icon = found?.[2] || Home; return <button key={key} className={section === key ? "active" : ""} onClick={() => openSection(key)}><Icon size={18}/><span>{found?.[1]}</span></button>; })}</nav><DetailModal item={selected} busy={!!busyActionId} onClose={() => setSelected(null)} onApprove={approve} onOpenFull={(path) => { setSelected(null); navigate(path); }}/></div>;
}
