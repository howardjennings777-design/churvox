  ...
  return (
    <section className="freshXeroPage">
      <div className="freshXeroCompactHeader">
        <div><span>Accounting connection</span><h1>Xero</h1><p>Connect Xero, create draft invoices, check payment status, and download accounting exports.</p></div>
      ...
      <div className="freshXeroLayout">
        <aside className="freshXeroList">
          <header><div><b>Accounting flow</b><span>Invoice - payment check - Xero draft</span></div><button type="button" onClick={loadStatus} disabled={loading || busy}>Reload</button></header>
          ...
          <button type="button" className="active" onClick={() => downloadAccountingPack("both")}><b>5. Export pack</b><span>Xero CSV, MYOB CSV, clients, jobs, and notes</span><small>Download</small></button>
        </aside>

        <article className="freshXeroDetail">
          <div className="freshXeroHead"><div><span>{connected ? "Connected" : "Setup required"}</span><h2>{connected ? connection?.tenant_name || "Xero connected" : "Connect Xero"}</h2><p>{connected ? "Sync creates a Xero draft invoice only. Send or reconcile inside Xero after checking it." : "Start the Xero OAuth connection when setup is ready."}</p></div><div className="freshXeroHeadActions">{!connected ? <button type="button" onClick={connectXero} disabled={busy || loading || !configured || !addonActive}>{busy ? "Opening Xero..." : "Connect to Xero"}</button> : null}{connected ? <button type="button" onClick={syncLatestInvoice} disabled={syncing || !draftReady || !latestInvoice}>{syncing ? "Syncing draft..." : "Sync latest invoice draft"}</button> : null}<button type="button" onClick={() => downloadAccountingPack("xero")}>Xero CSV</button><button type="button" onClick={() => downloadAccountingPack("myob")}>MYOB CSV</button><button type="button" onClick={() => downloadAccountingPack("both")}>Bookkeeper pack</button><button type="button" onClick={loadAccountingHealth} disabled={accountingBusy === "health"}>{accountingBusy === "health" ? "Checking..." : "Accounting health"}</button><button type="button" onClick={loadPaymentStatus} disabled={accountingBusy === "payments"}>{accountingBusy === "payments" ? "Loading..." : "Payment status"}</button>{connected ? <button type="button" onClick={disconnectXero} disabled={busy || syncing}>Disconnect Xero</button> : null}</div></div>

          <div className="freshXeroCards"><section><span>Latest invoice</span><b>{latestInvoice ? latestInvoiceTotal : "None"}</b><p>{latestInvoice ? latestInvoiceLabel : "Create an invoice before syncing to Xero."}</p></section><section><span>Invoice sync</span><b>{draftReady ? "Draft ready" : "Not ready"}</b><p>Creates a Xero draft invoice from the latest Churvox invoice.</p></section><section><span>Payment status</span><b>{settings.payment_sync_enabled ? "Read-back available" : "Manual/refresh"}</b><p>Use payment refresh before closing out paid invoices.</p></section><section><span>Accounting export</span><b>CSV ready</b><p>Xero, MYOB, clients, jobs, and notes are available as an export pack.</p></section><section><span>Health check</span><b>{accountingHealth ? "Loaded" : "Ready"}</b><p>{accountingHealth ? `${healthCounts.invoices || 0} invoices checked.` : "Run Accounting health to check sync/export readiness."}</p></section><section><span>Safety</span><b>Owner controlled</b><p>No auto-send, tax filing, payout files, or automatic paid marking.</p></section></div>
