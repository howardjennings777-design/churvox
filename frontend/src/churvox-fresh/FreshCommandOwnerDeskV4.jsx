...
function makeDraft(type, slip = {}) {
  ...
  return {
    ...
    found: slip.found || slip.title || config.title,
    why: slip.why || "Owner approval is required before Churvox changes, sends or assigns anything.",
    fields: config.fields.map((label) => ({ label, value: values[label] || "" })),
  };
}
...
function normalise(slip, index = 0) {
  ...
  return {
    id: slip.id || slip._id || `cmd-${type}-${Date.now()}-${index}`,
    type,
    title: slip.title || CONFIG[type]?.title || "Command action",
    status: slip.status || "open",
    urgency: slip.urgency || "Medium",
    draft: slip.draft || makeDraft(type, slip),
    source: slip.source || slip.info || "Prepared from Churvox records",
    page: slip.page || "jobs",
  };
}

function fallbackSlip() {
  return normalise({ id: "command-setup-review", type: "setup", title: "Command needs live data", urgency: "High", source: "No live Command actions came back from the backend yet" }, 0);
}

function readLocal() {
  try {
    const value = JSON.parse(localStorage.getItem(KEY) || "[]");
    return Array.isArray(value) ? value.map(normalise).filter((slip) => slip.status !== "approved") : [];
  } catch {
    return [];
  }
}
...
function missing(draft) {
  return (draft?.fields || []).filter((field) => isPlaceholder(field.value)).map((field) => field.label);
}

function category(slip) {
  if ((slip.status || "").includes("blocked") || slip.type === "blocked") return "blocked";
  if (missing(slip.draft).length) return "missing";
  return CONFIG[slip.type]?.cat || "missing";
}

function detail(slip) {
  const missed = missing(slip.draft);
  if (missed.length) return `Missing: ${missed.slice(0, 3).join(", ")}`;
  return (slip.draft.fields || []).slice(0, 3).map((field) => field.value).filter(Boolean).join(" · ");
}
...
export default function FreshCommandOwnerDesk({ onNavigate }) {
  ...
  async function load() {
    try {
      ...
      if (Array.isArray(data.slips) && data.slips.length) { setAndSave(data.slips.map(normalise)); return; }
      const scan = await request("/scan", {});
      if (Array.isArray(scan.slips) && scan.slips.length) { setAndSave(scan.slips.map(normalise)); return; }
    } catch {}
  ...
  async function runChecks() {
    try {
      ...
      if (Array.isArray(data.slips) && data.slips.length) { setAndSave(data.slips.map(normalise)); setMessage("Command updated from backend."); return; }
      setAndSave([fallbackSlip()]); setMessage("No live actions found yet. Setup check opened.");
    } catch {
