// CHURVOX_OPTION_F_PLANS_LIVE_CHECKOUT_V2_20260629
// Account-style Plans page: current plan, country/GST, Stripe Checkout, and billing portal.

import API_BASE from '../lib/apiBase';

const LAYER_ID = 'option-f-plans-pricing-desk';
const STYLE_ID = 'option-f-plans-live-checkout-v2-style';
const TOAST_ID = 'option-f-plans-live-checkout-v2-toast';
...
const aliases = { start: 'solo', solo: 'solo', crew: 'team', team: 'team', operator: 'pro', pro: 'pro', command: 'enterprise', enterprise: 'enterprise' };
...
function tokenHeaders() { const token = localStorage.getItem('token') || localStorage.getItem('authToken') || localStorage.getItem('access_token') || ''; return token ? { Authorization: `Bearer ${token}` } : {}; }
...
async function loadBillingStatus(force = false) {
  ...
  renderPlans();
  try {
    const [sub, me] = await Promise.allSettled([requestJson('/billing/subscription-status', { method: 'GET' }), requestJson('/auth/me', { method: 'GET' })]);
...
function currentPlanButtonLabel(plan) {
  if (state.currentPlan && state.currentPlan === plan.key) return 'Current plan';
...
function renderPlanCard(plan, country) {
  ...
  const taxText = meta.tax ? `inc ${meta.taxName}` : 'monthly';
  return `<article class="ofPlanLiveCard ${plan.badge ? 'featured' : ''}" data-plan-key="${plan.key}" data-current="${isCurrent ? 'true' : 'false'}">${isCurrent ? '<em>Current plan</em>' : plan.badge ? `<em>${escapeHtml(plan.badge)}</em>` : ''}<h3>${escapeHtml(plan.name)}</h3><div class="ofPlanPrice"><b>${escapeHtml(meta.symbol)}${plan.price}</b><span>/month ${escapeHtml(meta.tax)}</span></div><strong>${escapeHtml(incTax(plan.price, country))}/month ${taxText}</strong><p>${escapeHtml(plan.summary)}</p><ul>${plan.includes.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul><button type="button" class="ofPlanAction ${isCurrent ? 'current' : ''}" data-of-plan-checkout="${plan.key}">${escapeHtml(currentPlanButtonLabel(plan))}</button></article>`;
}

function renderAddonCard(addon, country) {
  const meta = countries[country] || countries.NZ;
  const taxText = meta.tax ? `inc ${meta.taxName}` : 'monthly';
  return `<article class="ofAddonLiveCard" data-addon-key="${addon.key}"><h3>${escapeHtml(addon.name)}</h3><div><b>${escapeHtml(meta.symbol)}${addon.price}</b> <span>/month ${escapeHtml(meta.tax)}</span></div><strong>${escapeHtml(incTax(addon.price, country))}/month ${taxText}</strong><p>${escapeHtml(addon.detail)}</p><button type="button" class="ofAddonAction" data-of-addon-checkout="${addon.key}">Add to plan</button></article>`;
}

function renderPlans() {
  if (!isPlansPage()) {
    const existing = document.getElementById(LAYER_ID);
    if (existing?.dataset.liveCheckout === '2') existing.remove();
    return;
  }
  ensureStyle();
  ...
  if (!layer) { layer = document.createElement('section'); layer.id = LAYER_ID; root.appendChild(layer); }
  layer.className = 'ofPlansLiveDesk';
  layer.dataset.liveCheckout = '2';
  ...
  layer.innerHTML = `<div class="ofBillingHero"><section class="ofBillingStatus"><h2>Plan and billing</h2><p>Select a plan and Churvox opens Stripe Checkout. Current plan and trial status stay visible so the boss knows exactly what is active.</p><div class="ofCurrentGrid"><span><b>${escapeHtml(current)}</b><small>Current plan</small></span><span><b>${escapeHtml(nice(status))}</b><small>Status</small></span><span><b>${escapeHtml(trial)}</b><small>Trial ends</small></span></div>${state.error ? `<div class="ofPlanError">${escapeHtml(state.error)}</div>` : ''}</section><section class="ofBillingRegion"><label><span>Billing country</span><select data-of-country>${Object.entries(countries).map(([code, item]) => `<option value="${code}" ${code === country ? 'selected' : ''}>${escapeHtml(item.label)} - ${escapeHtml(item.currency)}</option>`).join('')}</select></label><label><span>Owner email</span><input data-of-billing-email type="email" placeholder="owner email" value="${escapeHtml(email)}" /></label><button type="button" data-of-manage-billing>Manage billing</button><small>Prices are monthly. ${meta.tax ? `${meta.taxName} is added at checkout and shown here as an inclusive guide.` : 'Local tax may apply where required.'}</small></section></div><div class="ofPlanGridLive">${plans.map((plan) => renderPlanCard(plan, country)).join('')}</div><div class="ofAddonGridLive">${addons.map((addon) => renderAddonCard(addon, country)).join('')}</div><div class="ofPlanFinePrint">Pricing is unchanged: Start $39/month + GST, Crew $89/month + GST, Operator $149/month + GST, Command $299/month + GST, Command Growth Pack $99/month + GST, Accounting Sync Add-on $39/month + GST for non-Command tiers.</div>`;
}
...
function checkoutPayload({ plan, addon }) {
  ...
  const base = { country, billing_country: country, email, billing_interval: 'monthly', interval: 'month' };
  if (addon) return { ...base, addon, addon_key: addon, success_url: `${window.location.origin}/billing/success?session_id={CHECKOUT_SESSION_ID}&addon=${encodeURIComponent(addon)}&country=${country}`, cancel_url: `${window.location.origin}/billing/cancel?addon=${encodeURIComponent(addon)}&country=${country}` };
  return { ...base, plan, plan_key: plan, success_url: `${window.location.origin}/billing/success?session_id={CHECKOUT_SESSION_ID}&plan=${encodeURIComponent(plan)}&country=${country}`, cancel_url: `${window.location.origin}/billing/cancel?plan=${encodeURIComponent(plan)}&country=${country}` };
}
...
async function openCheckout({ plan, addon, button }) {
  ...
  const endpoints = addon ? ['/billing/create-addon-checkout', '/billing/addon/checkout', '/stripe/addon/checkout', '/billing/create-checkout-session', '/billing/checkout'] : ['/billing/create-checkout-session', '/billing/checkout', '/stripe/checkout', '/subscriptions/checkout', '/checkout/session', '/create-checkout-session'];
  let lastError = null;
...
function handleClick(event) {
  ...
  if (planButton) { event.preventDefault(); openCheckout({ plan: planButton.dataset.ofPlanCheckout, button: planButton }); return; }
  const addonButton = event.target.closest('[data-of-addon-checkout]');
  if (addonButton) { event.preventDefault(); openCheckout({ addon: addonButton.dataset.ofAddonCheckout, button: addonButton }); }
}

function handleInput(event) {
  const country = event.target.closest('[data-of-country]');
  if (country) { state.country = normalizeCountry(country.value); localStorage.setItem(COUNTRY_STORE, state.country); scheduleRender(); return; }
  const email = event.target.closest('[data-of-billing-email]');
  if (email) { state.email = clean(email.value); if (state.email) localStorage.setItem(EMAIL_STORE, state.email); }
}
...
function scheduleRender() {
  ...
  setTimeout(() => {
    renderPlans();
    loadBillingStatus();
    setTimeout(() => { state.renderQueued = false; }, 180);
  }, 80);
}

if (typeof window !== 'undefined' && !window.__CHURVOX_OPTION_F_PLANS_LIVE_CHECKOUT_V2__) {
  window.__CHURVOX_OPTION_F_PLANS_LIVE_CHECKOUT_V2__ = true;
