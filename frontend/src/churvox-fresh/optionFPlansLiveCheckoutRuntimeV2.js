// CHURVOX_OPTION_F_PLANS_ACCOUNT_CENTER_V5_20260706
// Compact Plans runtime: real plan page, real checkout for users, audit-safe checkout guards.
// Nav is rendered inside this billing page so it cannot flicker or disappear.
...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    #${LAYER_ID} .ofBillingHero{display:grid;grid-template-columns:minmax(260px,1fr) minmax(260px,380px);gap:14px;scroll-margin-top:110px}
    ...
    #${LAYER_ID} .ofBillingNav{display:flex;align-items:center;gap:7px;width:100%;max-width:100%;box-sizing:border-box;padding:7px;border:1px solid rgba(16,21,19,.1);border-radius:17px;background:rgba(255,255,255,.92);box-shadow:0 14px 32px rgba(16,21,19,.07);overflow-x:auto;scrollbar-width:thin}
    #${LAYER_ID} .ofBillingNav button{flex:1 0 auto;min-width:max-content;min-height:32px;padding:7px 12px;border-radius:999px;background:#eef2ed;color:#111815;font-size:12px;font-weight:1000;white-space:nowrap}
    #${LAYER_ID} .ofBillingNav button:first-child,#${LAYER_ID} .ofBillingNav button:hover{background:#111815;color:#fff}
    ...
    #${LAYER_ID} .ofBillingFlow{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:14px;background:#f8faf9;scroll-margin-top:110px}
    ...
    #${LAYER_ID} .ofPlanGridLive{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;scroll-margin-top:110px}
    ...
    #${LAYER_ID} .ofAddonGridLive{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;scroll-margin-top:110px}
    ...
    #${LAYER_ID} .ofPlanFinePrint{padding:12px 14px;border-radius:14px;background:#f8faf9;scroll-margin-top:110px}
    ...
    @media(max-width:760px){#${LAYER_ID} .ofBillingNav button{flex:0 0 auto;font-size:11px}#${LAYER_ID} .ofPlanGridLive,#${LAYER_ID} .ofAddonGridLive,#${LAYER_ID} .ofBillingFlow,#${LAYER_ID} .ofCurrentGrid{grid-template-columns:1fr}#${LAYER_ID} .ofBillingStatus h2{font-size:27px}#${TOAST_ID}{left:10px;right:10px;bottom:12px;max-width:none}}
  `;
...
function renderPlans() {
  ...
  layer.innerHTML = `
    <div class="ofBillingHero" data-of-billing-section="overview">
      ...
      <section class="ofBillingRegion" data-of-billing-section="help">
    ...
    <nav class="ofBillingNav" aria-label="Plans and billing navigation">
      <button type="button" data-of-billing-nav="overview">Overview</button>
      <button type="button" data-of-billing-nav="usage">Usage</button>
      <button type="button" data-of-billing-nav="plans">Plans</button>
      <button type="button" data-of-billing-nav="addons">Add-ons</button>
      <button type="button" data-of-billing-nav="help">Billing help</button>
    </nav>
    <div class="ofBillingFlow" data-of-billing-section="usage">
    ...
    <div class="ofPlanGridLive" data-of-billing-section="plans">${PLANS.map((plan) => planCard(plan, country)).join('')}</div>
    <div class="ofAddonGridLive" data-of-billing-section="addons">${ADDONS.map((addon) => addonCard(addon, country)).join('')}</div>
...
function scrollBillingNav(section) {
  const layer = document.getElementById(LAYER_ID);
  if (!layer) return;
  let target = layer.querySelector(`[data-of-billing-section="${CSS.escape(section)}"]`);
  if (section === 'usage') {
    const liveUsage = [...document.querySelectorAll('section,article,div')].find((node) => /live plan usage/i.test(node.textContent || ''));
    target = liveUsage || target;
  }
...
function handleClick(event) {
  const nav = event.target.closest('[data-of-billing-nav]');
  if (nav) { event.preventDefault(); scrollBillingNav(nav.dataset.ofBillingNav); return; }
