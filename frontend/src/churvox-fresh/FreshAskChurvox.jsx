...
export default function FreshAskChurvox({ onNavigate }) {
  ...
  return (
    <section className="freshAskPage tellChurvoxPage">
      <form className="tellCommandBox tellCommandBoxTop" onSubmit={submit}><label><span>What do you want to do?</span><input value={text} onChange={(event) => setText(event.target.value)} placeholder="open jobs, add client, show unpaid invoices…" autoFocus /></label><button type="submit">{result.button}</button></form>
