...
export default function FreshSettings({ onNavigate }) {
  ...
  function update(field, value) {
    ...
    if (["businessName", "region", "tradingName", "invoicePrefix", "supportEmail", "replyEmail", "phone", "contactPreference", "commandRule", "tradeType", "gstRate"].includes(field)) { setDraft((current) => ({ ...current, [field]: value })); setSettings((current) => ({ ...current, [field]: value })); setSavedAt(`Draft saved ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`); return; }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      const draftToSave = { ...draft, businessName: settings.businessName, country: settings.country, region: settings.region, tradingName: settings.tradingName, invoicePrefix: settings.invoicePrefix, supportEmail: settings.supportEmail, replyEmail: settings.replyEmail, phone: settings.phone, contactPreference: settings.contactPreference, commandRule: settings.commandRule, gstRate: settings.gstRate, tradeType: settings.tradeType };
      saveDraftLocally(draftToSave); setDraft(draftToSave);
      const saveResults = await Promise.allSettled([
        patch("/user/gst", { gst_rate: gst }),
        patch("/user/trade", { trade_type: settings.tradeType || "other" }),
        patch("/user/business-profile", { business_name: settings.businessName, trading_name: settings.tradingName, billing_country: settings.country, service_region: settings.region, phone: settings.phone, invoice_prefix: settings.invoicePrefix, support_email: settings.supportEmail, reply_email: settings.replyEmail }),
      ]);
      const profileSave = saveResults[2]?.status === "fulfilled" ? saveResults[2].value : null;
      const synced = saveResults.filter((result) => result.status === "fulfilled" && result.value?.success !== false).length;
      ...
      else if (synced) setSavedAt(profileSave?.success === false ? "Saved locally. Business profile will sync when available." : "Saved to business account");
      else setSavedAt("Saved locally. Cloud sync will retry when available.");
