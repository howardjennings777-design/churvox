...
const helpTopics = [
  { id: 1, title: "Getting started", type: "Setup", status: "Recommended", note: "Set business details, GST, branding, team roles and email before sending real work." },
...
export default function FreshSupport({ onNavigate }) {
  ...
  async function sendSupportRequest() {
    ...
    try {
      ...
      setError(err?.message || "Support message could not send. Email hello@churvox.com if it keeps failing.");
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <section>
      ...
      <section className="freshGrid">
        ...
        <aside className="freshCard"><h2>Contact Churvox</h2><p>Send a real support request to hello@churvox.com without leaving the app.</p>{status ? <div className="freshItem"><b>Support status</b><span>{status}</span></div> : null}{error ? <div className="freshItem need"><b>Support needs attention</b><span>{error}</span></div> : null}<label className="freshField"><span>Help type</span><select value={helpType} onChange={(event) => setHelpType(event.target.value)}><option>Setup help</option><option>Billing help</option><option>Bug report</option><option>Feature request</option><option>Worker app help</option><option>Xero sync help</option></select></label><label className="freshField"><span>Message</span><textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Tell us what is stuck or what you expected to happen." /></label><div className="freshActions"><button className="freshPrimary" type="button" disabled={sending} onClick={sendSupportRequest}>{sending ? "Sending…" : "Send support request"}</button><button type="button" className="freshOrange" onClick={() => onNavigate?.("setupassistant")}>Open AI guide</button><button type="button" className="freshGhost" onClick={() => onNavigate?.("command")}>Send issue to Command</button></div><div className="freshItem need"><b>Launch note</b><span>Live chat can come later. Start with clean support requests and helpful guides.</span></div></aside>
      </section>
