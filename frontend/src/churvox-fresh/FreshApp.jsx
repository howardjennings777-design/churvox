...
function Drawer({ selected, onClose }) {
  ...
  return <aside className={`cocDrawer ${detail.approval ? "approvalSlip" : ""} ${detail.job ? "jobSlip" : ""}`}><button type="button" onClick={onClose}>Close</button><em>{selected.type || "Record"}</em><h2>{detail.title}</h2><p>{detail.note}</p><div>{detail.fields.map(([label, value, textarea, type, options, readOnly]) => <Field key={label} label={label} value={value} textarea={textarea} type={type} options={options} readOnly={readOnly} />)}</div>{detail.approval ? <div className="approvalActions"><button type="button" className="action">Approve</button><button type="button" className="action dark">Edit form</button><button type="button" className="action quiet">Park</button></div> : null}{detail.job ? <div className="approvalActions"><button type="button" className="action">Save job</button><button type="button" className="action dark">Create quote</button><button type="button" className="action quiet">Close</button></div> : null}{detail.client ? <div className="approvalActions"><button type="button" className="action">Save client</button><button type="button" className="action dark">Add job</button><button type="button" className="action quiet">New quote</button></div> : null}{detail.worker ? <div className="approvalActions"><button type="button" className="action">Save day</button><button type="button" className="action dark">Message worker</button><button type="button" className="action quiet">Open timesheet</button></div> : null}{detail.quote ? <div className="approvalActions"><button type="button" className="action">Save quote</button><button type="button" className="action dark">Prepare follow-up</button><button type="button" className="action quiet">Command approval</button></div> : null}{detail.invoice ? <div className="approvalActions"><button type="button" className="action">Save invoice</button><button type="button" className="action dark">Mark reviewed</button><button type="button" className="action quiet">Command sync</button></div> : null}{detail.message ? <div className="approvalActions"><button type="button" className="action">Save draft</button><button type="button" className="action dark">Edit reply</button><button type="button" className="action quiet">Command send</button></div> : null}{detail.person ? <div className="approvalActions"><button type="button" className="action">Save person</button><button type="button" className="action dark">Update access</button><button type="button" className="action quiet">Payroll review</button></div> : null}</aside>;
}
...
function Quotes({ data, open }) {
  ...
  return <div className="cocPage quotesPage"><div className="toolbar"><button type="button">+ New Quote</button><button type="button">Follow-ups</button><button type="button">Accepted to Jobs</button></div><Panel title="Quote Pipeline" tone="amber" className="full"><div className="miniStats">{stages.map((stage) => <Stat key={stage} label={stage.toLowerCase()} value={data.quotes.filter((quote) => String(quote.status).toLowerCase().includes(stage.toLowerCase())).length} tone={stage === "Draft" ? "amber" : stage === "Accepted" ? "coral" : "blue"} />)}<Stat label="value" value={money(total)} tone="green" /></div></Panel><Panel title="Quote Cards" tone="blue" className="full"><div className="workCards">{data.quotes.slice(0, 8).map((quote) => <button key={quote.id} type="button" className="workCard" onClick={() => open("Quote", quote)}><b>{quote.title}</b><small>{quote.client} - {quote.status}</small><span>{quote.scope || "Scope ready"}</span><em>{money(quote.amount)}</em><i>{quote.next || quote.followUp}</i></button>)}</div></Panel><Panel title="Follow-Up Ready" tone="coral" className="full"><div className="proofGrid">{data.quotes.filter((quote) => /ready|tomorrow|follow|convert/i.test(`${quote.followUp} ${quote.next}`)).slice(0, 3).map((quote) => <Row key={quote.id} title={quote.title} meta={`${quote.client} - ${quote.followUp}`} tone="coral" onClick={() => open("Quote", quote)} />)}</div><p>Churvox prepares the next message. Sending approval still happens in Command.</p></Panel></div>;
}
...
function Team({ data, open }) {
  ...
  return <div className="cocPage teamPage"><Panel title="Team Pulse" tone="blue" className="full"><div className="miniStats"><Stat label="staff" value={data.workers.length} tone="blue" /><Stat label="app active" value={active} /><Stat label="payroll" value={payroll} tone="amber" /><Stat label="roles" value="live" tone="coral" /></div></Panel><Panel title="Staff Cards" tone="blue" className="full"><div className="workerCards">{data.workers.map((worker) => <button key={worker.id} type="button" className="workerCard" onClick={() => open("Person", worker)}><b>{worker.name}</b><small>{worker.role} - {worker.app}</small><span>{worker.job} - {worker.gps}</span><em>{worker.payroll}</em><i>{worker.timesheet} - {worker.slip}</i></button>)}</div></Panel><Panel title="Roles + Access" tone="coral"><div className="scroll">{data.workers.map((worker) => <Row key={worker.id} title={worker.name} meta={`${worker.role} - ${worker.app}`} tone="coral" onClick={() => open("Person", worker)} />)}</div></Panel><Panel title="Payroll Review" tone="amber"><div className="scroll">{data.workers.map((worker) => <Row key={worker.id} title={worker.name} meta={`${worker.timesheet} - ${worker.payroll}`} tone="amber" onClick={() => open("Person", worker)} />)}</div></Panel><Panel title="Editable Person Form" tone="blue" className="wide"><div className="formGrid"><Field label="Name" value={data.workers[0]?.name} /><Field label="Role/access" value={data.workers[0]?.role} options={optionSets.role} /><Field label="Payroll review" value={data.workers[0]?.payroll} /><Field label="Worker app" value={data.workers[0]?.app} /><Field label="Current job" value={data.workers[0]?.job} /><Field label="Timesheet" value={data.workers[0]?.timesheet} /></div></Panel></div>;
}
