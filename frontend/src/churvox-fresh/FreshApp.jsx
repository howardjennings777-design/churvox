...
function detailFor(selected) {
  ...
  if (kind.includes("command") || kind.includes("approval")) return { title: "Approval slip", note: "This is the owner check. Approve, edit and park live only on Command.", fields: [["Type", selected.type], ["Record", selected.title], ["Prepared status", selected.status], ["Recommended action", selected.owner], ["Source", "Job records, client memory, messages, time, photos and accounting state"], ["Decision", "Approve / Edit / Park in Command"]] };
...
function Drawer({ selected, onClose }) {
  ...
  return <aside className="cocDrawer"><button type="button" onClick={onClose}>Close</button><em>{selected.type || "Record"}</em><h2>{detail.title}</h2><p>{detail.note}</p><div>{detail.fields.map(([label, value, textarea]) => <Field key={label} label={label} value={value} textarea={textarea} />)}</div></aside>;
}
...
const baseCss = `
.churvoxOptionC,.churvoxOptionC *{box-sizing:border-box}.churvoxOptionC{min-height:100vh;display:grid;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.churvoxOptionC button,.churvoxOptionC input,.churvoxOptionC textarea{font:inherit}.cocBar{display:grid}.brand{display:flex;align-items:center}.brand i{display:block}.title,.brand,.owner{min-width:0}.title h1,.title p{margin:0}.owner span,.owner b,.cocRow b,.cocRow small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cocNav{display:flex}.workspace{min-width:0}.cocPage{display:grid}.toolbar{display:flex;flex-wrap:wrap}.cocPanel{position:relative}.scroll{display:grid;overflow:auto}.scroll.tight{max-height:150px}.cocRow{display:grid;align-items:center;border:0;cursor:pointer}.cocRow i{display:block;border-radius:50%}.cocRow em{font-style:normal}.cocWeek,.pulse,.proofGrid,.moneyStrip,.miniStats{display:flex;flex-wrap:wrap}.dayControl{display:grid;gap:12px}.miniStats{gap:8px}.miniStat{display:grid;min-width:92px;border-radius:12px;padding:10px 12px;background:#eef2ed;color:#151c19}.miniStat b{font-size:20px;line-height:1}.miniStat small{font-size:11px;font-weight:900;color:#5e6b65}.chip{display:inline-flex;align-items:center;cursor:pointer}.map{position:relative;overflow:hidden}table{width:100%}.formGrid{display:grid}.cocField{display:grid}.cocField input,.cocField textarea{width:100%}.cocDrawer{position:fixed}.cocDrawer>div{display:grid}
`;
