...
function useOsData() {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      const workers = listFrom(responses[2].value, "team").map((worker, index) => ({ ...seed.workers[index % seed.workers.length], ...worker, id: idOf(worker) || `worker-${index}`, name: textOf(worker.name, worker.full_name, worker.email, seed.workers[index % seed.workers.length]?.name) }));
