...
const seed = {
  ...
  command: [
    { id: "a1", type: "Invoice ready", title: "Belmont hedge trim", status: "Filled invoice", owner: "Approve" },
    { id: "a2", type: "Quote ready", title: "Fence repair", status: "Filled quote", owner: "Edit" },
    { id: "a3", type: "Message ready", title: "Friday request", status: "Draft reply", owner: "Approve" },
    { id: "a4", type: "Client/job issue", title: "Extra green waste", status: "Needs call", owner: "Park" },
    { id: "a5", type: "Timesheet/proof/slip issue", title: "Alex clock-out", status: "Mismatch", owner: "Edit" },
  ],
};
...
function Field({ label, value, textarea = false }) {
  ...
  return <label className="cocField"><span>{label}</span><Tag value={value || ""} readOnly rows={textarea ? 4 : undefined} /></label>;
}

function Drawer({ selected, onClose }) {
  if (!selected) return null;
  return <aside className="cocDrawer"><button type="button" onClick={onClose}>Close</button><em>{selected.type || "Record"}</em><h2>{selected.title || selected.name || selected.subject || selected.number}</h2><p>{selected.detail || selected.notes || selected.status || "Useful detail opens here without leaving the page."}</p><div>{Object.entries(selected).filter(([key, value]) => !["id", "raw", "type"].includes(key) && value !== undefined && typeof value !== "object").slice(0, 10).map(([key, value]) => <Field key={key} label={key.replace(/_/g, " ")} value={String(value)} textarea={String(value).length > 90} />)}</div></aside>;
}

function WeekStrip() {
  return <div className="cocWeek">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day, index) => <button key={day} type="button" className={index === 0 ? "active" : ""}>{day}</button>)}</div>;
}
...
function Today({ data, open }) {
  ...
  return <div className="cocPage today"><Panel title="Monday-Sunday Selector" className="wide"><WeekStrip /></Panel><Panel title="Money Due Today" tone="amber"><strong className="money">{money(due)}</strong><span className="chip amber">{data.invoices.filter((invoice) => /due/i.test(invoice.status)).length} due today</span></Panel><Panel title="Jobs Today"><div className="scroll">{data.jobs.slice(0, 5).map((job) => <Row key={job.id} title={`${job.time} ${job.title}`} meta={`${job.client} - ${job.worker} - ${job.status}`} onClick={() => open("Job", job)} />)}</div></Panel><Panel title="Who Is Working" tone="blue"><div className="scroll">{data.workers.slice(0, 5).map((worker) => <Row key={worker.id} title={`${worker.name}: ${worker.status}`} meta={`${worker.job} - GPS ${worker.gps}`} tone="blue" onClick={() => open("Worker day", worker)} />)}</div></Panel><Panel title="Worker Messages / Photos" tone="coral">{data.messages.slice(0, 3).map((message) => <span key={message.id} className="chip coral" onClick={() => open("Message", message)}>{message.subject}</span>)}<span className="chip coral">proof photos ready</span></Panel><Panel title="Problems Today" tone="red">{data.jobs.filter((job) => job.issue).slice(0, 5).map((job) => <span key={job.id} className="chip red" onClick={() => open("Job problem", job)}>{job.issue}</span>)}</Panel><Panel title="Approvals Waiting" tone="amber">{data.command.slice(0, 5).map((item) => <Row key={item.id} title={item.type} meta={item.title} tone="amber" onClick={() => open("Command item", item)} />)}</Panel></div>;
}

function Command({ data, open }) {
  const selected = data.command[0] || seed.command[0];
  return <div className="cocPage command"><Panel title="Waiting For Approval" tone="coral"><div className="scroll">{data.command.slice(0, 5).map((item) => <Row key={item.id} title={item.type} meta={`${item.title} - ${item.status}`} tone="coral" onClick={() => open("Command item", item)} />)}</div></Panel><Panel title="Filled Approval Form" tone="blue" className="wide"><h3>{selected.type}</h3><p>Churvox prepared this from job records, client memory, messages, time, photos and accounting state. The owner only checks the filled record.</p><div className="formGrid"><Field label="Record" value={selected.title} /><Field label="Prepared status" value={selected.status} /><Field label="Recommended owner action" value={selected.owner} /><Field label="Churvox note" value="Approve, edit or park. These actions only live on Command." textarea /></div></Panel><Panel title="Owner Actions" tone="amber"><button className="action">Approve</button><button className="action dark">Edit</button><button className="action quiet">Park</button><p>Command remains the approval desk.</p></Panel></div>;
}

function Jobs({ data, open }) {
  return <div className="cocPage"><div className="toolbar"><button type="button">+ Add Job</button><button type="button">Recurring</button><button type="button">Dispatch Board</button></div><Panel title="Dispatch Board + Runs"><div className="map" /><Row title="Run A: 4 jobs" meta="assigned worker and route" /><Row title="Run B: 3 jobs" meta="assigned worker and route" /><Row title="Recurring inside Jobs" meta="weekly / fortnightly / monthly" /></Panel><Panel title="Job List + Status" tone="blue" className="wide"><table><tbody>{data.jobs.slice(0, 5).map((job) => <tr key={job.id} onClick={() => open("Job", job)}><td>{job.title}</td><td>{job.status}</td><td>{job.worker}</td><td>{job.proof}</td></tr>)}</tbody></table><div className="formGrid"><Field label="Client" value={data.jobs[0]?.client} /><Field label="Assigned worker" value={data.jobs[0]?.worker} /><Field label="Job status" value={data.jobs[0]?.status} /><Field label="Time / proof / photos" value={`${data.jobs[0]?.time || ""} - ${data.jobs[0]?.proof || ""}`} /></div></Panel><Panel title="Problems Become Command Items" tone="red">{data.jobs.filter((job) => job.issue).slice(0, 5).map((job) => <Row key={job.id} title={job.title} meta={job.issue} tone="red" onClick={() => open("Job problem", job)} />)}</Panel></div>;
}
...
function Workers({ data, open }) {
  return <div className="cocPage"><Panel title="GPS Map" tone="blue" className="wide"><div className="map big" /></Panel><Panel title="Clocked In + Current Job">{data.workers.slice(0, 4).map((worker) => <Row key={worker.id} title={worker.name} meta={`${worker.status} - ${worker.job}`} tag={worker.gps} onClick={() => open("Worker day", worker)} />)}</Panel><Panel title="Proof / Photos / Worker Messages" tone="coral" className="wide"><div className="proofGrid">{["3 photos ready", "2 worker messages", "GPS trail", "1 proof check"].map((item) => <Row key={item} title={item} meta="from worker app" tone="coral" />)}</div></Panel><Panel title="Timesheets / Slips" tone="amber">{data.workers.slice(0, 4).map((worker) => <Row key={worker.id} title={worker.name} meta={`${worker.timesheet} - ${worker.payroll}`} tone="amber" onClick={() => open("Timesheet", worker)} />)}</Panel></div>;
}
