...
const emptyClient = { name: "", email: "", billingEmail: "", phone: "", address: "", notes: "" };
...
function normalizeClient(client, index) {
  ...
  return { ...client, id, name, type, status, email, phone, address, billingEmail, notes, risk: billingEmail || email ? "Clean setup" : "Contact detail missing", value: client?.value || client?.lifetime_value || client?.amount_due || "Real client", sortTime: timeValue(client) };
}
...
export default function FreshClients({ onNavigate }) {
  ...
  async function saveSelectedClient() {
    ...
    setSaving(true);
    const res = await api.patch(`/clients/${encodeURIComponent(selected.id)}`, { name: selected.name, email: selected.email, phone: selected.phone, address: selected.address, billing_email: selected.billingEmail, notes: selected.notes });
    setSaving(false);
    if (!res.success) { setError(res.error || "Could not update client"); return; }
    await loadClients();
  }
  ...
  async function saveNewClient() {
    ...
    const payload = { name, client_name: name, customer_name: name, email: newClient.email.trim(), billing_email: newClient.billingEmail.trim(), phone: newClient.phone.trim(), address: newClient.address.trim(), notes: newClient.notes.trim() };
    ...
    setAddOpen(false);
    await loadClients();
    window.dispatchEvent(new CustomEvent("churvox:fresh-data-updated", { detail: { type: "client-created" } }));
  }
  ...
  return (
    <section>
      ...
      <section className="freshGrid">
        ...
        <section className="freshCard"><h2>{selected?.name || "Select client"}</h2>{selected ? <><div className="freshMiniGrid"><div><span>Status</span><b>{selected.status}</b></div><div><span>Type</span><b>{selected.type}</b></div><div><span>Value</span><b>{selected.value}</b></div><div><span>Risk</span><b>{selected.risk}</b></div></div><label className="freshField"><span>Client name</span><input value={selected.name} onChange={(event) => updateSelectedClient({ name: event.target.value })} /></label><label className="freshField"><span>Email</span><input value={selected.email} onChange={(event) => updateSelectedClient({ email: event.target.value })} /></label><label className="freshField"><span>Billing email</span><input value={selected.billingEmail} placeholder="Required for invoice automation" onChange={(event) => updateSelectedClient({ billingEmail: event.target.value })} /></label><label className="freshField"><span>Phone</span><input value={selected.phone} onChange={(event) => updateSelectedClient({ phone: event.target.value })} /></label><label className="freshField"><span>Service address</span><input value={selected.address} onChange={(event) => updateSelectedClient({ address: event.target.value })} /></label><label className="freshField"><span>Client notes</span><textarea value={selected.notes} onChange={(event) => updateSelectedClient({ notes: event.target.value })} /></label><div className="freshActions"><button className="freshPrimary" type="button" disabled={saving} onClick={saveSelectedClient}>{saving ? "Saving…" : "Save client"}</button></div></> : <div className="freshItem"><b>No client selected</b><span>Add your first client to start the workflow.</span></div>}</section>
      ...
      {addOpen ? (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(15,23,42,.62)", display: "grid", placeItems: "center", padding: 16 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setAddOpen(false); }}>
          <section className="freshCard" style={{ width: "min(640px, 100%)", maxHeight: "88vh", overflow: "auto", boxShadow: "0 30px 80px rgba(0,0,0,.35)" }}>
            ...
            <label className="freshField"><span>Email</span><input value={newClient.email} onChange={(e) => setNewClient((c) => ({ ...c, email: e.target.value }))} placeholder="customer@email.com" /></label>
            <label className="freshField"><span>Billing email</span><input value={newClient.billingEmail} onChange={(e) => setNewClient((c) => ({ ...c, billingEmail: e.target.value }))} placeholder="for invoices" /></label>
