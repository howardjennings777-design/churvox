...
function sendLaunchControlToCommand(items, onNavigate) {
  ...
  try {
    ...
    const slip = { id: `launch-control-${Date.now()}`, group: "Launch Readiness", area: "Launch Readiness", page: "launchcontrol", title: blocked.length ? "Launch blocked" : attention.length ? "Launch needs fixes" : needs.length ? "Launch needs final tests" : "Launch looks ready", info: `${blocked.length} blocked · ${attention.length} attention · ${needs.length} tests`, urgency: blocked.length ? "High" : attention.length ? "Medium" : needs.length ? "Medium" : "Low", found: [...blocked, ...attention].length ? [...blocked, ...attention].map((item) => `${item.area}: ${item.proof}`).join(" | ") : "No hard launch blockers marked.", prepared: needs.length ? `Test next: ${needs.map((item) => item.area).join(", ")}` : "Controlled beta can start when owner is comfortable.", why: "Launch should be decided from core workflow readiness, not page count.", owner: "Fix blockers, test needs, then approve a controlled beta.", payload: { blocked: blocked.map((item) => item.area), attention: attention.map((item) => item.area), needs_test: needs.map((item) => item.area) }, fromInbox: true, createdAt: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) };
