...
function businessDetailsDone(settings = {}) {
  const hasName = Boolean(pick(settings, "business_name", "trading_name"));
  const hasContact = Boolean(pick(settings, "email", "phone"));
  const hasInvoiceDefaults = Boolean(pick(settings, "invoice_prefix")) && Number(settings?.default_gst_rate || 0) >= 0;
  return hasName && hasContact && hasInvoiceDefaults;
}
...
function buildSetupSteps(data, businessSettings = {}) {
  ...
  const settingsDone = businessDetailsDone(businessSettings);
...
function buildToday(data, businessSettings = {}) {
  ...
  const setup = buildSetupSteps({ jobs, clients, invoices, quotes, workers }, businessSettings);
...
function TodayJobCard({ job, onNavigate }) {
  ...
  return <button type="button" className="freshTodayJobCard" onClick={() => onNavigate?.("jobs")}><b>{timeText(when)}</b><strong>{jobTitle(job)}</strong><span>{clientName(job)} · {jobAddress(job)}</span><small>{jobWorker(job) ? `Worker: ${jobWorker(job)}` : "Worker needed"} · {status || "ready"}</small></button>;
}
...
export default function FreshSmartHub({ onNavigate }) {
  ...
  const loadLiveData = React.useCallback(async () => {
    ...
    const next = {}; const failedCore = [];
    await Promise.all(Object.entries(LIVE_ENDPOINTS).map(async ([key, endpoint]) => {
      try { const result = await get(endpoint, { timeout: 25000 }); if (!result?.success) { if (["jobs", "invoices"].includes(key)) failedCore.push(key); next[key] = []; return; } next[key] = asArray(result.data, key); }
      catch { if (["jobs", "invoices"].includes(key)) failedCore.push(key); next[key] = []; }
    }));
  ...
  React.useEffect(() => {
    ...
    const refreshSettings = () => setBusinessSettings(loadBusinessSettings());
    window.addEventListener("churvox:fresh-data-updated", refresh);
    window.addEventListener("churvox-business-settings-updated", refreshSettings);
    window.addEventListener("focus", refresh);
    const timer = window.setInterval(refresh, 45000);
    return () => { window.removeEventListener("churvox:fresh-data-updated", refresh); window.removeEventListener("churvox-business-settings-updated", refreshSettings); window.removeEventListener("focus", refresh); window.clearInterval(timer); };
  }, [loadLiveData]);
