...
export default function FreshImports({ onNavigate }) {
  ...
  return <section className="freshImportsPage">
    ...
    <section className="freshCard freshImportsUploader"><div className="freshMiniGrid"><div><span>Type</span><b>{type}</b></div><div><span>Required</span><b>{cfg.required}</b></div><div><span>File</span><b>{fileName || "None"}</b></div><div><span>Failed</span><b>{failed}</b></div></div><div className="freshImportsForm"><label><span>Data type</span><select value={type} onChange={(e) => { setType(e.target.value); reset(); }}>{TYPES.map((x) => <option key={x}>{x}</option>)}</select></label><label><span>CSV file</span><input type="file" accept=".csv,text/csv" onChange={readFile} /></label><label className="wide"><span>Expected headers</span><input readOnly value={cfg.example} /></label></div><div className="freshImportsActions"><button type="button" onClick={importReady} disabled={importing || !ready.length}>{importing ? "Importing..." : `Import ${ready.length} ready rows`}</button><button type="button" onClick={() => openArea(type)}>Open {type}</button><button type="button" onClick={() => onNavigate?.("command")}>Open Command</button><button type="button" onClick={reset}>Reset</button></div>{message ? <div className={`freshImportsNotice ${failed || blocked ? "need" : ""}`}><b>Import status</b><span>{message}</span></div> : null}</section>
