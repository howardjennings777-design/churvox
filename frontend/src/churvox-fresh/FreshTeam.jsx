...
function normalizeMember(member, index) {
  ...
  return {
    ...member,
    id,
    name: member?.name || member?.full_name || member?.display_name || "Unnamed worker",
    role: roleOf(member?.team_role || member?.worker_role || member?.role),
    status: statusOf(member?.status),
    phone: member?.phone || member?.mobile || "",
    email: member?.email || "",
    payRate: member?.pay_rate || member?.payRate || member?.hourly_rate
      ? `$${member?.pay_rate || member?.payRate || member?.hourly_rate}/hr`
      : "Not set",
...
export default function FreshTeam({ onNavigate }) {
  ...
  const loadTeam = React.useCallback(async () => {
    ...
    if (!res.success) {
      setTeam([]);
      setSelectedId("");
      setError(res.error || "Could not load real team members");
      setLoading(false);
      return;
    }
  ...
  async function removeSelected() {
    ...
    if (!res.success) {
      setError(res.error || "Could not remove worker");
      return;
    }
  ...
  return (
    <section>
      ...
      <section className="freshCommandPulse">
        ...
        <aside className="freshCard">
          <h2>{loading && team.length === 0 ? "…" : team.length}</h2>
          <p>Total people</p>
        </aside>
      </section>
      ...
      {error ? (
        <section className="freshCard freshItem need">
          <b>Team needs attention</b>
          <span>{error}</span>
          <button type="button" className="freshPrimary" onClick={loadTeam}>Retry</button>
        </section>
      ) : null}

      {actionMessage ? (
        <section className="freshCard freshItem">
          <b>Done</b>
      ...
      <section className="freshGrid">
        <aside className="freshCard">
          ...
          {loading && team.length > 0 ? (
            <div className="freshItem">
              <b>Refreshing team…</b>
              <span>Showing saved workers while Churvox refreshes.</span>
            </div>
          ) : null}
          ...
          {selected ? (
            <>
              <div className="freshMiniGrid">
                ...
                <div><span>Role</span><b>{selected.role}</b></div>
                <div><span>Pay</span><b>{selected.payRate}</b></div>
