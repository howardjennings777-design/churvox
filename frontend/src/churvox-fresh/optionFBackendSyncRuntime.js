...
const ENABLE_KEY = 'churvox_enable_backend_autosync';
...
function isBlocked(record) { return Boolean(record?._blockedByCommand || record?._doNotShowToday || record?._commandMissing); }
function mapPayload(key, record) {
  if (key === 'jobs') return { title: record.title || record['Job name'] || 'New job', client_name: record.client || record.Client || '', address: record.address || record['Site address'] || '', service: record.service || record.Service || '', assigned_worker_name: record.worker || record['Assigned worker'] || '', scheduled_date: record.date || record['Scheduled date'] || '', scheduled_time: record.time || record['Start time'] || '', price: Number(record.price || record['Price NZD'] || 0), billing_type: record.billing || record['Billing type'] || '', recurring: record.recurring || record.Frequency || '', notes: record.notes || record['Job notes'] || '', source: 'option_f_working_actions' };
...
async function postRecord(key, record) {
  ...
  try {
    const response = await fetch(apiUrl(endpoint), { method: 'POST', credentials: 'include', headers: authHeaders(), body: JSON.stringify(mapPayload(key, record)) });
...
function renderBadge(state = loadState()) {
  ...
  if (!pending && !synced && !blocked) { badge?.remove(); return; }
  if (!badge) { badge = document.createElement('div'); badge.id = SYNC_BADGE_ID; badge.style.cssText = 'position:fixed;left:18px;bottom:58px;z-index:99991;border:1px solid rgba(16,21,19,.09);border-radius:999px;padding:8px 11px;background:#fff;color:#111815;box-shadow:0 12px 28px rgba(16,21,19,.12);font:900 12px Inter,system-ui,sans-serif;pointer-events:none'; document.body.appendChild(badge); }
  badge.textContent = blocked ? `${blocked} held for Command fix` : pending ? `${pending} pending owner sync` : `${synced} synced to backend`;
}
