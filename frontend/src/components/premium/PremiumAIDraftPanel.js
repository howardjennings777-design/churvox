...
import { Sparkles, ShieldCheck, Copy, Trash2, Wand2 } from 'lucide-react';
...
export default function PremiumAIDraftPanel({
  ...
  const runGenerate = (nextPrompt) => generate(nextPrompt || prompt || defaultPrompt, context || {});
  ...
  return (
    <div className="rounded-2xl border border-[#d8e3f3] bg-white p-4 sm:p-5 shadow-sm">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {quickActions.map((q) => (
          <PremiumButton key={q.label} size="sm" variant="secondary" onClick={() => { setPrompt(q.prompt); runGenerate(q.prompt); }}>{q.label}</PremiumButton>
        ))}
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <PremiumButton iconLeft={<Wand2 className="h-4 w-4" />} disabled={loading} onClick={() => runGenerate()}>{loading ? 'Generating…' : 'Generate'}</PremiumButton>
      </div>
