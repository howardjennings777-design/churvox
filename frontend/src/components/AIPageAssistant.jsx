    ...
    <section className="px-card">
      <div className="px-card__body space-y-4">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <h3 className="font-heading text-lg font-bold text-[#0d1b34]">AI {page} Assistant</h3>
            <p className="text-sm text-[#5b6c87] mt-1">Auto-check: ON · Last scan: {lastScan}</p>
          </div>
        ...
        {loading ? <p className="text-sm text-[#5b6c87]">AI is checking live records…</p> : null}
        ...
        {!!rows.length && <div className="space-y-1">{rows.slice(0, 4).map((r, i) => <p key={`${r}-${i}`} className="text-sm text-[#1a2c4d]">• {r}</p>)}</div>}
        ...
        <div className="rounded-xl border border-[#dbe6f6] bg-white px-3 py-3">
          ...
          {!hasCounts && !rows.length ? <p className="text-sm text-[#5b6c87] mt-1">{emptyMessage}</p> : null}
          <div className="flex flex-wrap gap-2 mt-3">
            {actions.length ? actions : <button type="button" className="px-btn px-btn--secondary px-btn--sm">Review</button>}
          </div>
        </div>

        <p className="text-xs text-[#5b6c87] flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5" /> {safetyNote || "No auto-send, no auto-charge, no payroll changes, and no MYOB writes without owner approval."}</p>
      </div>
    </section>
  );
}
