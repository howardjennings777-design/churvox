...
export default function WorkerContactOfficePanel({ open, onClose, defaultMessage = "I need help with my jobs", jobId = "", jobTitle = "" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[rgba(0,0,0,0.65)] backdrop-blur-[1px] p-3 sm:p-6 flex items-end sm:items-center justify-center" onClick={onClose}>
      <PremiumCard className="w-full max-w-lg max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="px-card__body space-y-3">
          ...
          {!loading && !hasContacts ? (
            <div className="rounded-xl border border-[var(--cx-border)] bg-[var(--cx-surface-2)] p-3">
          ...
          {contacts.map((contact, idx) => (
            <div key={`${contact.email || contact.phone || contact.name}-${idx}`} className="rounded-xl border border-[var(--cx-border)] p-3 space-y-2">
