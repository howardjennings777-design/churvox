...
function slipEditText(item, fallbackTitle, fallbackText) { return `${item?.title || fallbackTitle || "Prepared slip"}\n${item?.meta || fallbackText || "Churvox prepared this for owner approval."}\n${item?.status ? `Status: ${item.status}` : ""}`; }
      ...
      <div className="mx-auto flex min-h-[calc(100vh-24px)] max-w-6xl flex-col rounded-[34px] border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 shadow-2xl md:min-h-[calc(100vh-48px)]">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 p-5 md:p-7">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-sm font-bold leading-6 text-slate-300 md:text-base">{text || "Churvox checked this item and prepared the next owner action."}</p>
          </div>
        ...
        <div className="grid flex-1 gap-5 p-5 md:grid-cols-[1.1fr_.9fr] md:p-7">
          ...
          <aside className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
            ...
            <p className="mt-3 text-sm font-bold leading-6 text-slate-300">Review this selected item. Approve it, edit it, or jump to the full page only when you need to change the full record.</p>
            {approved ? <div className="mt-4 rounded-3xl border border-emerald-300/25 bg-emerald-300/10 p-4 text-sm font-black text-emerald-100">Approved. Churvox has recorded this slip decision.</div> : null}
...
function CommandTile({ label, title, count, text, color, to, actionLabel = "Open page", items = [], children, className = "" }) {
  ...
  const modalItem = selectedItem || null;
  const modalItems = modalItem ? [modalItem] : visibleItems;
  const modalTitle = modalItem?.title || title;
  const modalText = modalItem?.meta || text;
  const editText = modalItem ? slipEditText(modalItem, title, text) : (visibleItems.length ? visibleItems.map((item) => `${item.title}${item.meta ? ` - ${item.meta}` : ""}`).join("\n") : `${title}\n${text || "Churvox checked this area and prepared the next action."}`);
  const openItem = (item, mode = "details") => { setSelectedItem(item); setModalMode(mode); };
  const openCard = (mode = "details") => { setSelectedItem(null); setModalMode(mode); };
  return <><div data-cv-command-tile="true" className={`cv-command-tile relative min-h-[220px] overflow-hidden rounded-[28px] border border-white/10 p-4 pl-7 text-white no-underline ${className}`} style={tileStyle}><SecurityTape color={color} /><div className="flex min-h-full flex-col gap-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-300">{label}</div><h2 className="mt-1 text-[1.35rem] font-black leading-[1.05] tracking-[-0.05em] text-white md:text-2xl">{title}</h2></div>{count !== undefined ? <div className="shrink-0 rounded-2xl bg-emerald-400/15 px-3 py-1.5 text-2xl font-black text-white ring-1 ring-emerald-300/25">{approved ? "✓" : count}</div> : null}</div>{text ? <p className="text-xs font-bold leading-5 text-slate-300 md:text-sm">{text}</p> : null}{visibleItems.length ? <div className="grid gap-2">{visibleItems.slice(0, 3).map((item, index) => <SimpleLine key={`${item.title || item}-${index}`} {...item} onOpen={() => openItem(item, "details")} />)}</div> : null}{children}{approved ? <div className="rounded-2xl border border-emerald-300/25 bg-emerald-300/10 p-3 text-xs font-black text-emerald-100">Approved. Tap a line or View details to review it again.</div> : null}<div className="mt-auto flex flex-wrap gap-2 pt-2"><button type="button" onClick={() => { setApproved(true); openCard("details"); }} className="rounded-xl bg-emerald-300 px-3 py-2 text-xs font-black text-slate-950">Approve</button><button type="button" onClick={() => openCard("edit")} className="rounded-xl bg-white/10 px-3 py-2 text-xs font-black text-white ring-1 ring-white/10">Edit</button><button type="button" onClick={() => openCard("details")} className="rounded-xl bg-white/10 px-3 py-2 text-xs font-black text-white ring-1 ring-white/10">View details</button>{to ? <Link to={to} className="rounded-xl bg-white px-3 py-2 text-xs font-black text-slate-950 no-underline">{actionLabel}</Link> : null}</div></div></div><FullscreenSlipModal mode={modalMode} onClose={() => setModalMode(null)} onApprove={() => setApproved(true)} approved={approved} label={label} title={modalTitle} text={modalText} items={modalItems} editText={editText} to={to} actionLabel={actionLabel} /></>;
}
