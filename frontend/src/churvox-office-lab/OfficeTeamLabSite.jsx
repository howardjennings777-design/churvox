...
const screens = [["command", "Command"], ["team", "Office Team"], ["playbooks", "Playbooks"], ["activity", "Activity"], ["safety", "Safety"]];
...
export default function OfficeTeamLabSite() {
  ...
  return <main className="cvOfficeFinal cvOfficeSite"><Topbar screen={screen} go={go} /><Status metrics={metrics} sourceLabel={sourceLabel} notice={notice} />{screen === "command" && <Command tray={tray} setTray={setTray} counts={counts} decisions={pending} onAction={actionDecision} />}{screen === "team" && <Team roles={roles} activeRole={activeRole} setActiveRole={setActiveRole} />}{screen === "playbooks" && <Playbooks />}{screen === "activity" && <Activity pending={pending} resolved={resolved} />}{screen === "safety" && <Safety />}</main>;
}
...
function Safety() { return <section className="cvSiteScreen"><Header eyebrow="Safety" title="Rules before this moves into the real app" text="Owner approval first. Safe recording second. Public site untouched until ready." /><div className="cvSiteSafetyGrid">{rules.map((rule, index) => <article key={rule}><strong>{index + 1}</strong><p>{rule}</p></article>)}</div></section>; }
...
function cleanScreen(hash = "") { const key = String(hash || "").replace(/^#/, ""); return screens.some(([id]) => id === key) ? key : "command"; }
