    ...
    <main className="cvWorkerRouteShell" data-worker-view={viewKey}>
      ...
      <section className="cvWorkerRoutePhone" aria-label="Churvox worker phone app">
        ...
        {showJobs ? <section className="cvWorkerJobsWorkspace" aria-label="Assigned jobs workspace">
            ...
            {hasWork ? <article className="cvWorkerJobsSelected"><small>Selected job</small><h3>{title}</h3><p>{detail}</p><div><span>{badge}</span><span>{type}</span></div><div className="cvWorkerRouteSteps">{statusSteps.map((step) => <button key={step} type="button" disabled={!hasWork || Boolean(stepBusy) || proofCoachBusy} onClick={() => recordWorkerStep(step)}>{proofCoachBusy && step === "Complete" ? "Checking proof…" : stepBusy === step ? "Saving…" : step}</button>)}</div><section className="cvWorkerRouteNoteBox"><span>Job note</span><h3>What changed?</h3><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add context for this selected job" /><small>Add a note when the proof or completion needs context.</small></section><section className="cvWorkerRouteProof"><label className="cvWorkerProofPicker">Photo proof<input type="file" accept="image/*" capture="environment" multiple disabled={!hasWork || proofBusy} onChange={(event) => setProofFiles(event.target.files)} /></label><button type="button" disabled={!hasWork || proofBusy} onClick={sendProof}>{proofBusy ? "Sending…" : selectedProofNames.length ? `Send ${selectedProofNames.length} proof item${selectedProofNames.length === 1 ? "" : "s"}` : "Send proof note"}</button></section><Link className="cvWorkerJobsOpenToday" to="/worker/today">Open full field view and proof checklist</Link></article> : null}
          </section> : null}
