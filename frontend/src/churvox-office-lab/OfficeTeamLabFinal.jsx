...
const decisions = [
  decision("Money", "Bookkeeper + Extras", "Top priority", "Completed service has extra charge decision", "Stuart’s work was completed and staff noted extra green waste.", ["completed", "staff note", "normal price", "invoice not sent"], "Invoice draft ready. Extra line is held for owner decision.", "Charge extra, include free, or ask staff first?", ["Add charge", "Include free", "Ask staff", "Edit", "Park"]),
  decision("Bookings", "Receptionist", "Next", "Regular client has no next booking", "Jay usually books every 3 weeks but has no next appointment.", ["last visit", "usual cycle", "preferred staff", "calendar space"], "Rebooking message and suggested date are ready.", "Send rebooking message or create booking now?", ["Send", "Book", "Edit date", "Park"]),
  decision("Staff", "Payroll Clerk", "Needs check", "Hours review has one odd timer", "Cam has 36.5 hours ready, with one timer much longer than usual.", ["period", "timers", "manual time", "rate"], "Hours review is ready and the odd timer is flagged.", "Approve, edit timer, or ask Cam?", ["Approve", "Edit", "Ask Cam", "Park"]),
  decision("Clients", "Client Memory", "Low risk", "Service note should become client memory", "Sarah’s appointment note includes colour, shape and sensitivity detail.", ["existing notes", "latest service", "duplicates"], "Client memory update is ready.", "Save this to client memory?", ["Save", "Save + rebook", "Edit", "Ignore"]),
];

const liveActivity = [
  ["Bookkeeper", "Checked completed work and prepared one invoice decision.", "2 min ago"],
  ["Receptionist", "Found one regular client with no next booking.", "4 min ago"],
  ["Payroll Clerk", "Prepared staff hours review and flagged one timer.", "8 min ago"],
  ["Client Memory", "Prepared one service preference for owner save.", "12 min ago"],
];

const buildMap = [
  "Connect Command Queue to Admin Brain scan results",
  "Make playbook labels change wording across the app",
  "Wire approve, edit and park to safe owner-decision routes",
  "Replace demo activity with real mimic activity log",
  "Add role settings so owners can choose review-only or active",
  "Move this finished shell into the real app once approved",
];
...
export default function OfficeTeamLabFinal() {
  ...
  return (
    <main className="cvOfficeFinal">
      <header className="cvOfficeTopbar">
        <div className="cvOfficeBrand">
          <img src={BRAND_ICON} alt="Churvox" />
          <div><strong>Churvox Office Team</strong><span>Command desk · owner approval · hidden build</span></div>
        </div>
        <nav><a href="#command">Command</a><a href="#team">Office Team</a><a href="#playbooks">Playbooks</a><a href="#build">Build Map</a></nav>
      </header>

      <section className="cvOfficeStatus">
        <div className="cvOfficeStatusLead"><span>Office running</span><h1>Staff update the work. Churvox runs the office. The owner approves the decisions.</h1><p>Nothing is sent, synced, charged, or changed until the owner approves it in Command.</p></div>
        <Metric value="35" label="Prepared" note="Ready for review" />
        <Metric value="6" label="Needs owner" note="Ranked by risk" />
        <Metric value="0" label="Auto-sent" note="Approval locked" />
        <Metric value="4" label="Parked" note="Waiting on info" />
      </section>

      <section className="cvOfficeGrid" id="command">
        <section className="cvCommandPanel">
          <PanelHeader eyebrow="Needs owner now" title="Command Queue" text="Prepared by mimics. Organised by risk. Owner decides the next step." />
          <div className="cvDepartmentRail">{departments.map(([key, label, count]) => <button key={key} className={activeDepartment === key ? "active" : ""} onClick={() => chooseDepartment(key)}><strong>{count}</strong><span>{label}</span></button>)}</div>
          <div className="cvDecisionBoard">{decisions.map((item) => <DecisionCard key={item.title} item={item} />)}</div>
        </section>

        <aside className="cvRightRail">
          <section className="cvPlaybookCard" id="playbooks"><PanelHeader eyebrow="Business playbook" title={activePlaybook[0]} />
            <div className="cvPlaybookButtons">{playbooks.map((item) => <button key={item[0]} className={activePlaybook[0] === item[0] ? "active" : ""} onClick={() => setActivePlaybook(item)}>{item[0]}</button>)}</div>
            <dl className="cvPlaybookTerms"><div><dt>Work word</dt><dd>{activePlaybook[1]}</dd></div><div><dt>Staff word</dt><dd>{activePlaybook[2]}</dd></div><div><dt>Customer word</dt><dd>{activePlaybook[3]}</dd></div></dl>
          </section>
          <section className="cvActivityCard"><PanelHeader eyebrow="Live office activity" title="Background work" />
            <ul>{liveActivity.map(([who, text, time]) => <li key={`${who}-${time}`}><strong>{who}</strong><p>{text}</p><small>{time}</small></li>)}</ul>
          </section>
        </aside>
      </section>

      <section className="cvTeamWorkbench" id="team"><PanelHeader eyebrow="Office team" title="Every mimic has a real job." text="Roles sit in the background, prepare admin, and send only clean decisions to Command." />
        <div className="cvTeamLayout"><aside className="cvRoleList">{visibleRoles.map((item) => <button key={item.name} className={activeRole.name === item.name ? "active" : ""} onClick={() => setActiveRoleName(item.name)}><strong>{item.name}</strong><span>{item.summary}</span></button>)}</aside>
          <article className="cvRoleDetail"><div className="cvRoleHero"><span>{activeRole.dept}</span><h3>{activeRole.name}</h3><p>{activeRole.summary}</p></div><div className="cvRoleColumns"><RoleBlock title="Checks" items={activeRole.checks} /><RoleBlock title="Prepares" items={activeRole.prepares} /><section><h4>Owner question</h4><p>{activeRole.ownerAsk}</p></section></div></article>
        </div>
      </section>
...
function decision(tray, roleName, level, title, happened, checked, prepared, need, actions) { return { tray, roleName, level, title, happened, checked, prepared, need, actions }; }
...
function DecisionCard({ item }) { return <article className="cvDecisionCard"><div className="cvDecisionMeta"><span>{item.level}</span><em>{item.tray}</em></div><h3>{item.title}</h3><p>{item.happened}</p><dl><dt>Checked</dt><dd>{item.checked.map((x) => <span key={x}>{x}</span>)}</dd><dt>Prepared</dt><dd>{item.prepared}</dd><dt>Owner decision</dt><dd>{item.need}</dd></dl><div className="cvDecisionActions">{item.actions.map((action, i) => <button key={action} className={i === 0 ? "primary" : ""}>{action}</button>)}</div><small>Approval locked · no auto-send · no auto-sync</small></article>; }
