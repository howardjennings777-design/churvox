...
function build(data) {
  ...
  const money = [...readyInvoice, ...owing, ...draftInvoices];
  ...
  const urgent = [...workReview, ...readyInvoice, ...unassigned, ...issues, ...followUp];

  return { jobs, invoices, quotes, clients, crew, actions, alerts, messages, doneJobs, approved, approvedValue: sum(approved), workReview, readyInvoice, openJobs, active, unassigned, owing, overdue, draftInvoices, quoteFollow, clientWatch, issues, money, dispatch, live, done, followUp, urgent };
}
...
function makeGroup(title, meta, items, tone = "blue") {
  return { type: "action_group", title, code: "WORK SLIP", state: `${items.length} items`, meta, items, tone, amount: sum(items), href: "#" };
}
...
function Card({ title, eyebrow, value, children, onOpen, className = "" }) {
  ...
  return <section className={`xcf-card ${onOpen ? "xcf-card-clickable" : ""} ${className}`} onClick={onOpen || undefined} onKeyDown={onKeyDown} role={onOpen ? "button" : undefined} tabIndex={onOpen ? 0 : undefined}><header><span><small>{eyebrow}</small><b>{title}</b></span>{value !== undefined && <strong>{value}</strong>}{onOpen && <button type="button" onClick={(event) => { event.stopPropagation(); onOpen(); }}>Open slip</button>}</header>{children}</section>;
}
function Empty({ text = "Clear right now." }) { return <div className="xcf-empty">{text}</div>; }

function ActionHub({ m, onPick }) {
  const groups = {
    review: makeGroup("Needs Approval", "Finished jobs waiting for owner approval.", m.workReview, "amber"),
    approved: makeGroup("Been Approved", "Signed-off work ready for the next admin step.", m.approved, "green"),
    invoice: makeGroup("Ready to Invoice", "Approved jobs ready for invoice preparation.", m.readyInvoice, "green"),
    assign: makeGroup("Dispatch", "Unassigned jobs and crew gaps ready to fix.", m.unassigned, "blue"),
    money: makeGroup("Money Desk", "Ready-to-invoice, owing and overdue work.", m.money, "green"),
    issues: makeGroup("Issues", "Missing details, overdue money and risks.", m.issues, "red"),
  };
  return <section className="xcf-card xcf-action-hub-card"><header><span><small>Tap a box. Work in the slip.</small><b>Take Action</b></span><strong>{m.urgent.length}</strong><button type="button" onClick={() => onPick(makeGroup("All Actions", "Everything needing owner attention.", m.urgent, "purple"))}>Open slip</button></header><div className="xcf-action-box-grid"><ActionBox title="Needs Approval" value={m.workReview.length} note="finished jobs" tone="amber" onClick={() => onPick(groups.review)} /><ActionBox title="Been Approved" value={m.approved.length} note="signed off" tone="green" onClick={() => onPick(groups.approved)} /><ActionBox title="Ready to Invoice" value={m.readyInvoice.length} note={cash(sum(m.readyInvoice))} tone="green" onClick={() => onPick(groups.invoice)} /><ActionBox title="Dispatch" value={m.unassigned.length} note="need workers" tone="blue" onClick={() => onPick(groups.assign)} /><ActionBox title="Issues" value={m.issues.length} note="need attention" tone="red" onClick={() => onPick(groups.issues)} /></div><div className="xcf-list xcf-urgent-list">{m.urgent.length ? m.urgent.slice(0, 5).map((x, i) => <Row key={`urgent-${i}`} item={x} onPick={onPick} />) : <Empty text="No priority actions waiting." />}</div></section>;
}

function Dashboard({ m, loading, onPick }) {
  const nextAction = m.workReview.length ? "Review finished work" : m.readyInvoice.length ? "Prepare invoices" : m.unassigned.length ? "Assign workers" : m.issues.length ? "Fix issues" : "All clear";
  return <main className="xcf-shell" data-version="CHURVOX_COMMAND_FLOOR_REAL_FLOW_REBUILD_20260527"><TopBar loading={loading} /><section className="xcf-hero"><div><p>AI OPERATOR COMMAND FLOOR</p><h1>Command Floor</h1><span>Tap anything. Review the slip. Approve the next step.</span></div><aside><i>⚡</i><small>Next Best Action</small><b>{nextAction}</b><em>{m.workReview.length + m.readyInvoice.length} jobs are ready for owner action</em></aside></section><section className="xcf-metrics"><Metric label="Needs Approval" value={m.workReview.length} note="finished jobs" tone="amber" onClick={() => onPick(makeGroup("Needs Approval", "Finished jobs waiting for owner approval.", m.workReview, "amber"))} /><Metric label="Been Approved" value={m.approved.length} note={cash(m.approvedValue)} tone="green" onClick={() => onPick(makeGroup("Been Approved", "Signed-off work ready for the next admin step.", m.approved, "green"))} /><Metric label="Ready to Invoice" value={cash(sum(m.readyInvoice))} note={`${m.readyInvoice.length} jobs`} tone="green" onClick={() => onPick(makeGroup("Ready to Invoice", "Approved work ready for invoice preparation.", m.readyInvoice, "green"))} /><Metric label="Dispatch" value={m.unassigned.length} note="need workers" tone="blue" onClick={() => onPick(makeGroup("Dispatch", "Jobs needing worker assignment.", m.unassigned, "blue"))} /><Metric label="Money Desk" value={cash(sum(m.money))} note="invoice control" tone="purple" onClick={() => onPick(makeGroup("Money Desk", "Ready-to-invoice, owing and overdue work.", m.money, "purple"))} /><Metric label="Issues" value={m.issues.length} note="need attention" tone="red" onClick={() => onPick(makeGroup("Issues", "Risks, missing details and overdue money.", m.issues, "red"))} /></section><section className="xcf-main-grid"><ActionHub m={m} onPick={onPick} /><Card title="Been Approved" eyebrow="Signed-off work ready for admin" value={m.approved.length} onOpen={() => onPick(makeGroup("Been Approved", "Jobs already approved by the owner. Open one to prepare invoice/admin steps.", m.approved, "green"))} className="xcf-approved-page-card"><div className="xcf-money-hero"><span>Approved work</span><b>{cash(m.approvedValue)}</b><small>{m.approved.length} signed-off job{m.approved.length === 1 ? "" : "s"}</small></div><div className="xcf-list">{m.approved.length ? m.approved.slice(0, 4).map((x, i) => <Row key={`approved-${i}`} item={x} onPick={onPick} />) : <Empty text="No approved work yet." />}</div></Card><Card title="Work Review" eyebrow="Jobs waiting for your approval" value={m.workReview.length} onOpen={() => onPick(makeGroup("Work Review", "Approve finished work without leaving Command Floor.", m.workReview, "amber"))} className="xcf-review-card"><div className="xcf-list">{m.workReview.length ? m.workReview.slice(0, 6).map((x, i) => <Row key={`review-${i}`} item={x} onPick={onPick} />) : <Empty text="No finished jobs waiting for review." />}</div></Card><Card title="Money Desk" eyebrow="Your cashflow at a glance" value={cash(sum(m.money))} onOpen={() => onPick(makeGroup("Money Desk", "Ready-to-invoice, owing and overdue work in one slip.", m.money, "green"))} className="xcf-money-card"><div className="xcf-money-hero"><span>Ready to invoice</span><b>{cash(sum(m.readyInvoice))}</b><small>{m.readyInvoice.length} approved jobs</small></div><div className="xcf-money-queue"><p><span>Invoice Queue</span><b>{m.readyInvoice.length}</b><em>{cash(sum(m.readyInvoice))}</em></p><p><span>Overdue</span><b>{m.overdue.length}</b><em>{cash(sum(m.overdue))}</em></p><p><span>Owing</span><b>{m.owing.length}</b><em>{cash(sum(m.owing))}</em></p></div></Card><Card title="Live Crew" eyebrow="Real-time crew activity in the field" value={m.live.length} onOpen={() => onPick(makeGroup("Live Crew", "Crew, job status, GPS and evidence in one place.", m.live, "cyan"))} className="xcf-live-card"><div className="xcf-map-card"><span>Owner crew map</span><b>Timers • GPS • photos • status</b></div><div className="xcf-live-stats"><i>{m.live.length}<small>Crew on jobs</small></i><i>{m.active.length}<small>Active jobs</small></i><i>{m.unassigned.length}<small>Need worker</small></i></div><div className="xcf-list">{m.live.length ? m.live.slice(0, 4).map((x, i) => <Row key={`live-${i}`} item={x} onPick={onPick} />) : <Empty text="No crew on jobs right now." />}</div></Card></section><BottomNav /></main>;
}
...
function DetailDrawer({ picked, onClose, onAction, onPick, workers = [] }) {
  ...
  useEffect(() => {
    ...
    setDraft({ title: picked?.title || "", meta: nextMeta, status: picked?.state || "", worker_id: firstText(raw.assigned_worker_id, raw.worker_id, raw.assigned_to), worker_name: firstText(raw.assigned_worker_name, raw.worker_name, raw.assigned_to_name, raw.assigned_worker_email), message: messageDraftFromPicked(picked, { meta: nextMeta }) });
  ...
  return <aside className={`xcf-drawer xcf-drawer-${isGroup ? "group" : "record"}`}><button className="xcf-close" type="button" onClick={onClose}>Close</button><p>{active.code || active.type}</p><h2>{active.title}</h2><span>{isGroup ? active.meta : brief.summary}</span>{isGroup ? <><section className="xcf-group-summary"><span><small>Items</small><b>{items.length}</b></span><span><small>Total value</small><b>{Number(active.amount || 0) > 0 ? cash(active.amount) : "—"}</b></span><p>{active.meta}</p></section><div className="xcf-slip-list">{items.length ? items.slice(0, 12).map((x, i) => <button className="xcf-slip-row" type="button" key={`${x.type}-${x.id}-${i}`} onClick={() => onPick(x)}><b>{x.title}</b><small>{x.code} · {detailText(x.raw || {}, x.meta)}</small><em>{Number(x.amount || 0) > 0 ? cash(x.amount) : x.state}</em></button>) : <Empty text="Nothing waiting in this slip." />}</div></> : <><dl><div><dt>Status</dt><dd>{active.state}</dd></div><div><dt>Value</dt><dd>{Number(active.amount || 0) > 0 ? cash(active.amount) : "—"}</dd></div><div><dt>Code</dt><dd>{active.code}</dd></div></dl><section className="xcf-approval-brief"><header><small>{isAction ? "AI operator action" : isJobLike ? "AI evidence brief" : "AI approval brief"}</small><b>{isAction ? "Approve or reject this action" : isJobLike ? "Evidence before approval" : "What you are reviewing"}</b></header><p>{brief.description}</p><div>{brief.facts.map(([label, value]) => <span key={label}><small>{label}</small><b>{value}</b></span>)}</div><strong>{brief.outcome}</strong></section><PhotoEvidencePanel photos={photos} preview={previewPhoto} onPreview={setPreviewPhoto} /><section className="xcf-message-draft"><header><small>Customer message</small><b>Draft before sending</b></header><textarea value={draft.message} onChange={(e) => setDraft((d) => ({ ...d, message: e.target.value }))} /><p>No message sends from here yet. This stays owner-editable and approval-first.</p></section>{isJobLike && <section className="xcf-worker-assign"><header><small>Dispatch</small><b>Assign worker in this slip</b></header><select value={draft.worker_id} onChange={(e) => changeWorker(e.target.value)}><option value="">Choose worker</option>{workers.map((worker) => { const wid = workerIdOf(worker); return <option key={wid || worker.title} value={wid}>{workerNameOf(worker)}{worker.state ? ` · ${worker.state}` : ""}</option>; })}</select><p>{draft.worker_name ? `Selected: ${draft.worker_name}` : workers.length ? "Pick a worker, then tap Assign worker." : "No workers loaded yet."}</p></section>}<EditableField label="Title" value={draft.title} onChange={(v) => setDraft((d) => ({ ...d, title: v }))} /><EditableField label={isAction ? "AI reason / drafted message" : "Approval description / edit before saving"} value={draft.meta} onChange={(v) => setDraft((d) => ({ ...d, meta: v }))} textarea /><EditableField label="Status" value={draft.status} onChange={(v) => setDraft((d) => ({ ...d, status: v }))} /></>}<div className="xcf-drawer-actions">{isGroup ? <>{items.length ? <button className="xcf-action-primary" type="button" onClick={() => onPick(items[0])}>Open first item in slip</button> : <button className="xcf-action-muted" type="button" disabled>Nothing waiting</button>}</> : <>{!isAction && <button className="xcf-action-muted" type="button" disabled={busy} onClick={() => run("save")}>Save changes</button>}{isAction && <button className="xcf-action-primary" type="button" disabled={busy} onClick={() => run("approve")}>Approve & execute</button>}{isAction && <button className="xcf-action-danger" type="button" disabled={busy} onClick={() => run("reject")}>Reject action</button>}{isJobLike && <button className="xcf-action-primary" type="button" disabled={busy} onClick={() => run("approve")}>Approve work</button>}{isInvoice && <button className="xcf-action-primary" type="button" disabled={busy} onClick={() => run("approve")}>Approve invoice</button>}{isJobLike && <button type="button" disabled={busy || !draft.worker_id} onClick={() => run("assign")}>Assign worker</button>}{isJobLike && <button type="button" disabled={busy} onClick={() => run("invoice")}>Prepare invoice</button>}{!isAction && <button type="button" disabled={busy} onClick={() => run("message")}>Save message draft</button>}{active.href && active.href !== "#" && <Link to={active.href}>Full page backup</Link>}</>}</div>{notice && <strong className="xcf-drawer-notice">{notice}</strong>}</aside>;
}
...
async function runRecordAction(action, picked, draft, api, reload) {
  ...
  try {
    ...
    if (action === "save") { const endpoint = picked.type === "invoice" ? `/invoices/${id}` : picked.type === "quote" ? `/quotes/${id}` : picked.type === "client" ? `/clients/${id}` : `/jobs/${id}`; const res = await patchWithFallback(api, endpoint, { title: draft.title, description: draft.meta, status: draft.status }); if (apiOk(res)) { await reload(); return "Saved in this slip."; } return `Could not save: ${apiError(res)}`; }
    if (action === "approve") { if (picked.type === "invoice") { const res = await patchWithFallback(api, `/invoices/${id}`, { status: "approved" }); if (apiOk(res)) { await reload(); return "Invoice approved."; } return `Could not approve invoice: ${apiError(res)}`; } if (picked.type !== "job" && picked.type !== "work_review") return "Only jobs, work reviews and invoices can be approved from this slip."; const res = await patchWithFallback(api, `/jobs/${id}`, { owner_review_status: "approved", work_review_status: "approved", reviewed: true }); if (apiOk(res)) { await reload(); return "Work approved. It now belongs in Been Approved."; } return `Could not approve work: ${apiError(res)}`; }
    if (action === "assign") { if (picked.type !== "job" && picked.type !== "work_review") return "Select a job before assigning a worker."; if (!draft.worker_id) return "Choose a worker first."; const res = await patchWithFallback(api, `/jobs/${id}`, { assigned_worker_id: draft.worker_id, assigned_worker_name: draft.worker_name, assigned_to: draft.worker_id, status: picked.raw?.status || "assigned" }); if (apiOk(res)) { await reload(); return `Assigned to ${draft.worker_name || "selected worker"}.`; } return `Could not assign worker: ${apiError(res)}`; }
    if (action === "invoice") { if (picked.type !== "job" && picked.type !== "work_review") return "Select a job or work review item before preparing an invoice."; const payload = invoicePayloadFromPicked(picked, draft); if (!payload.ok) return payload.error; const res = await api.post("/invoices", payload.data); if (!apiOk(res)) return `Could not prepare invoice: ${apiError(res)}`; const invoiceId = recordIdFromResponse(res); if (invoiceId && id) { try { await patchWithFallback(api, `/jobs/${id}`, { draft_invoice_id: invoiceId, invoice_description_draft: payload.data.description }); } catch (_err) {} } await reload(); return invoiceId ? `Draft invoice prepared: INV ${invoiceId}. Open Money Desk or Full page backup to review/send.` : "Draft invoice prepared. Open Money Desk to review/send."; }
    if (action === "message") { const message = draft.message || messageDraftFromPicked(picked, draft); if (!message) return "No message draft to save."; const endpoint = picked.type === "invoice" ? `/invoices/${id}` : picked.type === "quote" ? `/quotes/${id}` : picked.type === "client" ? `/clients/${id}` : `/jobs/${id}`; const res = await patchWithFallback(api, endpoint, { customer_message_draft: message, draft_message: message, last_message_draft: message }); if (apiOk(res)) { await reload(); return "Message draft saved. Nothing has been sent."; } return `Could not save message draft: ${apiError(res)}`; }
  } catch (err) {
