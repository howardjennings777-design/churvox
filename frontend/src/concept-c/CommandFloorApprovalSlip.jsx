...
function Field({ label, value, onChange, textarea = false, type = "text" }) {
  return <label className="cfs-field"><span>{label}</span>{textarea ? <textarea value={value || ""} onChange={(e) => onChange(e.target.value)} /> : <input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} />}</label>;
}
...
export default function CommandFloorApprovalSlip({ picked, onClose, onAction, onPick, workers = [], jobs = [] }) {
  ...
  const value = moneyNumber(draft.amount, active.amount) ? cash(moneyNumber(draft.amount, active.amount)) : "—";
  ...
  return <aside className="cfs-overlay" data-version="CHURVOX_EDITABLE_JOB_FORM_WORK_SLIP_20260527"><section className="cfs-sheet">
    <header className="cfs-head"><div><p>WORK SLIP</p><h2>{draft.customer_name || active.title}</h2><em>{active.code || active.type}</em><span>Review the proper job form Churvox filled in, edit anything if needed, then approve.</span></div><button type="button" onClick={onClose}>× Close</button></header>
    <section className="cfs-facts"><Fact label="Status" value={draft.status} /><Fact label="Value" value={value} /><Fact label="Site" value={draft.site_address} /><Fact label="Customer" value={draft.customer_name} /><Fact label="Worker" value={draft.worker_name || (recommendation?.best ? workerNameOf(recommendation.best.worker) : "No safe match")} /><Fact label="Invoice" value={draft.invoice_status} /></section>
    <section className="cfs-decision-grid"><article className="cfs-decision cfs-happened"><header><i>1</i><b>What happened</b></header><ul>{situation.map((x) => <CheckLine key={x}>{x}</CheckLine>)}</ul></article><article className="cfs-decision cfs-ai"><header><i>2</i><b>AI Recommendation</b></header><div className="cfs-ai-box"><strong>✓ Approve work</strong><p>{recommendation?.best ? `No worker conflict found, ${workerNameOf(recommendation.best.worker)} selected, invoice draft prepared, and customer update drafted.` : "Review the job form, choose a worker if needed, then approve when ready."}</p></div></article><article className="cfs-decision cfs-attention"><header><i>3</i><b>Needs attention</b></header><ul>{needs.map((x) => <WarnLine key={x}>{x}</WarnLine>)}</ul></article></section>
    <h3 className="cfs-section-title">Editable job form Churvox prepared</h3>
    <section className="cfs-job-form cfs-job-form-editable">
      <article><header><small>Job details</small><b>Proper editable job record</b></header><div className="cfs-form-grid"><Field label="Job title" value={draft.title} onChange={updateDraft(setDraft, "title")} /><Field label="Client / customer" value={draft.customer_name} onChange={updateDraft(setDraft, "customer_name")} /><Field label="Site address" value={draft.site_address} onChange={updateDraft(setDraft, "site_address")} /><Field label="Service type" value={draft.service_type} onChange={updateDraft(setDraft, "service_type")} /><Field label="Scheduled date / time" value={draft.scheduled} onChange={updateDraft(setDraft, "scheduled")} /><Field label="Status" value={draft.status} onChange={updateDraft(setDraft, "status")} /><Field label="Job description / scope" value={draft.meta} onChange={updateDraft(setDraft, "meta")} textarea /></div></article>
      <article><header><small>Assignment</small><b>Worker selected by AI</b></header><div className="cfs-form-grid cfs-form-grid-small"><label className="cfs-field"><span>Assigned worker</span><select value={draft.worker_id || ""} onChange={(e) => changeWorker(e.target.value)}><option value="">Choose worker</option>{workers.map((worker) => { const wid = workerIdOf(worker); const blocked = isWorkerBlocked(worker) || workerHasActiveConflict(worker, jobs, active); return <option key={wid || worker.title} value={wid}>{workerNameOf(worker)}{blocked ? ` · ${blocked}` : worker.state ? ` · ${worker.state}` : ""}</option>; })}</select></label><Field label="Assigned worker name" value={draft.worker_name} onChange={updateDraft(setDraft, "worker_name")} /><Field label="AI reason / conflict check" value={recommendation?.summary || "Needs manual check"} onChange={() => {}} textarea /></div></article>
      <article><header><small>Pricing + invoice prep</small><b>Editable admin fields</b></header><div className="cfs-form-grid cfs-form-grid-small"><Field label="Pricing type" value={draft.pricing_type} onChange={updateDraft(setDraft, "pricing_type")} /><Field label="Amount" type="number" value={draft.amount} onChange={updateDraft(setDraft, "amount")} /><Field label="Invoice status" value={draft.invoice_status} onChange={updateDraft(setDraft, "invoice_status")} /><Field label="Invoice description" value={draft.invoice_description} onChange={updateDraft(setDraft, "invoice_description")} textarea /></div></article>
      <article><header><small>Completion</small><b>Editable worker evidence</b></header><div className="cfs-form-grid cfs-form-grid-small"><Field label="Worker notes" value={draft.worker_notes} onChange={updateDraft(setDraft, "worker_notes")} textarea /><Field label="Photos" value={photos.length ? `${photos.length} uploaded` : "No photos saved"} onChange={() => {}} /><Field label="Owner approval" value="Waiting for your approval" onChange={() => {}} /></div></article>
    </section>
