...
test.describe('Paid-launch auth contract v2', () => {
  test('signup blocks short passwords and browser blocks missing consent', async ({ page }) => {
    ...
    const consent = page.locator('input[name="termsAccepted"]');
    await consent.uncheck();
    const beforeRegistrations = api.requests.filter((item) => item.pathname === '/api/auth/register').length;
    await page.getByRole('button', { name: 'Create account and choose plan' }).click();

    await expect(consent).not.toBeChecked();
    expect(await consent.evaluate((node) => node.validity.valueMissing)).toBe(true);
    expect(page.url()).toMatch(/\/signup/i);
    expect(api.requests.filter((item) => item.pathname === '/api/auth/register').length).toBe(beforeRegistrations);
  });
