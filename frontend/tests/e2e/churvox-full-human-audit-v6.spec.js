...
function textHas(value, token) { return JSON.stringify(value || {}).toLowerCase().includes(String(token || '').toLowerCase()); }
...
async function pageText(page) { return (await page.locator('body').innerText({ timeout: 8000 }).catch(() => '')).replace(/\s+/g, ' ').trim(); }
async function attachReport(testInfo, name, data) { await testInfo.attach(name, { body: JSON.stringify(data, null, 2), contentType: 'application/json' }); }
...
async function clickLogin(page) {
  ...
  if (await submit.isVisible().catch(() => false)) { await submit.click(); await waitSettled(page); return; }
  const button = page.getByRole('button', { name: /log in|login|sign in/i }).first();
  if (await button.isVisible().catch(() => false)) { await button.click(); await waitSettled(page); return; }
...
async function closeTransientUI(page) {
  for (let i = 0; i < 6; i += 1) {
    ...
    const overlay = page.locator('.freshPopupBackdrop, .freshClientPopupBackdrop, .freshJobPopupBackdrop, .freshReviewPopupBackdrop, .freshRoutePopupBackdrop, [role="dialog"], dialog, [aria-modal="true"]').first();
    if (await overlay.isVisible().catch(() => false)) {
      const closeButton = overlay.getByRole('button', { name: /cancel|close|done|back|dismiss|no/i }).last();
      if (await closeButton.isVisible().catch(() => false)) {
        await closeButton.scrollIntoViewIfNeeded().catch(() => null);
        await closeButton.click({ force: true, timeout: 2500 }).catch(() => null);
      } else {
        await page.keyboard.press('Escape').catch(() => null);
        await overlay.click({ position: { x: 8, y: 8 }, force: true }).catch(() => null);
      }
      closed = true;
    }

    const notifyClose = page.locator('.freshNotifyBell').getByRole('button', { name: /close|dismiss|back/i }).first();
    if (await notifyClose.isVisible().catch(() => false)) {
      await notifyClose.click({ force: true, timeout: 2500 }).catch(() => null);
      closed = true;
    }

    const mobileMore = page.locator('.freshMobileMore').first();
    if (await mobileMore.isVisible().catch(() => false)) {
      await page.keyboard.press('Escape').catch(() => null);
      await page.mouse.click(6, 6).catch(() => null);
      closed = true;
    }
...
function controlKey(control) { return `${control.tag}|${control.href}|${control.label}`; }
...
async function centerControl(page, locator) {
  await locator.evaluate(element => element.scrollIntoView({ block: 'center', inline: 'center', behavior: 'instant' })).catch(() => null);
...
async function clickControl(page, route, control, routeReport) {
  ...
  if (!match) { routeReport.unavailableSkipped.push({ label, href: control.href, reason: 'not found after reload' }); return; }
  if (match.disabled || match.pointerEvents === 'none') { routeReport.unavailableSkipped.push({ label, href: control.href, reason: 'disabled or pointer-events none after reload' }); return; }
  const locator = page.locator(`[data-churvox-qa-control="${match.id}"]`).first();
  if (!(await locator.isVisible().catch(() => false))) { routeReport.unavailableSkipped.push({ label, href: control.href, reason: 'locator not visible after reload' }); return; }
  ...
  try {
    ...
    const openedDialog = await page.locator('[role="dialog"], dialog, .modal, [aria-modal="true"], .freshPopupBackdrop, .freshClientPopupBackdrop, .freshJobPopupBackdrop, .freshReviewPopupBackdrop, .freshRoutePopupBackdrop').first().isVisible().catch(() => false);
...
async function sweepRoute(page, route, report, testInfo) {
  if (!Array.isArray(report.routes)) report.routes = [];
  ...
  for (const control of initialControls) {
    ...
    const isDanger = hardDanger.test(label);
    if (isDanger && (!DANGEROUS_GUARD_CLICKS || !allowedDangerProbe.test(label))) { routeReport.dangerousSkipped.push(control); continue; }
    await page.goto(route, { waitUntil: 'domcontentloaded' });
    await waitSettled(page);
    await closeTransientUI(page);
    await clickControl(page, route, control, routeReport);
  }
  await attachReport(testInfo, `${report.account || 'flow'}-${route.replace(/[^a-z0-9]+/gi, '-')}-${testInfo.project.name}.json`, routeReport);
...
async function apiSession(page) { const token = await page.evaluate(() => window.localStorage.getItem('token') || ''); return { request: page.context().request, token }; }
function apiOptions(session, extra = {}) { return { ...extra, headers: { ...(extra.headers || {}), ...(session?.token ? { Authorization: `Bearer ${session.token}` } : {}) } }; }
async function getJson(session, url) { const res = await session.request.get(apiUrl(url), apiOptions(session)); const body = await res.json().catch(() => ({})); return { ok: res.ok(), status: res.status(), body }; }
async function postJson(session, url, data) { const res = await session.request.post(apiUrl(url), apiOptions(session, { data })); const body = await res.json().catch(() => ({})); return { ok: res.ok(), status: res.status(), body }; }
async function patchJson(session, url, data) { const res = await session.request.patch(apiUrl(url), apiOptions(session, { data })); const body = await res.json().catch(() => ({})); return { ok: res.ok(), status: res.status(), body }; }
...
test.describe('Churvox full human audit v6', () => {
  ...
  for (const route of workerRoutes) {
    test(`worker route ${route} renders and visible safe controls click`, async ({ page }, testInfo) => {
      test.setTimeout(240000);
      const report = { account: 'worker', project: testInfo.project.name, route, routes: [], errors: [] };
      await watchPage(page, report);
  ...
  test('owner and worker: full boss-to-worker and worker-to-boss message loop', async ({ browser }, testInfo) => {
    ...
    const createJob = await postJson(ownerSession, '/api/jobs', { title: `Playwright full worker message job ${id}`, job_type: 'other', customer_name: 'Playwright Full Audit Customer', address: '1 Test Street, Wellington', scheduled_date: new Date().toISOString(), scheduled_time: '09:00', estimated_duration: 60, price: 0, assigned_worker_id: workerId, worker_instructions: bossToken, notes: bossToken });
    ...
    let jobId = idOf(job);
    if (!jobId) { job = await getJob(ownerSession, '', bossToken); jobId = idOf(job); }
    ...
    report.flow.push({ step: 'worker loaded assigned job through API', job: workerSeesJob });
    ...
    const complete = await postJson(workerSession, `/api/worker/jobs/${encodeURIComponent(jobId)}/complete`, { worker_notes: workerDoneToken, photos: [], completed_by_worker: true, work_review_status: 'ready_for_review', review_status: 'ready_for_review', owner_review_status: 'ready_for_review' });
    ...
    await attachReport(testInfo, `worker-employer-full-loop-v6-${testInfo.project.name}.json`, report);
