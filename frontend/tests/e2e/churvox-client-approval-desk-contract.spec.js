...
test.describe("Churvox connected owner approval desk contract", () => {
  ...
  test("edits canonical client and job fields before Command approval", async () => {
    const desk = readFrontend("src/churvox-office-os/OfficeOSApprovalDesk.jsx");

    for (const field of ["name", "phone", "email", "address", "title", "client", "date", "worker", "price", "notes"]) {
  ...
  test("keeps repeat clicks, business scope and external side effects locked", async () => {
    ...
    expect(desk).toContain('if (!String(draft[config.primaryField] || "").trim() || busyId) return');
    expect(desk).toContain("disabled={busy || Boolean(busyId) || !primaryReady}");
