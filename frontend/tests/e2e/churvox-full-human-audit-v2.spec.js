...
const ownerRoutes = [
  ...
  '/dashboard#clients', '/clients', '/clients/new',
  '/dashboard#quotes', '/quotes', '/quotes/new',
  '/dashboard#invoices', '/invoices', '/invoices/new',
  '/dashboard#dispatch', '/calendar', '/dashboard#team', '/team',
  '/dashboard#command', '/dashboard#payroll', '/payroll',
  '/dashboard#automation', '/automation', '/dashboard#xero',
  '/settings', '/plans', '/support', '/help',
];
...
async function findControl(page, control) {
  const controls = await tagControls(page);
  return controls.find(c => c.label === control.label && c.href === control.href && c.tag === control.tag) || controls[control.index] || null;
}
...
async function clickControl(page, route, control, routeReport) {
  ...
  await locator.scrollIntoViewIfNeeded({ timeout: 4000 }).catch(() => null);
...
async function sweepRoute(page, route, report) {
  ...
  for (const control of initialControls) {
    ...
    const isDanger = hardDanger.test(label);
    if (isDanger && (!process.env.CHURVOX_FULL_AUDIT_DANGEROUS || !allowedDangerProbe.test(label))) {
...
test.describe.serial('Churvox full human audit v2', () => {
  ...
  test('owner app: every major route renders and every visible safe control clicks', async ({ page }, testInfo) => {
    ...
    for (const route of ownerRoutes) await sweepRoute(page, route, report);
    await attachReport(testInfo, `owner-full-human-audit-v2-${testInfo.project.name}.json`, report);
  ...
  test('worker app: every worker route renders and every visible safe control clicks', async ({ page }, testInfo) => {
    ...
    for (const route of workerRoutes) await sweepRoute(page, route, report);
    await attachReport(testInfo, `worker-full-human-audit-v2-${testInfo.project.name}.json`, report);
    expect(report.errors, 'browser/page/backend errors').toEqual([]);
  });
  ...
  test('owner and worker: full boss-to-worker and worker-to-boss message loop', async ({ browser }, testInfo) => {
    ...
    const bellClicked = await ownerPage.getByRole('button', { name: /notification|bell|alert|activity/i }).first().click({ timeout: 4000 }).then(() => true).catch(() => false);
    if (bellClicked) await waitSettled(ownerPage);
    ...
    await attachReport(testInfo, `worker-employer-full-loop-v2-${testInfo.project.name}.json`, report);
