...
async function fillAny(page, names, value) {
  for (const name of names) {
    const candidates = [page.getByLabel(new RegExp(name, 'i')).first(), page.getByPlaceholder(new RegExp(name, 'i')).first(), page.locator(`input[name*="${name}" i], textarea[name*="${name}" i], select[name*="${name}" i]`).first()];
...
async function watchPage(page, report) {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (msg.type() === 'error' && !/favicon|manifest|ResizeObserver|AbortError|net::ERR_ABORTED|401|403|404/i.test(text)) report.errors.push({ type: 'console', message: text.slice(0, 800) });
  });
  page.on('response', response => {
    const url = response.url();
    if (response.status() >= 500 && /churvox|grassley-backend|onrender|localhost|127\.0\.0\.1/i.test(url)) report.errors.push({ type: 'http', status: response.status(), url });
  });
}
...
async function clickControl(page, route, control, routeReport) {
  ...
  try {
    ...
    const openedDialog = await page.locator('[role="dialog"], dialog, .modal, [aria-modal="true"], .freshPopupBackdrop').first().isVisible().catch(() => false);
...
test.describe('Churvox full human audit v5', () => {
  ...
  test('owner and worker: full boss-to-worker and worker-to-boss message loop', async ({ browser }, testInfo) => {
    ...
    await attachReport(testInfo, `worker-employer-full-loop-v5-${testInfo.project.name}.json`, report);
