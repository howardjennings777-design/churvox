...
async function createClientThroughStudio(page, run) {
  ...
  const createMenu = page.getByRole('dialog', { name: /Create in Churvox/i });
  await expect(createMenu).toBeVisible();
  const clientChoice = createMenu.locator('button').filter({ hasText: /\bClient\b/i }).first();
  await expect(clientChoice, 'Client choice did not render in the Create menu').toBeVisible();
  await clientChoice.click();
  ...
  await drawer.getByLabel('Access notes', { exact: true }).fill(`Created through current Studio UI ${run}`);
...
async function createAssignedJob(request, ownerToken, worker, run) {
  ...
  const result = await api(request, 'post', '/api/jobs', ownerToken, {
    ...
    notes: instruction,
    source: 'studio-human-owner-worker-test-v2',
  });
  expect(result.response.ok(), `Owner could not create assigned job ${result.response.status()}: ${JSON.stringify(result.body).slice(0, 900)}`).toBeTruthy();
...
async function clickWorkerStep(page, request, workerToken, jobId, title, label, expected) {
...
async function closeCommandItems(request, ownerToken, tokens) {
  const listed = await api(request, 'get', `/api/command/slips?ts=${Date.now()}`, ownerToken);
  if (!listed.response.ok()) return;
  for (const slip of rowsFrom(listed.body).filter((row) => tokens.some((token) => contains(row, token)))) {
    const id = idOf(slip);
    if (!id) continue;
    const ignored = await api(request, 'post', `/api/command/slips/${encodeURIComponent(id)}/ignore`, ownerToken, { action: 'Ignore', note: 'Studio human audit cleanup' });
    if (!ignored.response.ok()) await api(request, 'post', `/api/command/field-slips/${encodeURIComponent(id)}/dismiss`, ownerToken, { note: 'Studio human audit cleanup' });
  }
}
...
async function cleanupRecord(request, ownerToken, kind, id, token) {
  if (!id && token) {
    ...
    id = idOf(rowsFrom(listed.body).find((row) => contains(row, token)));
  }
  if (!id) return;
...
test.describe('Current Studio real owner-worker loop v2', () => {
  test.setTimeout(600_000);

  test('owner creates a client, worker completes field work, Command receives the issue and owner prepares an invoice', async ({ browser, request }) => {
    ...
    try {
      client = await test.step('Owner creates a client through the current Studio drawer', () => createClientThroughStudio(ownerPage, run));
      job = await test.step('Owner creates an isolated assigned job', () => createAssignedJob(request, ownerToken, worker, run));

      await test.step('Worker sees the job and attaches photo proof', async () => {
        await workerPage.goto(`${BASE_URL}/worker/jobs`, { waitUntil: 'domcontentloaded' });
        await expect(workerPage.getByText(job.title).first()).toBeVisible({ timeout: 25_000 });
      ...
      await test.step('Worker completes every lifecycle state', async () => {
      ...
      await test.step('Worker sends an issue and owner sees it in Command', async () => {
        ...
        await expect.poll(async () => (await ownerPage.locator('body').innerText()).includes(issueToken), {
          message: 'Worker issue did not appear in Command',
          timeout: 30_000,
          intervals: [700, 1200, 2200, 3500],
        }).toBe(true);
      });

      await test.step('Owner prepares the invoice from the completed job', async () => {
      ...
      await closeCommandItems(request, ownerToken, [issueToken, job.title, client.name]);
