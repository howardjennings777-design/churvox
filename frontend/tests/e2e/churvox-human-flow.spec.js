...
async function checkReadableAndClickable(page, label) {
  ...
  const result = await page.evaluate(() => {
    ...
    controls.forEach((el, i) => {
      ...
      if (r.width < 14 || r.height < 14) issues.push(`tiny control ${label || i + 1} tag=${el.tagName} type=${el.getAttribute("type") || ""} name=${el.getAttribute("name") || ""} placeholder=${el.getAttribute("placeholder") || ""} class=${el.className || ""} aria=${el.getAttribute("aria-label") || ""} title=${el.getAttribute("title") || ""} size=${Math.round(r.width)}x${Math.round(r.height)}`);
